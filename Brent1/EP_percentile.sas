...
filename EP3 "&path.rollins_ed_pv_data_2011q1.xls";
filename EP4 "&path.Rollins_edmd_2009q3_2011q2.xls";
filename EP5 "&path.ed_pv_data_2011q3 rollins.xls";
filename EP6 "&path.ed_pv_data_2011_ytd.xls";
...
data edmd2;
	set edmd20(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
...
data edmd3;
	set edmd30(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

PROC IMPORT OUT= edmd40 
            DATAFILE= EP4 
            DBMS=EXCEL REPLACE;
     sheet="edmd"; 
     GETNAMES=YES;
...
data edmd4;
	set edmd40(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

PROC IMPORT OUT= edmd50 
            DATAFILE= EP5 
            DBMS=EXCEL REPLACE;
     sheet="edmd"; 
     GETNAMES=YES;
...
data edmd5;
	set edmd50(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

PROC IMPORT OUT= edmd60 
            DATAFILE= EP6 
            DBMS=EXCEL REPLACE;
     sheet="edmd"; 
     GETNAMES=YES;
...
data edmd6;
	set edmd60(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;
...
data brent.EDMD;
	set edmd1 edmd2 edmd3(in=A) edmd4(in=B) edmd5(in=C) edmd6;

	if (A or B or C) and visit_date>"1Jan2011"d then delete;
	...
	if attending_num=. then delete;

	if md_to_exit_minutes<0 then md_to_exit_minutes=.;
...
proc contents data=brent.edmd;run;
...
data pnp2;
	set pnp20(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

PROC IMPORT OUT= pnp30 
            DATAFILE= EP3 
            DBMS=EXCEL REPLACE;
     sheet="PNP"; 
     GETNAMES=YES;
...
data pnp3;
	set pnp30(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

PROC IMPORT OUT= pnp40 
            DATAFILE= EP4 
            DBMS=EXCEL REPLACE;
     sheet="PNP"; 
     GETNAMES=YES;
...
data pnp4;
	set pnp40(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;
...
data pnp5;
	set pnp50(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;
...
data pnp6;
	set pnp60(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;
...
data brent.PNP;
	set pnp1 pnp2 pnp3(in=A) pnp4(in=B) pnp5(in=C) pnp6;
	if (A or B or C) and visit_date>"1Jan2011"d then delete;
	if study_group="age" then group=1;
	if study_group="fever" then group=2;
	if study_group="head injury" then group=3;
...
%macro rank(data, out, varlist)/minoperator;
	...
	data sub;
		merge sub tmp(in=A keep=pid n); by pid;
		if A;
		if visit_date>'1Sep2010'd and n<25 then delete;
	run;
	...
	data tab&i;
		set tab&i;
		if pid=. or  _type_=0 then delete;
		drop _type_ _freq_;
		format &var 5.2;
	run;
	%end;
	...
	proc rank groups=100 out=rank&i ties=low; 
     	var &var;
   		ranks rank_&var;
	run;
	...
	data &data._&out;
		...
		%end;
	run;		
	proc rank groups=100 out=&data._&out ties=low; 
     	var rk;
   		ranks rank;
	run;
...
proc export data=brent.EDMD_Age outfile='H:\SAS_Emory\Consulting\Brent1\EDMD.xls' dbms=xls replace; sheet='AGE'; run;
proc export data=brent.EDMD_fever outfile='H:\SAS_Emory\Consulting\Brent1\EDMD.xls' dbms=xls replace; sheet='fever'; run;
proc export data=brent.EDMD_injury outfile='H:\SAS_Emory\Consulting\Brent1\EDMD.xls' dbms=xls replace; sheet='injury'; run;
proc export data=brent.EDMD_respiratory outfile='H:\SAS_Emory\Consulting\Brent1\EDMD.xls' dbms=xls replace; sheet='respiratory'; run;
proc export data=brent.EDMD_overall outfile='H:\SAS_Emory\Consulting\Brent1\EDMD.xls' dbms=xls replace; sheet='overall'; run;
...
proc export data=brent.PNP_Age outfile='H:\SAS_Emory\Consulting\Brent1\PNP.xls' dbms=xls replace; sheet='AGE'; run;
proc export data=brent.PNP_fever outfile='H:\SAS_Emory\Consulting\Brent1\PNP.xls' dbms=xls replace; sheet='fever'; run;
proc export data=brent.PNP_injury outfile='H:\SAS_Emory\Consulting\Brent1\PNP.xls' dbms=xls replace; sheet='injury'; run;
proc export data=brent.PNP_respiratory outfile='H:\SAS_Emory\Consulting\Brent1\PNP.xls' dbms=xls replace; sheet='respiratory'; run;
proc export data=brent.PNP_overall outfile='H:\SAS_Emory\Consulting\Brent1\PNP.xls' dbms=xls replace; sheet='overall'; run;

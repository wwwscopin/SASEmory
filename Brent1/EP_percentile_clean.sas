...
filename EP7 "&path.Rollins_edmd_2011q4.xls";
...
data edmd7;
	set edmd70(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;
...
data brent.EDMD;
	set edmd4(in=A) edmd6 edmd7;

	if A  and visit_date>"1Jan2011"d then delete;
...
data pnp7;
	set pnp70(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
	if vh='SR' then Visit_hospital=2; else Visit_hospital=1;
	if apl='SR' then attending_primary_location=2; else attending_primary_location=1;
	if dpl='SR' then  disposition_primary_location=2; else  disposition_primary_location=1;
run;

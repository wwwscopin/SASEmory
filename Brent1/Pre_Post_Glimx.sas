...
proc glimmix data=lr;
	class pid;
	model rr=mon0 mon1/s;
...
%if &var=los %then %do;
	axis2 	label=(h=2.5 a=90 "&varname") value=(h=2) order= (100 to 140 by 5) offset=(.25 in, .25 in) minor=(number=1);
%end;

%if &var=rr %then %do;
	axis2 	label=(h=2.5 a=90 "&varname") value=(h=2) order= (0 to 5 by 0.5) offset=(.25 in, .25 in) minor=(number=1);
%end;
...
%if &var=rch  %then %do;
axis2 label=(h=2.5 a=90 "&varname") value=(h=2) order= (0 to 0.08 by 0.01) offset=(.25 in, .25 in) minor=none;
%end;
...
%if &var=iap  %then %do;
axis2 label=(h=2.5 a=90 "&varname") value=(h=2) order= (0 to 8 by 1) offset=(.25 in, .25 in) minor=none;
%end;
...
%if &var=izp %then %do;
axis2 label=(h=2.5 a=90 "&varname") value=(h=2) order= (0 to 4 by 0.5) offset=(.25 in, .25 in) minor=none;
%end;

...
data &var;
	length c c1-c&m $60 row $80 variable $120;
...
*ods trace off;
	...
	data &var;
		length col1-col&h $60 row $80 variable $120;
		merge &var wp;
...
%if &last_var=1 and &prn=1 %then %do;
	...
	proc report data =&data_out nowd headskip missing split = "*" style(header) = {just=center};
		...
		define row  /"Characteristic" style=[asis=on just=left vjust=middle width=2.0in];
...
%table(data_in=hsb,data_out=test,gvar=prog,var=female,type=cat, last_var=1, title="TableB Summary");quit;
*/

%macro anno(data=, where=, xvar=, first_anno=1, color="black");
%if &first_anno = 1 %then %do; data anno; if 1=1 then delete; run;	%end;

DATA &xvar; 
	set &data;
	where group=0;
	xsys='2'; ysys='2';  color=&color;
	X=&xvar; 	y=estimate; FUNCTION='MOVE'; when = 'A';  OUTPUT; * start at mean ;
	Y=lower; 	FUNCTION='DRAW'; when = 'A'; line=1; size=1; OUTPUT; * draw down;
	LINK TIPS; * make bar;
	Y=upper;	FUNCTION='DRAW'; when = 'A'; line=1; size=1;  OUTPUT; * draw up; 
	LINK TIPS; * make bar;
	TIPS:
    X=&xvar*(1-0.01); FUNCTION='DRAW'; when = 'A'; line=1; size=1; OUTPUT;
	X=&xvar*(1+0.01); FUNCTION='DRAW'; when = 'A'; line=1; size=1; OUTPUT;
  	X=&xvar;     FUNCTION='MOVE'; when = 'A';  OUTPUT;
  	return;
run;

data anno;
	length col1-col&h $60 row $80 variable $120;
	%if (&first_anno = 1) %then %do; set &xvar;	%end;
	%else %do;	set &anno &xvar; %end;
run;
%mend anno;

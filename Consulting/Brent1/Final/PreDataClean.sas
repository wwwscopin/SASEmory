...
filename EP "&path.Rollins_EDMD_2009q2_2012q4.xlsx";


PROC IMPORT OUT= edmd0 
            DATAFILE= EP 
            DBMS=EXCEL REPLACE;
     sheet="Research__base_rollins_expo"; 
     GETNAMES=YES;
     MIXED=YES;
     SCANTEXT=YES;
     USEDATE=YES;
...
data edmd;
	set edmd0(rename=(Visit_hospital=vh attending_primary_location=apl  disposition_primary_location=dpl));
...
data brent.EDMD;
	...
	if study_groups="age" then group=1;
	if study_groups="fever" then group=2;
	if study_groups="head injury" then group=3;
	if study_groups="respiratory" then group=4;
	...
	drop study_groups IBEX_EXIT_CODE;
...
proc export data=brent.EDMD_Age outfile='H:\SAS_Emory\Consulting\Brent1\Final\EDMD.xls' dbms=xls replace; sheet='AGE'; run;
proc export data=brent.EDMD_fever outfile='H:\SAS_Emory\Consulting\Brent1\Final\EDMD.xls' dbms=xls replace; sheet='fever'; run;
proc export data=brent.EDMD_injury outfile='H:\SAS_Emory\Consulting\Brent1\Final\EDMD.xls' dbms=xls replace; sheet='injury'; run;
proc export data=brent.EDMD_respiratory outfile='H:\SAS_Emory\Consulting\Brent1\Final\EDMD.xls' dbms=xls replace; sheet='respiratory'; run;
proc export data=brent.EDMD_overall outfile='H:\SAS_Emory\Consulting\Brent1\Final\EDMD.xls' dbms=xls replace; sheet='overall'; run;

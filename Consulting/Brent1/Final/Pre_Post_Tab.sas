options nofmterr /*orientation=landscape*/ orientation=portrait spool;
...
proc format;
	...
	value item 1="Length of ED stay, Mean&pm.SD[Q1-Q3], N"
               2="Return rate"
			   3="Admitted to hospital"
			   4="Count of number of Labs(per 100 patients), Mean&pm.SD, N"
			   5="Patient receive abd and/or pelvis CT"
			   6="Count of number of charges for head CT (per 100 patients), Mean&pm.SD, N"
			   7="Count of number of charges for chest X-ray (per 100 patients), Mean&pm.SD, N"
			   8="Count of number of charges for abdominal x-ray (per 100 patients), Mean&pm.SD, N"
			   9="Count of number of charges for abdominal x-ray and chest x-ray (per 100 patients), Mean&pm.SD, N"
...
ods rtf file="Tab_edmd.rtf" style=journal bodytitle startpage=no;

proc report data=tab nowindows headline spacing=1 split='*' style=[just=center];
	title "Test Pre/Post Intervention (by Patient)";
	column item nfn nfy or rg pv;
	define item/"." format=item. style=[just=Left cellwidth=4in];
	define nfn/"Pre Intervention" style(column)=[just=center cellwidth=1.75in] style(header)=[just=center];
	define nfy/"Post Intervention" style(column)=[just=center cellwidth=1.75in] style(header)=[just=center];
	define or/"Odds Ratio" format=4.2 style(column)=[just=center cellwidth=0.5in] style(header)=[just=center];
	define rg/"Range" style(column)=[just=center cellwidth=1in] style(header)=[just=center];
	define pv/"p value" style(column)=[just=center cellwidth=0.6in] style(header)=[just=center];
run;
ods rtf close;

...
data tmp_edmd; 
	merge brent.edmd brent.EDMD_Overall(in=A keep=pid rank where=(rank>72)); by pid;
	if A;
	drop rank;
run;
...
ods rtf file="overall_top.rtf" style=journal bodytitle startpage=no;

proc report data=tab nowindows headline spacing=1 split='*' style=[just=center];
	title "Test Pre/Post Intervention (Overall for Top Utilizer)";
	column item nfn nfy or rg pv;
	define item/"." format=item. style=[just=Left cellwidth=4in];
	define nfn/"Pre Intervention" style(column)=[just=center cellwidth=1.25in] style(header)=[just=center];
	define nfy/"Post Intervention" style(column)=[just=center cellwidth=1.25in] style(header)=[just=center];

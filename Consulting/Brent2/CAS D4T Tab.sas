...
data crf_tdf;
	...
	*if "3Aug2010"d<=start_cur_arv3<='17Mar2011'd;
	format date start_cur_arv3 start_22 start_23 start_24 start_25 start_26 mmddyy.;
...
data tdf;
	merge brent.quest(drop=other) brent.crf(drop=other) brent.tdf(keep=study_no tdf d4t in=A);by study_no;
	if A;
	if cd4<0  then cd4=.;
	if cd41<0 then cd41=.;
	if cd42<0 then cd42=.;
	...
	if "14Apr2010"d<=start_date1<='11Oct2010'd;
	...
	keep study_no tdf d4t age gender date12 arvs efv cd4 ncd cd41 cd42 cd43 cd44 cd45  vl vl1 vl2 vl3 vl4 vl5 vl0 start_22-start_26 start_date1 start_date2 date nday1 nday2 nvl
		start_22-start_26 arvs1-arvs6 num_epis num_epis1 num_epis2 naids diag_dis diag_dis1 diag_dis2 TB cryp HSV KS TOXO
		tdf_date1 tdf_date2 tdf_date3 tdf_date4 tdf_date5 d4t_date1 d4t_date2 d4t_date3 d4t_date4 d4t_date5; 
	format start_22-start_26 start_date1 start_date2 date tdf_date1 tdf_date2 tdf_date3 tdf_date4 tdf_date5 d4t_date1 d4t_date2 d4t_date3 d4t_date4 d4t_date5 mmddyy.;
run;
ods listing;


proc print data=tdf;
var study_no arvs2 tdf d4t date start_date1 tdf_date1 tdf_date2 tdf_date3 tdf_date4 tdf_date5 d4t_date1 d4t_date2 d4t_date3 d4t_date4 d4t_date5;
run;
...
data tab;
	...
	if G then do; item=item+10; if code=1; end;
run;
...
ods rtf file="D4T Tab.rtf" style=journal bodytitle ;
...
define nfy/"TDF-containing*(n=&n1)";
define nfn/"No TDF use*(n=&n0)";
define pv/"p value";
run;
ODS ESCAPECHAR='^';

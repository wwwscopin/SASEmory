...
data tdf;
	...
	if tdf then  do;
		...
		if arvs3="STAVUDINE" then 	do; d4t_date1=start_23; end;
		if arvs4="STAVUDINE" then 	do; d4t_date2=start_24; end;
		if arvs5="STAVUDINE" then 	do; d4t_date3=start_25; end;
		if arvs6="STAVUDINE" then 	do; d4t_date4=start_26; end;
		start_date3=min(d4t_date1,d4t_date2,d4t_date3,d4t_date4);
	end;
	...
	keep study_no tdf d4t age gender date12 arvs efv cd4 ncd cd41 cd42 cd43 cd44 cd45  vl vl1 vl2 vl3 vl4 vl5 vl0 start_22-start_26 start_date1 start_date2 date nday1 nday2 nvl
		start_22-start_26 arvs1-arvs6 num_epis num_epis1 num_epis2 naids diag_dis diag_dis1 diag_dis2 TB cryp HSV KS TOXO kidx k65 d4t_date1 d4t_date2 d4t_date3 d4t_date4 nday3; 
	format start_22-start_26 start_date2 date date9.;
run;
ods listing;

proc print data=tdf;
var study_no tdf d4t kidx k65 arvs3-arvs6 d4t_date1 d4t_date2 d4t_date3 d4t_date4 nday3;
run;

proc print data=tdf(where=(nday3^=.));
var study_no tdf d4t kidx k65 arvs3-arvs6 d4t_date1 d4t_date2 d4t_date3 d4t_date4 nday3;
run;

	proc means data=tdf n q1 median q3;
		var nday3;
	run;

proc freq data=tdf(where=(nday3^=.)); 
	table k65;
run;

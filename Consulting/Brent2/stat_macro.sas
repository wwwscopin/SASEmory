...
%macro stat(data, gp, varlist);
	...
	data tab&i;
		...
		mean0=put(median,5.0)||"["||compress(put(Q1,5.0))||" - "||compress(put(Q3,5.0))||"]";

		%if &var=age %then %do;
			mean0=put(mean,5.0)||" &pm "||compress(put(std,5.0))||"["||compress(put(Q1,5.0))||" - "||compress(put(Q3,5.0))||"]";
		%end;
		%if &var^=nday1 or &var^=nday2 %then %do;
			mean0=put(median,5.1)||"["||compress(put(Q1,5.1))||" - "||compress(put(Q3,5.1))||"]";
		%end;
...
%do %while ( &var NE );
	...
	data &out;
		set &out tab&i; 
		keep code0 item ny no nt fy fn ft nfy nfn nft rpct or range pvalue pv;
		format RowPercent 5.1;
	run; 

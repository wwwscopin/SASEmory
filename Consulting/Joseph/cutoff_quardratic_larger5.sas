...
axis1 	label=(f=Century h=3 "Age" ) split="*"	value=(f=Century h=3)  order= (4 to 16 by 1) minor=none offset=(0 in, 0 in);
...
axis2 	label=(f=Century h=3 a=90 "&text") value=(f=Century h=3) order= (0 to 0.30 by 0.02) offset=(.25 in, .25 in); 
%end;

%if &var=rtce or &var=rce %then %do;
axis2 	label=(f=Century h=3 a=90 "&text") value=(f=Century h=3) order= (0.8 to 1.5 by 0.1) offset=(.25 in, .25 in); 
%end;

%if &var=rtmle or &var=rfmle %then %do;
axis2 	label=(f=Century h=3 a=90 "&text") value=(f=Century h=3) order= (0.75 to 1.20 by 0.05) offset=(.25 in, .25 in); 
%end;

%if &var=rfce or &var=rtcev %then %do;
axis2 	label=(f=Century h=3 a=90 "&text") value=(f=Century h=3) order= (0 to 1.5 by 0.1) offset=(.25 in, .25 in); 
%end;

%if &var=rftc %then %do;
axis2 	label=(f=Century h=3 a=90 "&text") value=(f=Century h=3) order= (0.75 to 2 by 0.25) offset=(.25 in, .25 in); 
%end;
...
ods listing close;
ods pdf file = "ratio by age quardratic large5.pdf" style=journal;
proc greplay nofs;
igout xxx.graphs;
list igout;
tc template;

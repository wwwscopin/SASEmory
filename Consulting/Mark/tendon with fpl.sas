...
ods rtf file="rehab_fpl.rtf" style=journal bodytitle startpage=no;

proc print data=tend split="*" noobs label uniform style(head)=[just=center]; 
	title "Complication Rates(Rupture/Decreased ROM)for Rehab Methods(With FPL)";
	by group notsorted;
	id group;
...
proc print data=orp split="*" noobs label uniform style(head)=[just=center]; 
	title "Comparsion of Ruptures and Deceased ROM rates between Rehab Methods(With FPL)";
	by item notsorted;
	id item;

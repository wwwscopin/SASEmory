...
value wound 0='N' 1='Y';
value bleeding 0='N' 1='Y';
value deep_infect 0='N' 1='Y';
value super_infect 0='N' 1='Y';
value readmission 0='N' 1='Y';
run;
...
Data Control;
	...
	if _n_<=14 then bleeding=1;else bleeding=0;
	if _n_<=4 then deep_infect=1;else deep_infect=0;
	if _n_<=23 then super_infect=1;else super_infect=0;
	if _n_<=7 then readmission=1;else readmission=0;
...
Data study;
	...
	if pe0='No' then pe=0; else pe=1;
	if death0='no' then death=0; else death=1;
	if _n_<=9 then bleeding=1;else bleeding=0;
	if _n_<=1 then deep_infect=1;else deep_infect=0;
	if _n_<=14 then super_infect=1;else super_infect=0;
	if _n_<=5 then readmission=1;else readmission=0;
...
data HK;
	...
	format HK HK. gender gender. Hormone Hormone. smoker smoker. dvt dvt. pe pe. death death. group group. wound wound.
			bleeding bleeding. deep_infect deep_infect. super_infect super_infect. readmission readmission.;
run;
...
proc freq data=HK;
	table group*(HK gender hormone smoker dvt pe death wound bleeding deep_infect super_infect readmission)/nopercent nocol chisq fisher;
run;
proc freq data=HK;

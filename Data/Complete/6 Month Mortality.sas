...
run;
ods graphics on;
proc logistic data=glnd descending plots;
	model deceased=citrulline;
run;
ods graphics off;
/*
proc freq data=glnd;
...
data _null_;
	set glnd(where=(deceased=1 and citru^=.));
...
title1 h=3 justify=center "6-Month Mortality by Baseline Plasma Citrulline Levels (Number of Death=&num_death)";
title2 h=2.5 justify=center "Log-Rank Test: p=&pv";
         
axis1 	label=(h=2.5 'Days after Randomization' ) split="*" value=(h=2) order= (0 to 91 by 7) minor=none offset=(0.2in, 0);
axis2 	label=(h=2.5 a=90 "6-Month Mortality") order=(0 to 1 by 0.1) value=(h=2) ;
...
filename output 'citrulline_mortality_6mon.eps';
goptions reset=all BORDER device=pslepsfc gsfname=output gsfmode=replace ;

ods pdf file = "citrulline_km_6mon.pdf";

...
data glnd;
	merge glnd sofa	wbh.info(keep=id treatment deceased apache_2 hospital_death); by id;
	if sofa_tot>6 then sofa=2; else sofa=1;
	death_6month=deceased+1;
	hop_death=hospital_death+1;
	format apache_2 apache. sofa sofa. death_6month hop_death death.;
run;

proc freq data=glnd(where=(day=7));
	where citrulline^=.;
	tables treatment;
run;

proc means data=glnd(where=(day=7));
	var glutamine;
run;


proc mixed data=glnd (where=(day=7)) covtest;
	class id treatment;
	model citrulline=glutamine treatment treatment*glutamine;
...
%macro mixed(data, gvar, var, ylab, title)/minoperator;
...
*ods trace on/label listing;
	...
	class &gvar id day ; 	
	model &var=&gvar day &gvar*day/ solution ; 
...
%if &gvar # death_6month hop_death %then %do;
...
%if &var=citrulline %then %do; %let scaley=(8 to 40 by 2); %let scalex=(-1 to 29 by 1); %end;
...
%let ylab="(Citrulline &mu.M)";
%let title=Citrulline by In-Hospital Mortality;
%mixed(glnd,hop_death,citrulline,&ylab, &title); run;
...
goptions reset=all NOBORDER rotate=portrait device=pslepsfc /*gsfname=output*/ gsfmode=replace ;
	...
	ods  rtf  file="Citrulline.rtf" style=journal bodytitle startpage=never;
	
	proc report data=est_death_6month nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Citrulline by 6-Month Mortality and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est0 est1;
    	define day/"Day";
    	define est0/"Survivor*Mean[95%CI], N" style(column)=[width=2in];
    	define est1/"Non-Survivor*Mean[95%CI], N" style(column)=[width=2in];
       	define diff/"Difference*Mean[95%CI]" style(column)=[width=2in];
    	define pv/"p value";
	run;

	proc report data=est_hop_death nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Citrulline by In-Hospital Mortality and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est0 est1;
    	define day/"Day";
    	define est0/"Survivor*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=est_treatment nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Citrulline by Treatment and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est0 est1;
    	define day/"Day";
    	define est0/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
    	define est1/"STD-PN*Mean[95%CI], N" style(column)=[width=2in];
       	define diff/"Difference*Mean[95%CI]" style(column)=[width=2in];
    	define pv/"p value";
	run;

	proc report data=est_apache_2 nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Citrulline by Apache>15 and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est0 est1;
    	define day/"Day";
    	define est0/"Apache<=15*Mean[95%CI], N" style(column)=[width=2in];
    	define est1/"Apache>15*Mean[95%CI], N" style(column)=[width=2in];
       	define diff/"Difference*Mean[95%CI]" style(column)=[width=2in];
    	define pv/"p value";
	run;

		proc report data=est_SOFA nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Citrulline by SOFA>6(Median) and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est0 est1;
    	define day/"Day";
    	define est0/"SOFA<=6*Mean[95%CI], N" style(column)=[width=2in];
    	define est1/"SOFA>6*Mean[95%CI], N" style(column)=[width=2in];
       	define diff/"Difference*Mean[95%CI]" style(column)=[width=2in];
    	define pv/"p value";
	run;

...
value = (h=2.5 "Any Infection=Yes" "Any Infection=No") offset=(-0.2in, -0.2 in) frame;
...
title1 	height=5 "Model-Based Means and 95%CI by Any Infection and Days on Study";
title2  h=5 "for &title";
title3 	h=4 "p(Any Infection)=&p1, p(Days)=&p2, p(Any Infection*Days)=&p3";
...
proc gplot data= estimate gout=wbh.graphs;
	...
	note h=2.5 m=(10pct, 9 pct) "(#Any Infection=Yes)" ;
	note h=2.5 m=(10pct, 6 pct) "(#Any Infection=No)" ;
...
%let title=Vitamin D by Any Infection;
%mixed(glnd_vd,ohd,Any,&ylab, &title); run; 

%let ylab="LL-37 Total (ng/mL)";
%let title=LL-37 Total by Any Infection;
%mixed(glnd_vd,ll,Any,&ylab, &title); run; 

%let ylab="VDBP (mg/dL)";
%let title=VDBP by Any Infection;
%mixed(glnd_vd,vdbp,Any,&ylab, &title); run; 


	ods pdf file = "glnd_vd_Any Infection.pdf";
	...
	ods  rtf  file="GLND VD Any Infection.rtf" style=journal bodytitle startpage=never;
	
	proc report data=test_ohd nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for 25(OH)D (ng/mL) by Any Infection and Days on Study";
    	column day est1 est2 diff ;
    	define day/"Day" order;
    	define est1/"Any Infection=Yes*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Any Infection=No*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_ll nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for LL-37 (ng/mL) by Any Infection and Days on Study";
     	column day est1 est2 diff ;
    	define day/"Day" order;
    	define est1/"Any Infection=Yes*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Any Infection=No*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_vdbp nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for VDBP (mg/dL) by Any Infection and Days on Study";
    	column day est1 est2 diff;
    	define day/"Day" order;
    	define est1/"Any Infection=Yes*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Any Infection=No*Mean[95%CI], N" style(column)=[width=2in];

...
data infect;
    set wbh.info (keep = id ni_any ni_bsi ni_lri);
    any=ni_any>0;
        bsi=ni_bsi>0;
            lri=ni_lri>0;
    keep id any bsi lri;
...
proc sgpanel data=glnd_vd;
	title "Scatter Plot";
	panelby day/columns=3;
	scatter x=ll  y=vdbp/group=treatment;
run;

proc sgpanel  data=glnd_vd;
	panelby day/columns=3;
	scatter x=vdbp y=ohd/group=treatment;
run;

proc sgpanel  data=glnd_vd;
	panelby day/columns=3;
	scatter x=vdbp  y=ll/group=treatment;
run;

proc sgplot data=glnd_vd noautolegend;                                                                                                      
   series x=day y=ohd / group=id;                                                                                              
   xaxis label="Day" values=(0 14 28);                                                                                       
   yaxis label="25(OH)D" values=(0 to 50 by 5);                                                                                        
run;

proc sgplot data=glnd_vd noautolegend;                                                                                                      
   series x=day y=ll / group=id;                                                                                              
   xaxis label="Day" values=(0 14 28);                                                                                       
   yaxis label="LL37" values=(0 to 220 by 20);                                                                                        
run;

proc sgplot data=glnd_vd noautolegend;                                                                                                      
   series x=day y=vdbp / group=id;                                                                                              
   xaxis label="Day" values=(0 14 28);                                                                                       
   yaxis label="VDBP" values=(0 to 40 by 5);                                                                                        
run;
*/ 
...
%if &var=ll %then %do; %let scaley=(20 to 100 by 5); %let scalex=( 0 to 42 by 14); %end;
...
title1 	height=5 "Model-Based Means and 95%CI by Treatment and Days on Study";
title2  h=5 "for &title";
title3 	h=4 "p(Treatment)=&p1, p(Days)=&p2, p(Treatment*Days)=&p3";
...
proc gplot data= estimate gout=wbh.graphs;
	...
	note h=2.5 m=(10pct, 9 pct) "(# AG-PN)" ;
	note h=2.5 m=(10pct, 6 pct) "(#STD-PN)" ;
...
%let title=Vitamin D by Treatment;
%mixed(glnd_vd,ohd,treatment,&ylab, &title); run; 

%let ylab="LL-37 Total (ng/mL)";
%let title=LL-37 Total by Treatment;
%mixed(glnd_vd,ll,treatment,&ylab, &title); run; 

%let ylab="VDBP (mg/dL)";
%let title=VDBP by Treatment;
%mixed(glnd_vd,vdbp,treatment,&ylab, &title); run; 


	ods pdf file = "glnd_vd.pdf";
...
ods tagsets.excelxp file="glnd_vd.xls";
ods tagsets.excelxp
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND VD.rtf" style=journal bodytitle startpage=never;
	
	proc report data=test_ohd nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for 25(OH)D (ng/mL) by Treatment and Days on Study";
	...
	proc report data=test_ll nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for LL-37 (ng/mL) by Treatment and Days on Study";
     	column day est1 est2 diff ;
    	define day/"Day" order;
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"STD-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_vdbp nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for VDBP (mg/dL) by Treatment and Days on Study";
    	column day est1 est2 diff;
    	define day/"Day" order;
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"STD-PN*Mean[95%CI], N" style(column)=[width=2in];

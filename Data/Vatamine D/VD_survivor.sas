...
data glnd_VD;
	merge vd(in=A) wbh.info(keep=id age gender hospital_death treatment in=comp);	by id; 
	format treatment treatment.;
	if A and comp;
	label ohd="25(OH)D" ll="LL37" vdbp="VDBP";
run;

/*
proc sgplot  data=glnd_vd;
	title "Scatter Plot";
	scatter x=ohd y=ll/group=treatment;
	*scatter x=ll  y=ohd/group=treatment;
run;
proc sgplot  data=glnd_vd;
	*scatter x=ohd y=ll/group=treatment;
	scatter x=ll  y=ohd/group=treatment;
...
proc sgscatter  data=glnd_vd;
	plot ohd*ll/group=treatment;
	plot ll*ohd/group=treatment;
run;
...
proc print data=glnd_vd label noobs;
where id in(11485,42115);
var id treatment day ohd ll vdbp;
run;
...
data est_&var;
	...
	label n1="n*(Survivor)" n2="n*(Non-Survivor)" estimate1="Mean*(Survivor)" estimate2="Mean*(Non-Survivor)" 
	   error1="Error*(Survivor)" error2="Error*(Non-Survivor)";
run;
...
%if &var=ohd %then %do; %let scaley=(5 to 25 by 1); %let scalex=( 0 to 42 by 14); %end;
%if &var=ll %then %do; %let scaley=(10 to 80 by 5); %let scalex=( 0 to 42 by 14); %end;
%if &var=vdbp %then %do; %let scaley=(10 to 25 by 1); %let scalex=(0 to 42 by 14); %end;
...
title1 	height=5 "Model-Based Means and 95%CI by Vital Status and Days on Study";
title2  h=5 "for &title";
title3 	h=4 "p(Vital Status)=&p1, p(Days)=&p2, p(Vital Status*Days)=&p3";
...
proc gplot data= estimate gout=wbh.graphs;
	...
	note h=2.5 m=(10pct, 9 pct) "(# Survivor)" ;
	note h=2.5 m=(10pct, 6 pct) "(#Non-Survivor)" ;
...
%let title=Vitamin D by Vital Status;
%mixed(glnd_vd,ohd,hospital_death,&ylab, &title); run; 

%let ylab="LL-37 Total (ng/mL)";
%let title=LL-37 Total by Vital Status;
%mixed(glnd_vd,ll,hospital_death,&ylab, &title); run; 

%let ylab="VDBP (mg/dL)";
%let title=VDBP by Vital Status;
%mixed(glnd_vd,vdbp,hospital_death,&ylab, &title); run; 


	ods pdf file = "glnd_vd_survivor.pdf";
...
ods tagsets.excelxp file="glnd_vd_survivor.xls";
ods tagsets.excelxp
options(sheet_name="25(OH)D");
proc print data=est_ohd noobs label split="*";Run;

ods tagsets.excelxp
options(sheet_name="LL-37");
proc print data=est_ll noobs label split="*";Run;
ods tagsets.excelxp
options(sheet_name="VDBP");
proc print data=est_vdbp noobs label split="*";run;
ods tagsets.excelxp close;

	ods  rtf  file="GLND VD_survivor.rtf" style=journal bodytitle startpage=never;
	
	proc report data=test_ohd nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for 25(OH)D (ng/mL) by Vital Status and Days on Study";
	...
	proc report data=test_ll nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for LL-37 (ng/mL) by Vital Status and Days on Study";
     	column day est1 est2 diff ;
    	define day/"Day" order;
    	define est1/"Survivor*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Non-Survivor*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_vdbp nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for VDBP (mg/dL) by Vital Status and Days on Study";
    	column day est1 est2 diff;
    	define day/"Day" order;
    	define est1/"Survivor*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Non-Survivor*Mean[95%CI], N" style(column)=[width=2in];

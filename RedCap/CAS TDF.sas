...
proc freq; 
tables gp*tdf;
format gp idx. tdf tdf.;
run;

data tdf;
	merge brent.quest(where=(gp=1)) crf_tdf(keep=study_no tdf); by study_no;
run;

%let varlist=age;
%stat(tdf, tdf, &varlist);

%let varlist=gender;
%tab(tdf, tdf, demo, &varlist);
...
ods rtf file="prearvs.rtf" style=journal bodytitle;
...
var study_no arvs2 start_cur_arv3 arvs3 arvs4 arvs5 arvs6 tdf;

...
proc contents data=brent.kidx;run;
...
data tdf;
	merge brent.quest(drop=other) brent.crf(drop=other) brent.tdf(keep=study_no tdf d4t in=A) brent.kidx(drop=tdf d4t);by study_no;
	if  A;
	if cd4<0  then cd4=.;
	if cd41<0 then cd41=.;
	if cd42<0 then cd42=.;

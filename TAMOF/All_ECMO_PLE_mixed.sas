...
proc mixed data =tmp empirical covtest;
	...
	estimate "line trend0" day -5 -3 3 5 -1 1 censor*day -5 -3 3 5 -1 1 0 0 0 0 0 0/e;
	estimate "line trend1" day -5 -3 3 5 -1 1 censor*day 0 0 0 0 0 0 -5 -3 3 5 -1 1;
...
%if &var=plt %then %do;
	axis2 	label=(f=Century h=3 a=90 "Platelet Counts(*1000 cells/uL)") value=(f=Century h=3) order= (0 to 140 by 20) offset=(.25 in, .25 in) minor=(number=1); 
...
%if &var=vwf_ag %then %do;
	axis1 	label=(f=Century h=3 "Days on Study" ) split="*"	value=(f=Century h=3)  order= (-1 0 1 4 8 15 28 40) minor=none offset=(0 in, 0 in);
	axis2 	label=(f=Century h=3 a=90 "vWF Antigen (%)") value=(f=Century h=3) order= (0 to 200 by 20) offset=(.25 in, .25 in) minor=(number=1); 
...
%if &var=vwf_rca %then %do;
	...
	axis1 	label=(f=Century h=3 "Days on Study" ) split="*"	value=(f=Century h=3)  order= (-1 0 1 4 8 15 28 40) minor=none offset=(0 in, 0 in);
	axis2 	label=(f=Century h=3 a=90 "vWF Ristocetin Cofactor Activity (%)") value=(f=Century h=3) order= (0 to 200 by 20) offset=(.25 in, .25 in) minor=(number=1); 
...
proc gplot data= estimate_&var(where=(day^=99)) gout=tamof.graphs;
	...
	note h=3 m=(6pct, 14.75 pct) "Day :" ;
	note h=3 m=(1pct, 11.25 pct) "(#Survivors)" ;
	note h=3 m=(-1pct, 7.75 pct) "(#Non-Survivors)" ;

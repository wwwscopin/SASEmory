...
proc mixed data =tmp empirical covtest;
	class pe id day ; 	
	%if &var^=vwf_ag or &var^=vwf_rca %then %do;
	model &var= pe day pe*day/ solution ; * &source	day*&source/ solution;
	%end;
	%if &var=vwf_ag %then %do;
	model log_ag= pe day pe*day/ solution ; * &source	day*&source/ solution;
	%end;
	%if &var=vwf_rca %then %do;
	model log_rca= pe day pe*day/ solution ; * &source	day*&source/ solution;
	%end;
	repeated day / subject = id type = cs;
	lsmeans pe*day pe/ cl ;
...
data estimate_&var;
	...
	lsmeans_&var(where=(pe=1) rename=(estimate=estimate1 lower=lower1 upper=upper1)) ; by day;
run;
...
value = (f=Century h=2.5 "Plasma Exchange" "no Plasma Exchange") offset=(0.2in, -0.2 in) frame;
...
%if &var=adamts13 %then %do;
	...
	axis2 	label=(f=Century h=3 a=90 "ADAMTS-13 Activity (%)") value=(f=Century h=3) order= (0 to 130 by 10) offset=(.25 in, .25 in) minor=(number=1); 
	title1 	height=3.5 f=Century "ADAMTS-13 Activity vs Days on Study";
	title2 height=3 f=Century "(Plasma Exchange=&nya vs. no Plasma Exchange=&noa)";
%end;
...
%if &var=vwf_ag %then %do;
	...
	value = (f=Century h=2.5 "Plasma Exchange" "no Plasma Exchange") offset=(0.2in, 0.2 in) frame;
	axis1 	label=(f=Century h=3 "Days on Study" ) split="*"	value=(f=Century h=3)  order= (-1 0 1 4 8 15 28 40) minor=none offset=(0 in, 0 in);
	axis2 	label=(f=Century h=3 a=90 "vWF Antigen (%)") value=(f=Century h=3) order= (0 to 300 by 20) offset=(.25 in, .25 in) minor=(number=1); 
	title1 	height=3.5 f=Century "vWF Antigen vs Days on Study";
	title2 	height=3.0 f=Century "(Plasma Exchange=&nyv vs. no Plasma Exchange=&nov)";
%end;
...
%if &var=vwf_rca %then %do;
	...
	axis1 	label=(f=Century h=3 "Days on Study" ) split="*"	value=(f=Century h=3)  order= (-1 0 1 4 8 15 28 40) minor=none offset=(0 in, 0 in);
	axis2 	label=(f=Century h=3 a=90 "vWF Ristocetin Cofactor Activity (%)") value=(f=Century h=3) order= (0 to 300 by 20) offset=(.25 in, .25 in) minor=(number=1); 
	title1 	height=3.5 f=Century "vWF Ristocetin Cofactor Activity vs Days on Study";
	title2 	height=3.0 f=Century "(Plasma Exchange=&nyr vs. no Plasma Exchange=&nor)";
%end;
...
proc gplot data= estimate_&var(where=(day^=99)) gout=tamof.graphs;
	...
	note h=3 m=(0pct, 14.5 pct) "Day :" ;
	note h=3 m=(-6pct, 11.0 pct) "(#Plasma Exchange)" ;
	note h=3 m=(-9pct, 7.5 pct) "(#no Plasma Exchange)" ;
...
proc gslide gout=tamof.graphs;
  title height=1.5 f=Century "TAMOF children (Plasma Exchange=&ny vs. no Plasma Exchange=&no)";
run;

options orientation=portrait papersize=(8.5in 11in);
ods pdf file = "All_PLE_noPlE_mixed_log.pdf" style=journal;
...
ods rtf file="all_ple_nople_mixed.rtf" style=journal bodytitle startpage=no; 
proc print data=EP0 noobs label;
title "PELOD Score vs Days on Study";
var day col0 col1/style=[just=center cellwidth=1.5 in];
label day="Treatment Day"
	  col0="No Plasma Exchange"
	  col1="Plasma Exchange";
run;

proc print data=p_pelod0 noobs label;
title "P value for Repeated Measurements Analysis";
...
proc print data=EP noobs label;
title "PELOD Score vs Days on Study: Max PELOD used at all time points after death.";
var day col0 col1/style=[just=center cellwidth=1.5 in];
label day="Treatment Day"
	  col0="No Plasma Exchange"
	  col1="Plasma Exchange";

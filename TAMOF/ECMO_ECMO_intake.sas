...
data mixed;
	...
	if plasmaexchange=1 and ecmo=1;
run;
...
ods rtf file="ple_ECMO_intake.rtf" style=journal bodytitle;
proc print noobs ;
...
title2 height=2.5 f=zapf "for &nt Children with TAMOF treated with Plasma Exchange and ECMO (Survivors=&ns, Non-Survivors=&nd)";
%end;
...
axis2  label=(f=zapf h=2.5 a=90 "Daily Urine Output (mL)") value=(f=zapf h=2) order= (0 to 2600 by 200) offset=(.25 in, .25 in) minor=(number=1); 
...
proc print data=estimate_urineoutput;run;

options orientation=portrait;
goptions reset=all;
ods pdf file = "ple_ecmo_intake.pdf" style=journal;

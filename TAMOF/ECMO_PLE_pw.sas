...
data mixed;
	...
	tamof.survival(keep=patientid censor plasmaexchange ecmo where=(plasmaexchange=1 and ecmo=1) in=tmp); by patientid;
	if tmp;
...
%if &var=pw %then %do;
	...
	title2 height=3 "for &nt Children with TAMOF treated with Plasma Exchange and ECMO (Survivors=&ns, Non-Survivors=&nd)";
%end;
...
proc print data=estimate_pw;run;
...
data ttA;
	merge typeA(in=a rename=(col1=nA1 col2=nA2 col3=nA3 col4=nA4 col5=nA)) techA(in=b rename=(col1=mA1 col2=mA)); by day;
	fa1=na1/nA*100; fa2=na2/nA*100; fa3=na3/nA*100; fa4=na4/nA*100; ga1=ma1/mA*100;  
	ta1=na1||"/"||compress(nA)||"("||compress(put(fa1,4.0))||"%)";
	ta2=na2||"/"||compress(nA)||"("||compress(put(fa2,4.0))||"%)";
	ta3=na3||"/"||compress(nA)||"("||compress(put(fa3,4.0))||"%)";
	ta4=na4||"/"||compress(nA)||"("||compress(put(fa4,4.0))||"%)";
	sa1=ma1||"/"||compress(mA)||"("||compress(put(ga1,4.0))||"%)";
	if na=0 and na1=0 then ta1="--";
	if na=0 and na2=0 then ta2="--";
	if na=0 and na3=0 then ta3="--";
	if na=0 and na4=0 then ta4="--";
	if ma=0 and ma1=0 then sa1="--";
	*if ma=0 and ma2=0 then sa2="--";
	keep day nA1-nA4 nA mA1-mA4 mA ta1 ta2 ta3 ta4 sa1;
run;
...
ods pdf file = "ple_ECMO_pw.pdf" style=journal ;
/*
...
define tb1/"Single plasma volume" style(column)=[cellwidth=1in just=center];

...
data sub_censor;
	set mixed;
	if censor=0 and day=3 and pelod0=.;
	keep id;
run;
...
%do %while ( &var NE );
	...
	data tmp;
		merge &data sub_censor(in=A); by id;
		if A then delete;
		if 1<=day<=7 and pe=1;
	run;
...
ods listing close;
ods pdf file = "pelod_ple_cvvh_ecmo_peonly_sub.pdf" style=journal ;
...
ods ps file="pelod5.ps" color=no;
...
proc report data=est_censor_pelod nowindows split="*";

...
proc mixed data =tmp empirical covtest; 
	...
	ods output lsmeans = lsmeans_&i;
		ods output  Mixed.Diffs=diff_&i;
...
data diff_&var;
	...
	keep day Estimate StdErr Lower upper Probt pv;
run;
...
data estimate_&var;
	...
	diff_&var(keep=day probt pv); by day;
run;
...
%if &var=pelod0 %then %do;
	...
	title1 	height=3.5 f=Century "PELOD Score vs Days on Study by Treatment from Multivariate Analysis";
%end;

%if &var=dbp %then %do;
	axis2 	label=(f=Century h=3 a=90 "PELOD Score Change from Baseline") value=(f=Century h=3) order= (-14 to 10 by 2) offset=(.25 in, .25 in) minor=(number=1); 
	title1 	height=3.5 f=Century "PELOD Score Change from Baseline vs Days on Study by Treatment from Multivariate Analysis";
%end;
...
data est_pe_pelod;
	set estimate_pe_pelod0(in=A) estimate_pe_chg(in=B)  estimate_pe_dbp(in=C);
	if A then item=1;
	if B then item=2;
	if C then item=3;

	if (B or C) and day=1 then do; est0="-"; est1="-"; end;
	format item item. ;
...
ods listing close;
ods pdf file = "pelod_ple_cvvh_ecmo0.pdf" style=journal ;

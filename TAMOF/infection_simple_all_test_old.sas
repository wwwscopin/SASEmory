...
data infect;
	merge tmp(in=A) tamof.demographic(keep=patientid  EnrollmentDate rename=(patientid=id))
		 TAMOF.survival(keep=patientid plasmaexchange censor ecmo rename=(patientid=id) in=B); by id;
	...
	if BloodStreamInf then gnbsi=0; else if gi=1 then gnbsi=1;
...
proc sort data=&data nodupkey out=&data._&var; by id &var; run;
proc freq;
...
data tab&i;
	...
	col0=&n0||"/&m0("||put(f0,4.2)||"%)";
	col1=&n1||"/&m1("||put(f1,4.2)||"%)";
	item=&i	;
...
data subnoecmo subecmo;
	set infect;
	if plasmaexchange=1 and ecmo=0 then output subnoecmo;
	if plasmaexchange=1 and ecmo=1 then output subecmo;
run;
...
title "Infection Rates by Site for 24 TAMOF Children treated with Plasmae Exchange and ECMO";
...
title "Infection Rates by Site for 34 TAMOF Children treated with Plasmae Exchange but did not reveive ECMO";
var item/style(data)=[just=left cellwidth=2.5in] style(header)=[just=left];
var col1 col0 /style(data)=[just=center cellwidth=2in] style(header)=[just=center];
var pv/style(data)=[just=center cellwidth=1.0in] style(header)=[just=center];
label   item="."
...
title "Infection Rates for 81 TAMOF Children by Site";
var item/style(data)=[just=left cellwidth=2.5in] style(header)=[just=left];
var col1 col0 /style(data)=[just=center cellwidth=2in] style(header)=[just=center];
var pv/style(data)=[just=center cellwidth=1.0in] style(header)=[just=center];
label   item="."

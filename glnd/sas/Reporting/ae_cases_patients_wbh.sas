...
data ae_temp;
	...
	* mark AEs that do occur, for the patient group with AEs. will be "." for the AEs that they do not have.;
    ...
    cent=floor(id/10000);
	keep id cent ae1-ae17 hyper hypo hypo1 ae_type related_treat;
run;

proc freq data=ae_temp(where=(hyper=1));
by cent;
table id;
run;


proc freq data=ae_temp(where=(hypo=1));
table id;
run;

proc freq data=ae_temp(where=(hypo1=1));
table id;
run;
...
proc freq data= ae_temp;
	tables (ae_type hypo hypo1 hyper) * related_treat;
run;
...
proc means noprint;
	...
	output out=aes_by_patient max=ae1-ae17 sum = sum_ae1-sum_ae17 max(hyper hypo hypo1) = hyper hypo hypo1 sum(hyper hypo hypo1) = sum_hyper sum_hypo sum_hypo1;
proc print;
...
data glnd.ae_patients;
      ...
      keep id ae1-ae17 treatment hyper hypo hypo1;
      format ae1-ae17 yn.;

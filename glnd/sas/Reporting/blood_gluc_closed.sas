  	...
  	goptions reset=all rotate=landscape gunit=pct noborder cback=white 
		...
		* symbols for up to 10 patients - no longer labeling individuals ;
		symbol1 value = dot h=1.5 i=join repeat = 200;
 		...
 		axis1 	label=(f=triplex h=3 'Day' ) 	value=( h=1.3) order = (0 to 29) minor= none split="*";
 		axis2 	label=(f=triplex h=3 a=90 &description ) 	value=(h=3) 
 		...
 		title1 f=triplex h=4 justify=center &description ", treatment " &group;
		...
		axis1 	label=(f=triplex h=4 'Day' ) 	value=(h=1.3) order = (0 to 29) minor= none split="*"; * give one extra day in each direction to allow space for boxplots ; 
  ...
  data glnd.gluc_mixed_closed_&variable.&group;
   ...
   run;
		...
		* plot original data jitterd, estimated means and 95% CIs;
		...
		axis1 	label=(f=triplex h=4 'Day' ) 	value=(h=1.3) order = (0 to 29) minor= none split="*"; * give one extra day in each direction to allow space for boxplots ; 
		axis2 	label=(f=triplex h=4 a=90 &description ) 	value=( h=3) 
					order= (0 to 400 by 50) 	major=(h=1.5 w=2) minor=(number=4 h=1);
 	
		symbol1 ci=red value=dot h=1;
		...
		legend1 	across = 1 position=(top right inside) mode = reserve fwidth = .2
				...
				value = (h=3 "lab" "accucheck" "mean");

 		title1 f=triplex h=4 justify=center &description ", treatment " &group;
		title2 f=triplex h=3 justify=center "Longitudinal models (means and 95% CI)";

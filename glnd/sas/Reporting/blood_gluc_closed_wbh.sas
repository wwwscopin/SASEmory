...
run;
 	...
 	* capture sample size of total people in each trt group; 
		...
		proc mixed data = follow_up_treat empirical covtest;
			class id day treatment;
			model gluc_mrn = day treatment treatment*day/ solution;
			repeated day / subject = id type = csh;
			*lsmeans day / cl ;
			ods output tests3=temp1;
		run;
		
		data temp1;	set temp1;  n=_n_;  rename probf=pv1;	run;
		
		proc mixed data = follow_up_treat empirical covtest;
			class id day treatment;
			model gluc_aft = day treatment treatment*day/ solution;
			repeated day / subject = id type = csh;
			*lsmeans day / cl ;
			ods output tests3=temp2;
		run;
		
		data temp2;	set temp2;  n=_n_;  rename probf=pv2;	run;
		
		proc mixed data = follow_up_treat empirical covtest;
			class id day treatment;
			model gluc_eve = day treatment treatment*day/ solution;
			repeated day / subject = id type = csh;
			*lsmeans day / cl ;
    		ods output tests3=temp3;
		run;
		data temp3;	set temp3;  n=_n_;  rename probf=pv3;	run;
		
		data temp;
		  merge temp1(keep=effect n pv1)  temp2(keep=effect n pv2) 
		        temp3(keep=effect n pv3) ; by n;
		run;
  ...
  data glnd.gluc_mixed_closed_&variable.&group;
   ...
   run;
		...
		* plot original data jitterd, estimated means and 95% CIs;
		...
		symbol1 value=dot ci=red h=1;
		symbol2 ci=blue value=circle h=1;
		symbol3 value = diamond ci=black co=black h = 2 i = join ;
...
%make_nutr_plots(i=2) run; * treatment B;
	...
	goptions  reset=all rotate = portrait;
		...
		* MIXED Tables;
		...
		ods ps file = "/glnd/sas/reporting/gmtc.ps" style = journal startpage = no;
    		*title "Blood glucose longitudinal model means (mg/dL)";
    		title " ";   

			ods escapechar='^' ;

            ods ps text = "^S={font_weight=bold font_size=12pt just=center} Blood glucose longitudinal model means (mg/dL)";
            			ods ps text = " ";
			ods ps text = "^S={font_weight=bold font_size=12pt just=center} Treatment group A (n = &n_A)";		
			...
			ods ps text = " ";
   			ods ps text = " ";
			ods ps text = "^S={font_weight=bold font_size=12pt just=center} Treatment group B (n = &n_B)";		
			...
			ods ps text = " ";
   			ods ps text = " ";
			ods ps text = "^S={font_weight=bold font_size=12pt just=center} P values of Effect";		
			proc print data =temp noobs label split= '*';
				var effect pv3 pv1 pv2 ;
				label pv1="p value*Morning Glucose";
                label pv2="p value*Afternoon Glucose";
				label pv3="p value*Evening Glucose";
			run;
			*/
...
ods listing;
filename output 'blood8closed.eps';

...
/** MAKE OVERALL GLUCOSE PLOTS **/
   %macro make_nutr_plots/minoperator; 
  	%let x= 1;
 	...
 	proc greplay igout= glnd_rep.graphs nofs; delete _ALL_; run; 	*clear out the graphs catalog;
  	...
  	%do %while (&x < 5);
  		...
  		%else %if &x = 4 %then %do; %let variable = gluc_all ; %let source = all_gluc_src;		%let description = "Blood glucose (mg/dL)"; %let time =  "[14:00 - 18:00]"; %end; 
  	  
  	  %if &x # 1 2 3 %then %do; %let data=glnd.followup_all_long; %end;
   	  %if &x=4 %then %do; %let data=all_glucose; %end;
		...
		* merge the means and CIs into gluc_box to obtain plotting dataset;
		...
		data glu_mixed ;
			...
			* split accucheck and raw lab values into 2 different columns;
				...
				if all_gluc_src = 1 then lab_gluc_all = gluc_all;
				else if all_gluc_src = 2 then accucheck_gluc_all = gluc_all;
		...
		* now make a table of the means and 95% CI for morning, evening, and afternoon blood glucose on days 0, 7, 14;
		data glnd_rep.glucose_mixed_open;
			merge 	lsmeans_gluc_eve (keep = estimate upper lower day rename = (estimate = mean_eve upper = upper_eve lower = lower_eve))
					...
					lsmeans_gluc_all (keep = estimate upper lower day rename = (estimate = mean_all upper = upper_all lower = lower_all))
				;
			by day;
			...
			all = compress(put(mean_all, 5.1)) || " (" || compress(put(lower_all, 5.1)) || ", " || compress(put(upper_all, 5.1)) || ")";
			...
			label
				...
				all = "All Glucose*mean (95% CI)"
				
			;
   format day;

			keep day eve mrn aft all;
		...
		* make a table of the means and 95% CI for morning, evening, and afternoon blood glucose on days 0, 7, 14;
		...
		proc sort data = lsmeans_gluc_all_all_gluc_src; by day source; run;
		...
		data glnd_rep.glucose_source_mixed_open;
			merge 	lsmeans_gluc_eve_eve_gluc_src (keep = estimate upper lower day source rename = (estimate = mean_eve upper = upper_eve lower = lower_eve ))
					...
					lsmeans_gluc_all_all_gluc_src (keep = estimate upper lower day source rename = (estimate = mean_all upper = upper_all lower = lower_all ))
				;
			by day source;
			...
			;
    			format day;
		*	keep source day eve mrn aft all;
			
		run;

		ods rtf file = "/glnd/sas/reporting/blood_gluc_mixed_open.rtf" style = journal bodytitle startpage=never;
			title "Blood Glucose Longitudinal Model Means (mg/dL)"; 

			proc print data = glnd_rep.glucose_mixed_open noobs label split= '*' style(data header)=[just=center];
				var day eve mrn aft all;
			run;

			title "Blood Glucose Longitudinal Model Means (mg/dL) -by Lab and Accucheck"; 
			proc print data = glnd_rep.glucose_source_mixed_open noobs label split= '*' style(data header)=[just=center];
				var day source eve mrn aft all;
			run;
		ods rtf close;

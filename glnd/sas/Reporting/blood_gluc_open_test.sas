 ...
 * Eli Rosenberg
 *
 * Create longitundal plots of each patient's nutritional intake
 *
 */
...
/** MAKE OVERALL GLUCOSE PLOTS **/
   %macro make_nutr_plots; 
 	...
 	proc greplay igout= glnd_rep.graphs nofs; delete _ALL_; run; 	*clear out the graphs catalog;
  	...
  	%do %while (&x < 4);
		...
		* get 'n' at each day - SINCE LAST DSMB. TRACKINGS AND BOXPLOTS ARE SINCE LAST DSMB. MIXED IS CUMULATIVE! ;
		proc means data=glnd.followup_all_long noprint;
			...
			where ( month(this_date) >= month(mdy(&last_dsmb_date)) ) & ( year(this_date) >= year(mdy(&last_dsmb_date)) );
  	...
  	goptions reset=all rotate=landscape device=jpeg gunit=pct noborder cback=white gsfmode=replace
		...
		* symbols for up to 10 patients - no longer labeling individuals ;
		symbol1 value = dot h=1.5 i=join repeat = 20;
 		...
 		axis1 	label=(f=zapf h=4 'Day' ) 	value=(f=zapf h=1.3) order = (1 to 28) minor= none split="*";
		...
		DATA anno_mixed; 
			...
			* AFTER that, draw bars ('A' option ensure properly layering over the greay bars!);
			X=day; 	y=estimate; FUNCTION='MOVE'; when = 'A';  OUTPUT; * start at mean ;
					Y=lower; 	FUNCTION='DRAW'; when = 'A'; line=1; size=6; color='black';  OUTPUT; * draw down;
		
			LINK TIPS; * make bar;

			Y=upper;	FUNCTION='DRAW'; when = 'A'; line=1; size=6; color='black';  OUTPUT; * draw up; 
		
			LINK TIPS; * make bar;
		
			* draw top and bottoms of bars;
			TIPS:
			  X=day-.1; FUNCTION='DRAW'; when = 'A'; line=1; size=6; color='black'; OUTPUT;
			  X=day+.1; FUNCTION='DRAW'; when = 'A'; line=1; size=6; color='black'; OUTPUT;
		...
		* plot original data jitterd, estimated means and 95% CIs;
		...
		axis1 	label=(f=zapf h=4 'Day' ) 	value=(f=zapf h=1.1) order = (0 to 29) minor= none split="*"; * give one extra day in each direction to allow space for boxplots ; 
		axis2 	label=(f=zapf h=4 a=90 &description ) 	value=(f=zapf h=3) 
					order= (0 to 450 by 50) 	major=(h=1.5 w=2) minor=(number=4 h=1);
 	
		symbol1 value=dot h=1 ci=red;
		symbol2 ci=blue value=circle h=1;
		symbol3 value = diamond co=black h = 2 i = join w = 4;
...
/***************************** (see KITE cd4_cd8.sas for method of annotating with 2 groups. its very easy
			**  MAKE MIXED PLOTS FOR SOURCE OF MEASUREMENT;
				...
				* plot original data jitterd, estimated means and 95% CIs;
				...
				axis1 	label=(f=zapf h=4 'Day' ) 	value=(f=zapf h=1) order = (0 to 29) minor= none split="*"; * give one extra day in each direction to allow space for boxplots ; 
				axis2 	label=(f=zapf h=4 a=90 &description ) 	value=(f=zapf h=3) 
							order= (0 to 450 by 50) 	major=(h=1.5 w=2) minor=(number=4 h=1);

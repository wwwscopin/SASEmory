 	...
 	proc greplay igout= glnd_rep.graphs nofs; delete _ALL_; run; 	*clear out the graphs catalog;
  	...
  	%do %while (&x <2);
		...
		* populate 'n' annotation variables ;
		...
		data glu;
				set glnd.followup_all_long(keep=id gluc_eve eve_gluc_src rename=(gluc_eve=glu eve_gluc_src=src) in=C)
				 glnd.followup_all_long(keep=id gluc_aft aft_gluc_src rename=(gluc_aft=glu aft_gluc_src=src) in=B)
				 glnd.followup_all_long(keep=id gluc_mrn mrn_gluc_src rename=(gluc_mrn=glu mrn_gluc_src=src) in=A);
		...
		*ods trace on/label listing;
		proc mixed data = glu empirical covtest;
			...
			model glu = / solution covb; 
			repeated / subject = id type = cs r;
			ods output Mixed.CovParms=cov;
		run;
        *ods trace off;

        proc varcomp data = glu method=reml;
			class id ;
			model glu =id / fixed=0; 
		run;        

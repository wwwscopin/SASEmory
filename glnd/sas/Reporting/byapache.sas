...
value = (h=1.25 "Apache<=15" "Apache>15") offset=(-0.2in, -0.2 in) frame;
...
title1 	h=4 Means and 95%CI by Apache>15 and Days on Study;
%if &var # log_gsh_conc log_gssg_conc log_il6 log_il8 log_ifn log_tnf
	 %then %do;
        title1 	h=4 Geometric Means and 95%CI by Apache>15 and Days on Study;
     %end;
title2 h=4 for &title;
title3 	height=4 "p(Apache>15)=&p1, p(Days)=&p2, p(Apache>15*Days)=&p3";
...
proc gplot data= estimate gout=glnd_rep.graphs;
	...
	note h=1.75 m=(1pct, 7 pct) "(#Apache<=15)" ;
	note h=1.75 m=(1pct, 5 pct) "(#Apache>15)" ;
...
%mixed(hsp,hsp70_ng,apache_2,&ylab, &title); run; 

** For redox;
%let ylab="GSH/GSSG Redox (mV)";
%let title=GSH/GSSG Redox;
%mixed(redox,GSH_GSSG_redox,apache_2,&ylab, &title); run; 

* Cytokines;

%let ylab="IL-6 Concentration (pg/ml)";
%let title=IL-6 Concentration (pg/ml) ;
%mixed(cyto,log_il6,apache_2,&ylab, &title); run; 


	ods pdf file = "glnd_apache.pdf";
...
ods tagsets.excelxp file="glnd_apache.xls";
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND_apache.rtf" style=journal bodytitle startpage=never;

	proc report data=test_hsp70_ng  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for HSP-70(ng/dL) by Apache>15 and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"Apache<=15*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Apache>15*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_GSH_GSSG_redox   nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSH/GSSG Redox (mV) by Apache>15 and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"Apache<=15*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il6 nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines IL-6 (pg/mL) by Apache>15 and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"Apache<=15*Mean[95%CI], N" style(column)=[width=2in];

...
%if &var=log_gssg_conc %then %do; %let scaley=(0 to 0.3 by 0.02); %let scalex=(-1 to 29 by 1); %let y1 = 0.01; %let y2= 0.1; %end;
...
value = (h=1.25 "Any Infection=No" "Any Infection=Yes") offset=(-0.2in, -0.2 in) frame;
...
title1 	h=4 Means and 95%CI by Any Infection and Days on Study;
%if &var # log_gsh_conc log_gssg_conc log_il6 log_il8 log_ifn log_tnf
	 %then %do;
        title1 	h=4 Geometric Means and 95%CI by Any Infection and Days on Study;
     %end;
title2 h=4 for &title;
title3 	height=4 "p(Infection)=&p1, p(Days)=&p2, p(Infection*Days)=&p3";
...
proc gplot data= estimate gout=glnd_rep.graphs;
	...
	note h=1.75 m=(1pct, 7 pct) "(#Any Infection=No)" ;
	note h=1.75 m=(1pct, 5 pct) "(#Any Infection=Yes)" ;
...
%mixed(hsp,hsp70_ng,any,&ylab, &title); run; 

** For redox;
%let ylab="GSH/GSSG Redox (mV)";
%let title=GSH/GSSG Redox;
%mixed(redox,GSH_GSSG_redox,any,&ylab, &title); run; 

%let ylab="GSSG Concentration (" f=greek 'm' f=fixed "M)";
%let title=GSSG Concentration;
%mixed(redox,log_gssg_conc,any,&ylab, &title); run; 

* Cytokines;

%let ylab="IL-6 Concentration (pg/ml)";
%let title=IL-6 Concentration (pg/ml) ;
%mixed(cyto,log_il6,any,&ylab, &title); run; 


	ods pdf file = "glnd_infec_any_exclude_wk2.pdf";
...
ods tagsets.excelxp file="glnd_infect_any_exclude_wk2.xls";
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND_any_exclude_wk2.rtf" style=journal bodytitle startpage=never;

	proc report data=test_hsp70_ng  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for HSP-70(ng/dL) by Any Infection and Days on Study";
	...
	proc report data=test_GSH_GSSG_redox   nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSH/GSSG Redox (mV) by Any Infection and Days on Study";
    	column day est1 est2 diff pv;
    	define day/"Day";
    	define est1/"No*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Yes*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_gssg_conc  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSSG Concentration (&mu.M) by Any Infection and Days on Study";
    	column day est1 est2 diff pv;
    	define day/"Day";
    	define est1/"No*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il6 nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines IL-6 (pg/mL) by Any Infection and Days on Study";
      	column day est1 est2 diff pv;
      	define day/"Day";
    	define est1/"No*Mean[95%CI], N" style(column)=[width=2in];
    	define est2/"Yes*Mean[95%CI], N" style(column)=[width=2in];

...
%if &var=sofa_tot %then %do; %let scaley=(0 to 16 by 1); %let scalex=(0 to 29 by 1); %let y1 = 0; %let y2= 0; %end;
%if &var=glutamicacid %then %do; %let scaley=(0 to 260 by 20); %let scalex=(-1 to 29 by 1); %let y1 = 10; %let y2= 131; %end;
%if &var=glutamine %then %do; %let scaley=(200 to 900 by 50); %let scalex=(-1 to 29 by 1); %let y1 = 205; %let y2= 756; %end;
%if &var #gluc_all glu_local %then %do; %let scaley=(40 to 200 by 10); %let scalex=(0 to 29 by 1); %let y1 = 80; %let y2= 130; %end;
%if &var #glucose glu_center  %then %do; %let scaley=(40 to 200 by 10); %let scalex=(-1 to 29 by 1); %let y1 = 80; %let y2= 130;%end;
...
%if &var=Cys_CySS_redox %then %do; %let scaley=(-120 to -20 by 5); %let scalex=(-1 to 29 by 1); %let y1 = -98; %let y2= -62; %end;
%if &var=log_gsh_conc %then %do; %let scaley=(0 to 4 by 0.2); %let scalex=(-1 to 29 by 1); %let y1 = 0.9; %let y2= 2.9; %end;
%if &var=log_gssg_conc %then %do; %let scaley=(0 to 3.2 by 0.2); %let scalex=(-1 to 29 by 1); %let y1 = 0.01; %let y2= 0.1; %end;
%if &var=Cys_concentration %then %do; %let scaley=(0 to 60 by 5); %let scalex=(-1 to 29 by 1); %let y1 = 4; %let y2= 16; %end;
%if &var=CysSS_concentration %then %do; %let scaley=(20 to 220 by 10); %let scalex=(-1 to 29 by 1); %let y1 =30; %let y2= 85; %end;
...
%if &var=anti_flag_IgA %then %do; %let scaley=(1.0 to 3.6 by 0.2); %let scalex=(-1 to 29 by 1); %let y1 = .04; %let y2= .9; %end;
...
%mixed(glucose_center,glu_center,hospital_death,&ylab, &title); run;
	...
	ods pdf file = "glnd_survivor.pdf";
...
ods tagsets.excelxp file="glnd_graph_survivor.xls";
ods tagsets.excelxp
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND Data by Survivor.rtf" style=journal bodytitle startpage=never;

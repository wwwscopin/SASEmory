...
%if &var #gluc_all glu_local %then %do; %let scaley=(60 to 180 by 10); %let scalex=(0 to 29 by 1); %end;
%if &var #glucose glu_center  %then %do; %let scaley=(60 to 180 by 10); %let scalex=(-1 to 29 by 1); %end;
...
%if &var=log_gsh_conc %then %do; %let scaley=(0 to 2 by 0.2); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_gssg_conc %then %do; %let scaley=(0 to 0.2 by 0.02); %let scalex=(-1 to 29 by 1); %end;
...
%if &var=log_il6 %then %do; %let scaley=(0 to 200 by 10); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_il8 %then %do; %let scaley=(0 to 80 by 5); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_ifn %then %do; %let scaley=(0 to 80 by 5); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_tnf %then %do; %let scaley=(0 to 12 by 1); %let scalex=(-1 to 29 by 1); %end;

%if &var=anti_flag_IgG %then %do; %let scaley=(0 to 1.2 by 0.2); %let scalex=(-1 to 29 by 1); %end;
...
title3 	height=2 "p(Treatment)=&p1, p(Days)=&p2, p(Treatment*Days)=&p3";
...
ods tagsets.excelxp file="glnd_graph_week2.xls";
ods tagsets.excelxp
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND Data Week2.rtf" style=journal bodytitle startpage=never;
	...
	proc report data=test_gluc_all nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Blood Glucose (Local Lab) by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glucose  nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Blood Glucose(Central Lab) by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glu_local nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Blood Glucose (Local Lab for Matched Only) by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glu_center  nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Blood Glucose(Central Lab for Matched Only) by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_gsh_conc  nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of GSH concentration by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_gssg_conc  nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of GSSG concentration by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il6 nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Cytokines IL-6 by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il8 nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Cytokines IL-8  by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_ifn nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Cytokines IFN  by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_tnf nowindows split="*" style(column)=[just=center];
	    title "Model Based Estimate of Cytokines TNF by Treatment and Days";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];

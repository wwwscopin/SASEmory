...
%if &var=hsp70_ng %then %do; %let scaley=(0 to 80 by 5); %let scalex=(-1 to 29 by 1); %let y1 = 0; %let y2= 2; %end;
%if &var=hsp27_ng %then %do; %let scaley=(0 to 2 by 0.2); %let scalex=(-1 to 29 by 1); %let y1 = 0; %let y2= 2; %end;

%if &var=GSH_GSSG_redox %then %do; %let scaley=(-160 to -100 by 5); %let scalex=(-1 to 29 by 1); %let y1 = -155; %let y2= -121; %end;
...
%if &var=log_gssg_conc %then %do; %let scaley=(0 to 0.2 by 0.02); %let scalex=(-1 to 29 by 1); %let y1 = 0.01; %let y2= 0.1; %end;
...
%if &var=log_il6 %then %do; %let scaley=(0 to 200 by 10); %let scalex=(-1 to 29 by 1); %let y1 = 0.447; %let y2= 9.96;  %end;
...
title1 	h=4 Means and 95%CI by Treatment and Days on Study;
%if &var # log_gsh_conc log_gssg_conc log_il6 log_il8 log_ifn log_tnf
	 %then %do;
        title1 	h=4 Geometric Means and 95%CI by Treatment and Days on Study;
     %end;
title2 h=4 for &title;
title3 	height=4 "p(Treatment)=&p1, p(Days)=&p2, p(Treatment*Days)=&p3";
...
%let ylab="TNF-a Concentration (pg/ml)";
...
%let title=Blood Glucose(Local Lab, 5184 obs) ;
%mixed(all_glucose,gluc_all,treatment,&ylab, &title); run;

%let ylab="Blood Glucose (mg/dL)";
%let title=Blood Glucose(Central Lab, 329 obs) ;
%mixed(center_glucose,glucose,treatment,&ylab, &title); run;


%let ylab="Blood Glucose (mg/dL)";
%let title=Blood Glucose(Local Lab for Matched Only, 3278 obs) ;
%mixed(glucose_local,glu_local,treatment,&ylab, &title); run;

%let ylab="Blood Glucose (mg/dL)";
%let title=Blood Glucose(Central Lab for Matched Only, 329 obs) ;
%mixed(glucose_center,glu_center,treatment,&ylab, &title); run;




	ods pdf file = "glnd_wk2.pdf";
...
ods tagsets.excelxp file="glnd_graph_wk2.xls";
ods tagsets.excelxp
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="GLND Data_wk2.rtf" style=journal bodytitle startpage=never;
	
	proc report data=test_sofa_tot nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for SOFA by Treatment and Days on Study";
	...
	proc report data=test_glutamine nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Glutamine(&mu.M) by Treatment and Days on Study";
     	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glutamicacid nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Glutamic Acid (&mu.M) by Treatment and Days on Study";
    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_gluc_all nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Blood Glucose(mg/dL)(Local Lab) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glucose  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Blood Glucose (mg/dL) (Central Lab) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glu_local nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Blood Glucose (mg/dL) (Local Lab for Matched Only) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_glu_center  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Blood Glucose (mg/dL)(Central Lab for Matched Only) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_hsp70_ng  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for HSP-70(ng/dL) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_hsp27_ng  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for HSP-27(ng/dL) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_GSH_GSSG_redox   nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSH/GSSG Redox (mV) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_Cys_CySS_redox  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cys/CySS Redox (mV) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_gsh_conc  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSH Concentration (&mu.M) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_gssg_conc  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for GSSG Concentration (&mu.M) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_Cys_concentration   nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cys Concentration (&mu.M) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_CysSS_concentration  nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for CysSS Concentration (&mu.M) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il6 nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines IL-6 (pg/mL) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_il8 nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines IL-8 (pg/mL) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_ifn nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines IFN (pg/mL)  by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_log_tnf nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for Cytokines TNF (pg/mL) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_anti_flag_IgG nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-Flagellin IgG (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_anti_flag_IgA nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-Flagellin IgA (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];

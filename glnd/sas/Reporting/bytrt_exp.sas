...
data lsmean;
	...
	%if &var in log_il6, log_il8, log_ifn, log_tnf %then %do;
...
%if &var=log_il6 %then %do; %let scaley=(0 to 200 by 20); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_il8 %then %do; %let scaley=(0 to 80 by 10); %let scalex=(-1 to 29 by 1); %end;
%if &var=log_ifn %then %do; %let scaley=(0 to 100 by 10); %let scalex=(-1 to 29 by 1); %end;
...
proc gplot data= estimate gout=glnd_rep.graphs;
	...
	%else %if &var in log_il6, log_il8, log_ifn, log_tnf %then %do; format day dt. estimate1 4.0; %end;
...
filename output 'cyto_exp_trt.eps';

...
         					symbol2 ci=blue value=dot h=1;
     ...
     %if &x = 1 %then %do; 	%let variable =log_il6; %let description = f=zapf 'IL-6 Concentration (pg/ml)';
       			%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/; %let y1 = 0.447; %let y2= 9.96; %let pic= 'A)'; %end;
         
     %if &x = 2 %then %do; 	%let variable =log_il8; %let description = f=zapf 'IL-8 Concentration (pg/ml)';
       			%let scale = /*order = (0 to 25 by 1) minor=(number=3) */ ; %let y1 = 3.23; %let y2= 24.5; %let pic= 'B)'; %end;

		%if &x = 3 %then %do; 	%let variable =log_ifn; %let description = f=zapf 'IFN' f=greek ' g ' f=zapf 'Concentration (pg/ml)';
       			%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/; %let y1 = 0.25; %let y2= 15.6; %let pic= 'C)'; %end;
         
     	%if &x = 4 %then %do; 	%let variable =log_tnf; %let description = f=zapf 'TNF' f=greek ' a ' f=zapf 'Concentration (pg/ml)';
         		...
         		* populate 'n' annotation variables ;
         		...
         		axis1 	label=(f=zapf h=2 'Day' ) value=(f=zapf h=1.25) split="#" order= (-1 to 29 by 1) minor=none offset=(0 in, 0 in);
         		...
         		proc gplot data=cyto_plot gout=glnd_rep.graphs;
         			...
         			note h=2 m=(7pct, 7.5 pct) "(n)" ;
						%if &variable=log_ifn %then %do; 
										...
										note f='zapf / it' m=(1,1) h=1.5 "* &k_0 = measures| &m_0 = non-detectable";
								%end;
						%if &variable=log_tnf %then %do; 
										format visit time_axis_C. &variable 4.1; 
										note f='zapf / it' m=(1,1) h=1.5 "* &s_0 = measures| &t_0 = non-detectable";
								%end;
...
filename output 'cyto_open_nolog1.eps';
...
filename output 'cyto_open_nolog2.eps';
goptions reset=all rotate = portrait device=pslepsfc gsfname=output gsfmode=replace;        	

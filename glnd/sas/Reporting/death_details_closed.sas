...
options orientation = landscape papersize=("11.5", "10") leftmargin= .1 rightmargin = .1 topmargin =0.5 nodate nonumber;
...
ods pdf file = "/glnd/sas/reporting/death_details_closed3.pdf" style=journal;
ods ps file = "/glnd/sas/reporting/deathdetailsclosed3.ps" style=journal;
	proc print data = glnd_rep.death_details_closed label noobs;
	where id >40000;
		var treatment center id age gender race dt_admission primary_diag in_sicu_choice indic_pn apache_score hospital_death days_hosp days_study reasons_death;
	run;

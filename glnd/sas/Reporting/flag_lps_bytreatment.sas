...
proc freq data=flag_lps;
    tables day;
run;
proc print;
var id day anti_lps_igm;
run;
...
%if &var=anti_lps_IgA %then %do; %let scaley=(0 to 2.4 by 0.2); %let scalex=(-1 to 29 by 1); %let y1 = .4; %let y2= .8; %end;
...
proc genmod data =tmp;
	class &trt id day ; 	
	model &var=&trt day &trt*day/dist =poisson link = log type3 noscale maxiter=250000; 
...
%mixed(flag_lps,anti_flag_IgM,treatment,&ylab, &title); run; 


%let ylab="a-LPS IgG (O.D.)";
%let title=a-LPS IgG (O.D.) ;
%mixed(flag_lps,anti_lps_IgG,treatment,&ylab, &title); run; 

%let ylab= "a-LPS IgA (O.D.)";
%let title="a-LPS IgA (O.D.)" ;
%mixed(flag_lps,anti_lps_IgA,treatment,&ylab, &title); run; 

%let ylab="a-LPS IgM (O.D.)";
%let title=a-LPS IgM (O.D.) ;
%mixed(flag_lps,anti_lps_IgM,treatment,&ylab, &title); run; 



	ods pdf file = "flag_lps_bytrt.pdf";
...
ods tagsets.excelxp file="flag_lps_bytrt.xls";
/*
ods tagsets.excelxp
...
ods tagsets.excelxp close;
	...
	ods  rtf  file="flag_lps_bytrt.rtf" style=journal bodytitle startpage=never;
	/*
	...
	*/
	...
	proc report data=test_anti_flag_IgM nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-Flagellin IgM (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_anti_lps_IgG nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-LPS IgG (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_anti_lps_IgA nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-LPS IgA (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];
	...
	proc report data=test_anti_lps_IgM nowindows split="*" style(column)=[just=center];
	    title "Model-based Means and 95% CIs for alpha-LPS IgM (O.D.) by Treatment and Days on Study";
    	    	*column day est1 est2 diff pv;
    	column day est1 est2;
    	define day/"Day";
    	define est1/"AG-PN*Mean[95%CI], N" style(column)=[width=2in];

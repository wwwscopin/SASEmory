	...
	run;
         ...
         %macro make_plots (trt = ); 

			proc sort data= hsp_data; by id ; run;
         	...
         	%let x= 1;
           	...
           	%do %while (&x < 3);
          		...
          		%if &x = 1 %then %do; 	%let variable =hsp70_ng; %let description = f=zapf 'Heat-shock Protein 70 (ng/mL)';
         							%let scale = order = (0 to 180 by 10) minor=(number=1); %let y1 = 0; %let y2= 2; %let pic= 'A)'; %end;
         
         		%if &x = 2 %then %do; 	%let variable =hsp27_ng; %let description = f=zapf 'Heat-shock Protein 27 (ng/mL)';
         							%let scale = /*order = (0 to 3 by 1) minor=(number=3) */ ; %let y1 = 0; %let y2= 2; %let pic= 'B)'; %end;
         		...
         		* populate 'n' annotation variables ;
         		...
         		title2 h=3.5 justify=center &description ", treatment" &group ;
...
         
		...
		goptions rotate = portrait;
        	...
        	ods ps file = "/glnd/sas/reporting/hsp_closed_1.ps";
        	ods pdf file = "/glnd/sas/reporting/hsp_closed_1.pdf";
        	...
        	ods ps file = "/glnd/sas/reporting/hsp_closed_2.ps";
        	ods pdf file = "/glnd/sas/reporting/hsp_closed_2.pdf";
	 	    proc greplay igout = glnd_rep.graphs  tc=sashelp.templt template= v2s nofs; * L2R2s;
        			treplay 1:gplot1 2:gplot3;
        		run;
        	ods pdf close;
        	ods ps close;

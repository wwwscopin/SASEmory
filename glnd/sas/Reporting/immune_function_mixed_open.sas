...
%macro make_plots; 
	...
	%let l= 1;
  	...
  	%do %while (&l < 5);
 		...
 		%if &l = 1 %then %do; 	%let variable =ros_prod_stim; %let description = f=cent 'Neutrophil ROS production [stimulated] (%)';
							%let scale = order = (0 to 100 by 10)  minor=(number=3); %let y1 = 8; %let y2= 25; %let pic= 'A)'; %end; 

 		%if &l = 2 %then %do; 	%let variable =ros_dif; %let description = f=cent 'Neutrophil ROS production [stimulated - control] (%)';
							%let scale = order = (0 to 100 by 10)  minor=(number=3); %let y1 = 8; %let y2= 25; %let pic= 'B)'; %end; 

		%if &l = 3 %then %do; 	%let variable =phago_stim; %let description = f=cent 'Neutrophil phagocytosis [stimulated] (%)'; 
							%let scale = order = (40 to 100 by 10) minor=(number=3); %let y1 = .7; %let y2= 1.2; %let pic= 'C)'; %end; 

		%if &l = 4 %then %do; 	%let variable =phago_dif; %let description = f=cent 'Neutrophil phagocytosis [stimulated - control] (%)'; 
...
************************************************************************************************************;
************************************************************************************************************;
		proc mixed data = immune empirical covtest;
			class id day ; * &source;
		...
		* merge the means and CIs into gluc_box to obtain plotting dataset;
		proc sort data = immune out=tmp; by day; run;
...
************************************************************************************************************;
		...
		proc gplot data=immune1 gout=glnd_rep.graphs;
			...
			note h=2 f=cent m=(7pct, 10 pct) "Day:" ;
			note h=2 f=cent m=(7pct, 7.5 pct) "(n)" ;
...
filename output 'immune_function_open_1.eps';
...
filename output 'immune_function_open_2.eps';
goptions reset=all rotate = portrait device=pslepsfc gsfname=output gsfmode=replace;

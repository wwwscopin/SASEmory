...
run;
			 ...
			 symbol1 value=circle  i=j h=4 w=1 c=blue repeat=130; 
...
         
         		...
         		proc gplot data=cyto_plot gout=glnd_rep.graphs;
         			plot &variable*visit2=id / haxis = axis1 vaxis = axis2  nolegend annotate=anno;
  

         			note h=2 m=(7pct, 10 pct) "Day:" ;
         			note h=2 m=(7pct, 7.5 pct) "(n)" ;

         			format visit2 time_axis. &variable 4.1;
         ; 
         		run;	
        ...
        goptions rotate = portrait;
        	...
        	ods ps file = "/glnd/sas/reporting/log_cyto_line1.ps";
        	ods pdf file = "/glnd/sas/reporting/log_cyto_line1.pdf";
			...
			ods ps file = "/glnd/sas/reporting/log_cyto_line2.ps";
        	ods pdf file = "/glnd/sas/reporting/log_cyto_line2.pdf";
	 	    proc greplay igout = glnd_rep.graphs  tc=sashelp.templt template= v2s nofs; * L2R2s;

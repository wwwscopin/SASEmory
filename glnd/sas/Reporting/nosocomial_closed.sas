 ...
 * and episode tables in one program, simply inheriting the revised "adjudicated" dataset from "nosocomial_adjudicated_open.sas"
 */
...
if org_spec_1="Cytomegalovirus" then cult_org_code_1=23;
			...
			* Adjust "other" categories to include the name of the organism;
			if (cult_org_code_1 in (9, 20, 21,22,23)) then organism_1= trim(put(organism_1, $50.)) || " - " || trim(put(org_spec_1, $50.)) ;
			if (cult_org_code_2 in (9, 20, 21,22,23)) then organism_2= trim(put(organism_2, $50.)) || " - " || trim(put(org_spec_2, $50.)) ;
			if (cult_org_code_3 in (9, 20, 21,22,23)) then organism_3= trim(put(organism_3, $50.)) || " - " || trim(put(org_spec_3, $50.)) ;
			if (cult_org_code_4 in (9, 20, 21,22,23)) then organism_4= trim(put(organism_4, $50.)) || " - " || trim(put(org_spec_4, $50.)) ;
			if (cult_org_code_5 in (9, 20, 21,22,23)) then organism_5= trim(put(organism_5, $50.)) || " - " || trim(put(org_spec_5, $50.)) ;
            ...
            *This is to remove those ASUTI infections!!!;
            *****************************************************************************;
...
/*
proc print data=noso; title "wbh"; 
where compress(site_code) = "UTI";
run;
*/
...
proc sort data= glnd_rep.episode_summary_closed nodup; by site_code type_code;run;
data glnd_rep.episode_summary_closed;
	set glnd_rep.episode_summary_closed;
	num=_N_;
	if site_code='Overall' then num=99;
run;
...
/* PRINT TABLES */
	...
	options ls=120 nodate  	papersize= ("11", "10")   orientation=portrait center nonumber formdlim='-' formchar = "|----|+|---+=|-/\<>*";
			 * nodate contents;

	ods ps file = "notc1.ps"  style=journal startpage=no style=journal startpage=no;
	...
	ods ps file = "notc2.ps"  style=journal startpage=no style=journal startpage=no;
	...
	title "xxx";
	where site_code not in ("Site","Emor" ,"Miri","Vand" ,"Colo" ,	"Wisc");
	run;
	
	/*proc sort data= glnd_rep.episode_summary_closed; by site_code type_code; run;*/

		options ls=200; *nodate  	papersize= ("10", "9.5")  orientation=portrait center nonumber formdlim='-' formchar = "|----|+|---+=|-/\<>*"; 	
				* nodate contents;

		ods ps file = "notc3.ps" style = journal startpage=no ;
			title1 "GLND - Summary of Nosocomial Infections";
			title2 "By Type";


			proc report data= glnd_rep.episode_summary_closed nowindows split = '*' style= [font_width = compressed] ;
	
				where site_code not in ("Site","Emor" ,"Miri","Vand" ,"Colo" ,	"Wisc");
				columns site_code type_code prevalent_a incident_a prevalent_b incident_b;
				
				define site_code/group order=data format=$site_code. style(column) = [just=left];
				define type_code/style(column) = [just=left];
				define prevalent_a /"A*Prevalent Infec.:*# patients (% prev.)" style(column) = [just=center]; * separate var statement for separate atrributes ;
				define incident_a /"A*Incident Infec.:* # infec. (#patients, % incid.)" style(column) = [just=center]; * separate var statement for separate atrributes ;
				define prevalent_b /"B*Prevalent Infec.:*# patients (% prev.)" style(column) = [just=center]; * separate var statement for separate atrributes ;
				define incident_b /"B*Incident Infec.:* # infec. (#patients, % incid.)" style(column) = [just=center]; * separate var statement for separate atrributes ;	
			run;
		ods ps close;
  	...
  	*************/;
		...
		*options ls=80 nodate  orientation=portrait papersize= ("8", "6")  topmargin=0.5 nonumber  formdlim='-' formchar = "|----|+|---+=|-/\<>*"; 	
		options orientation = portrait papersize=("8", "6") leftmargin= .1 rightmargin = .1 topmargin =0.5 nodate nonumber;
		
				
		ods ps file = "notc4.ps" style = journal startpage=no ;
			title1 "GLND - Summary of Nosocomial Infections";
			title2 "By Clinical Site";


			proc print data= glnd_rep.episode_summary_closed noobs label width=minimum split = '*' style(table)= [font_width = compressed] ;
	
				where site_code in ("Emor" ,"Miri","Vand" ,"Colo" ,	"Wisc");
				id site_code   ;
				by site_code notsorted ;

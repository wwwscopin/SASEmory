...
data status;
    merge glnd.status glnd.plate6; by id;
    if 0<apache_total<=11 then apachese4=1;
    else if apache_total<=16 then apachese4=2;
         else if apache_total<=20 then apachese4=3;
              else apachese4=4;
              
              if apache_total<=15 then apa=0; else apa=1;

run;

proc print;
var id apache_total apachese4 apache_2 apa;
run;
	...
	* this is the patient infections, with the adjudicated infection data for those people adjudicated ;
	...
	data all_infections_for_calc;
		...
		merge 	
			...
			sub (keep = id days_hosp_post_entry still_in_hosp in=sub) 
...
data glnd.infectP;
    merge infect(in=A) glnd.george(keep = id treatment rename=(treatment=trt)) sub(where=(still_in_hosp = 0) in=hosp); by id;
...
*/
	...
	* calculate total patient days in hospital for the study ;
		proc means data = sub 	n sum median;
			where (still_in_hosp = 0) & (treatment = &x);
		...
		data rate_table_&x;
			...
			if (~from_header_1) & (~from_header_2) then do;	
				...
				* make columns for display ;
				...
				if incident_cases_sum=0 then do; 
				    display_count_people_&group=" ";
				    display_rate_&group=" ";
				    display_ci_&group=" ";
				end;
			end;
...
** GET N's for titles - needs to happen outside of macro since the variables' scopes are otherwise within the macros ** ;
	...
	* capture sample size of total people on study, that are no longer in the hospital because we need to know hospital ; 
		data n;
			merge	sub (keep = id still_in_hosp)
				glnd.george (keep = id treatment)
			;
			by id;
		run;
...
** end ** ;
	...
	* MERGE BOTH TERATMENTS ;
	...
	data rate_table_closed;
	   /*merge rate_table_closed tab; by order;*/
   	   set rate_table_closed; by order;
	run;
	
		* need to put titles in here so can access local macro variables ;
		title1 "Incident infection rates: AG-PN = &n_study_A patients, STD-PN = &n_study_B patients";
		title2 "(Patient hospital days observed: AG-PN = &total_hosp_days_A days, STD-PN = &total_hosp_days_B days)";
	...
	*ods pdf file = "/glnd/sas/reporting/nosocomial_rates_closed.pdf" style = journal;
	ods ps file = "ratesclosed_sub.ps" style = journal;		
	ods rtf file="rates_closed_apache1.rtf" style=journal bodytitle;
		...
		proc print data = rate_table_closed noobs label split = '+' style(table)= [font_width = compressed];
            ...
            *var pvalue/style(data) = [just=center];
		    ...
		    display_count_people_A="AG-PN+#infec.(# pat.)"
		    display_rate_A="AG-PN+infec./1000 hosp.days"
   		    display_count_people_B="STD-PN+#infec.(# pat.)"
		    display_rate_B="STD-PN+infec./1000 hosp.days"
		    display_ci_A="AG-PN+95%CI"
		    display_ci_B="STD-PN+95%CI";

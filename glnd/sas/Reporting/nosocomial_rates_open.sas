...
data _null_;
    set total_infect; 
    call symput("ny", compress(incident_people_sum));
    call symput("fno", put(100-incident_people_sum/&n_study_1*100,2.0));
run;

%let no=%eval(&n_study_1-&ny);
...
** combine these totals and calculate Poisson 95% CI ;
	...
	** stack!;
	...
	ods ps file = "nosocomial_rates_open.ps" style = journal;
		
		title1 "Incident nosocomial infection rates in the &n_study_1 patients no longer hospitalized";
		...
		** capture asymptomatic or unknown UTI infection counts for note  **;
		...
		ods escapechar='^' ;
		ods ps text = " ";
		ods ps text = "^S={font_size=11pt font_style= slant just=left}Note: Asymptomatic UTI (n = &non_SUTI) are excluded from all results";
		ods ps text = " ";
	...
	/*
	    	ods ps text = " ";
		ods ps text = "^S={font_size=11pt font_style= slant just=left}** &fno % (&no/&n_study_1) of patients have no new hospital infections.";
	*/

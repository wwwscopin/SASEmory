...
proc options group=listcontrol;run;
	options ls=100 ps=80 nodate 	orientation = landscape center nonumber formdlim='-' formchar = "|----|+|---+=|-/\<>*";
...
proc template;
      define style wbh_journal;
      parent = styles.Journal;
      replace Fonts from Fonts "replace fonts except for graphs" /
              'TitleFont' = ("Arial",11pt,Bold)
              'TitleFont2' = ("Arial",10pt,Bold)
              'headingFont' = ("Arial",7pt,Bold)
              'docFont' =  ("Arial",7.5pt)
              'EmphasisFont' = ("Arial",6pt,Italic)
              'headingEmphasisFont' = ("Arial",7pt,Bold Italic);
end;
run;

  
  %macro x(id);
  ods ps file = "/glnd/sas/reporting/nut&id..ps" style = wbh_journal startpage=no /* contents */;
  ods pdf file = "/glnd/sas/reporting/nut&id..pdf" style = wbh_journal startpage=no /* contents */;
		...
		proc print data= percent_pn noobs label ;
			...
			*title2 h=3 '#byval1';
			...
			var row /style(data) = [just=center];
			var daily_kcal_goal daily_prot_goal iv_kcal prop_kcal pn_aa_kcal pn_cho pn_lipid percent_iv_aa percent_iv_dextrose
				 percent_iv_lipid tot_parent_kcal tube_kcal oral_kcal tot_ent_kcal tot_kcal tot_insulin percent_parenteral 
				percent_enteral percent_overall	pn_aa_g tot_ent_prot tot_aa	 percent_parenteral_prot percent_enteral_prot 								percent_overall_prot;

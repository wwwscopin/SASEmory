...
data chemistries;
	...
	* transformations ;
    ...
    if treatment=1 then diff=abs(crp-30.6);
    if treatment=2 then diff=abs(crp-83.2);
run;

proc means data=chemistries(where=(day=0)) n mean std median min max; 
class treatment;
var crp diff;
run;
...
************************************************************************************************************;
************************************************************************************************************;
		proc mixed data = chemistries empirical covtest;
			class id day ; * &source;
		...
		* merge the means and CIs into gluc_box to obtain plotting dataset;
		proc sort data = chemistries out=tmp; by day; run;
...
************************************************************************************************************;
		...
		proc gplot data=chem gout=glnd_rep.graphs;
			plot estimate*day &variable*day2/ overlay haxis = axis1 vaxis = axis2 annotate=anno_mixed_&l nolegend;

			note h=2 f=cent m=(7pct, 9.25 pct) "Day:" ;
			note h=2 f=cent m=(7pct, 6.75 pct) "(n)" ;

	...
	* create recruitment dataset by making one entry for each study month and merging with the per month totals from PROC MEANS;
	data glnd.recruitment;	
		*do study_month = 1 to ( ((year(today()) - 2007) * 12)  + month(today())    -1 /*   <== '-1'Added by Eli temporarily 7/9/07*/ + 2); * 1 to the current month;
		do study_month = 1 to ( ((2011 - 2007) * 12)  + 11    -1 /*   <== '-1'Added by Eli temporarily 7/9/07*/ + 2); * 1 to the current month;
			output;
			put study_month;
		end;
...
run;
	...
	* create projections for all future months;
	data temp;
		do study_month = 1 to 60; * 39 months gives us ~150 patients - project recruitment needed is 150 patients. UPDATE 8/26/09: give one-year extension;
			...
			* Wisconsin will come on board in November;	
			...
			actual_month = mdy(month(actual_month), 15, year(actual_month)); * make observations at mid-point of each month;
                       ...
                       * if expected_patients>150 then expected_patients=150;
			output;
		end;
	...
	* plot both estimated and actual recruitment;
	goptions reset=all device=jpeg rotate=landscape gunit=pct noborder cback=white CHARTYPE=12
  		colors = (black) ftitle=cent  ftext=cent;
  		
		
	 	symbol1 value = "dot" h=2 i=join;
 		symbol2 value =  none h=3 i=join line=21 ;
 		
 		axis1 	label=(f=centb h=3 'Study Month' ) value=(h=1.5) 
					order= (17120 to 19200 by 182)	major=(h=3 w=2) minor=(number=2 h=1) ; * 17113 to 18253;
					*** added 91*2 more months  --- George;
 		axis2 	label=(f=centb h=3 a=90 'Patients Recruited' ) 	value=(h=3) 
					order= (0 to 250 by 25) 	major=(h=1.5 w=2) minor=(number=4 h=1) ;
 		
 		legend1 across=1 down=2 position=(top right outside) mode=protect
 			shape=symbol(3,2) label=(h= 2.5 '')
 			value=( h=2.5 'Actual' 'Expected')
			offset= (-13, -2.5 );
 		
 		title1 f=centb h=3 justify=center "GLND Patient Recruitment Summary - through &cur_month &cur_year";
 		title2 f=centb h=2 justify=center "&act_sum patients recruited of &cur_exp expected (&percent_recruit%)";


		ods ps file = "/glnd/sas/reporting/recruitment.ps";
		ods pdf file = "/glnd/sas/reporting/recruitment.pdf";
		ods rtf file="/glnd/sas/reporting/recruitment.rtf";
...
filename output 'recruitment.eps';

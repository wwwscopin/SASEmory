...
%macro make_blood_plots(idx);
	...
	%let l= 1;
  	%do %while (&l < 7);
 		...
 		%if &l = 1 %then %do; 	%let variable =GSH_GSSG_redox; %let description = f=cent 'GSH/GSSG redox (mV),' " &trt";
							%let scale = /*order = (-180 to -70 by 10)*/ minor=(number=3); %let y1 = -155; %let y2= -121; %let pic= 'A)'; 
							%let description1 = f=cent 'G S H / G S S G'; %end; 

		%if &l = 2 %then %do; 	%let variable =Cys_CySS_redox; %let description = f=cent 'Cys/CySS redox (mV),' " &trt"; 
							%let scale = /*order = (-110 to -20 by 10)*/ minor=(number=4); %let y1 = -98; %let y2= -62; %let pic= 'B)'; 
							%let description1 = f=cent 'C y s / C y S S';%end; 

		%if &l = 3 %then %do; 	%let variable =log_gsh_conc; %let description = f=cent 'ln[GSH concentration) ('f=greek h = 4 'm' h=3 f=cent 'M)],' " &trt";
		...
		%if &l = 4 %then %do; 	%let variable =log_gssg_conc; %let description = f=cent 'ln[GSSG concentration ('f=greek h = 4 'm' h=3 f=cent 'M)],' " &trt";
		...
		%if &l = 5 %then %do; 	%let variable =Cys_concentration; %let description = f=cent 'Cys concentration ('f=greek h = 4 'm' h=3 f=cent 'M),' " &trt"; 
							%let scale = /*order = (0 to 17 by 1)*/ minor=(number=1); %let y1 = 4; %let y2= 16; %let pic= 'E)';
							%let description1 = f=cent 'C y s';%end; 

		%if &l = 6 %then %do; 	%let variable =CysSS_concentration; %let description = f=cent 'CysSS concentration ('f=greek h = 4 'm' h=3 f=cent 'M),' " &trt";
...
************************************************************************************************************;
		...
		proc mixed data = bloodp empirical covtest;
		...
		* merge the means and CIs into gluc_box to obtain plotting dataset;
		proc sort data = bloodp out=tmp; by visit; run;
		...
		proc gplot data=blood_mean gout=glnd_rep.graphs;
...
%mend make_blood_plots;
	...
	*ods pdf file = "S:\Eli_Rosenberg\shared GLND\Bloods\result\Yi_Ruosha\blood_july31.pdf";
		%make_blood_plots(1); run;
		%make_blood_plots(2); run;
	*ods pdf close;


	* spit out 3 one-page PDF files so that George's program can handle them ;

filename output 'redox1c.eps';
goptions reset=all rotate = landscape device=pslepsfc gsfname=output gsfmode=replace;
 		...
 		proc greplay igout = glnd_rep.graphs tc=sashelp.templt template= l2r2s nofs; * L2R2s;
            list igout;
			treplay 1:gplot 2:gplot1 3:gplot6 4:gplot7;
		run;
	ods ps close;
	

filename output 'redox2c.eps';
goptions reset=all rotate = landscape device=pslepsfc gsfname=output gsfmode=replace;


 		proc greplay igout = glnd_rep.graphs tc=sashelp.templt template= l2r2s nofs; * L2R2s;
            list igout;
			treplay 1:gplot2 2:gplot3 3:gplot8 4:gplot9;
		run;
	ods ps close;
	
filename output 'redox3c.eps';
goptions reset=all rotate = landscape device=pslepsfc gsfname=output gsfmode=replace;

	ods ps file = "/glnd/sas/reporting/redox3c.ps";
 		proc greplay igout = glnd_rep.graphs tc=sashelp.templt template= l2r2s nofs; * L2R2s;
            list igout;
			treplay 1:gplot4  2:gplot5  3:gplot10; 2:gplot11;
		run;
	ods ps close;

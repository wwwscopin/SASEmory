	...
	ods ps file = "sofa.ps";
			...
			proc gplot data= sofa;
				...
				* annotate with sample sizes - more should be added if we have more data on dat 23, 24...;
				note h=1.5 m=(7pct, 6 pct) "Day:" ;
				note h=1.5 m=(7pct, 4.5 pct) " (n)" ;
...
goptions device=pslepsfc gsfname=output gsfmode=replace;
	...
	quit;			
		...
		filename output 'sofa_sur_nonsur.eps';
goptions device=pslepsfc gsfname=output gsfmode=replace;
	...
	/* Set up symbol for Boxplot */
	symbol1 interpol=j mode=exclude value=dot co=blue cv=blue ci=blue height=2 bwidth=3 width=4;
	/* Set up Symbol for Data Points */
	symbol2 i=j co=red cv=red ci=red value=circle h=2 w=4;
		...
		* now make a table of the means and 95% CI for sofa score on days 0, 7, 14;
		data glnd_rep.sofa_mixed_closed;
			...
			label
				...
				sofa_s = "Total Sofa Score for Survivors*mean (95% CI)"
				sofa_d = "Total Sofa Score for Non-Survivors*mean (95% CI)"

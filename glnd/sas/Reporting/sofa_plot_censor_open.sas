...
proc print data=glnd.followup_all_long;
where day=1 and sofa_tot=.;
run;
...
data sofa_day;
	set sofa;
	where day=1 and sofa_tot=. or id=32318;
run;
		...
		DATA anno_mixed_&data; 
			...
			* AFTER that, draw bars ('A' option ensure properly layering over the greay bars!);
			X=day; 	y=estimate; FUNCTION='MOVE'; when = 'A';  OUTPUT; * start at mean ;
					Y=lower; 	FUNCTION='DRAW'; when = 'A'; line=1; size=2; color=&mycolor;  OUTPUT; * draw down;
		
			LINK TIPS; * make bar;

			Y=upper;	FUNCTION='DRAW'; when = 'A'; line=1; size=2; color=&mycolor;  OUTPUT; * draw up; 
		
			LINK TIPS; * make bar;
		
			* draw top and bottoms of bars;
			TIPS:
			  X=day-.1; FUNCTION='DRAW'; when = 'A'; line=1; size=2; color=&mycolor; OUTPUT;
			  X=day+.1; FUNCTION='DRAW'; when = 'A'; line=1; size=2; color=&mycolor; OUTPUT;
...
proc sort data=sofa_all nodup; by day;run;
	...
	symbol2 i=j ci=black value=dot h=2 w=2;
...
options orientation=landscape;
	* plot! ;
	...
	ods ps file = "sofa.ps";
			proc gplot data= sofa;
				...
				* annotate with sample sizes - more should be added if we have more data on dat 23, 24...;
				note h=1.5 m=(7pct, 7.0 pct) "Day:" ;
				note h=1.5 m=(7pct, 5.1 pct) " (n)" ;
	...
	ods pdf file = "sofa_sur_nonsur.pdf";
	ods ps file = "sofa_sur_nonsur.ps";			

	/* Set up symbol for Boxplot */
	symbol1 interpol=j mode=exclude value=dot co=blue ci=blue cv=blue height=2 bwidth=3 width=2;
	/* Set up Symbol for Data Points */
	symbol2 i=j cv=red co=red ci=red value=circle h=2 w=2;
...
run;
			...
			proc gplot data= sofa_all;
				...
				* annotate with sample sizes - more should be added if we have more data on dat 23, 24...;
				note h=1.5 m=(7pct, 8.25 pct) "Day:" ;
				note h=1.5 m=(7pct, 6.5 pct) " (n)" ;
				note f='zapf / it' m=(11,2) h=1.25 "# &s_1 = Survivors,  &d_1 = Non-Survivors";

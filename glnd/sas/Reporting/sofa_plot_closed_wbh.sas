...
proc means data=sofa(where=(day=1));
class treatment;
var sofa_tot;
run;
...
*ods trace on/label listing;
		proc mixed data = sofa empirical covtest;
			class id day treatment ; * &source;
		
			model sofa_tot = day treatment treatment*day/ solution ; 
			repeated day / subject = id type = cs;
			*lsmeans day / cl ;
			ods output Tests3=tmp;
		run;
...
%macro anno(data=sofa);
		proc mixed data = &data empirical covtest;
			...
			repeated day / subject = id type = cs;
			lsmeans day / cl ;
			ods output lsmeans = lsmeans_&data.&x;
		run;

		* merge the means and CIs into gluc_box to obtain plotting dataset;
		proc sort data = &data; by day; run;
		proc sort data = lsmeans_&data.&x; by day; run;

		data lsmeans_&data.&x;
			set lsmeans_&data.&x;
		...
		data &data ;
			merge &data lsmeans_&data.&x;	by day;
   	run;
	   ...
	   DATA anno_mixed_&data; 
			...
			* AFTER that, draw bars ('A' option ensure properly layering over the greay bars!);
			X=day; 	y=estimate; FUNCTION='MOVE'; when = 'A';  OUTPUT; * start at mean ;
					Y=lower; 	FUNCTION='DRAW'; when = 'A'; line=1; size=2; color='blue';  OUTPUT; * draw down;
		
			LINK TIPS; * make bar;

			Y=upper;	FUNCTION='DRAW'; when = 'A'; line=1; size=2; color='blue';  OUTPUT; * draw up; 
		
			LINK TIPS; * make bar;
		
			* draw top and bottoms of bars;
			TIPS:
			  X=day-.1; FUNCTION='DRAW'; when = 'A'; line=1; size=2; color='blue'; OUTPUT;
			  X=day+.1; FUNCTION='DRAW'; when = 'A'; line=1; size=2; color='blue'; OUTPUT;
...
/* Load sample sizes for each day into macro variables, for plot annotation */
	...
	* obtain n-s for each day;
		proc means data= sofa noprint;
			class day;
			var sofa_tot;
			output out = sizes&x n(sofa_tot) = num_obs max(day) = last_day;
		run;
	...
	* change day 0 to blank ;
	...
	goptions reset=all  device=jpeg  rotate=landscape gunit=pct noborder cback=white
		colors = (black red) /*ftitle=Arial*/ ftext=centb  /*fby =Arial*/ hby = 3;

	/* Set up symbol for Boxplot */
	symbol1 interpol=none mode=exclude value=circle co=black cv=black height=1 bwidth=3 width=2;
	/* Set up Symbol for Data Points */
	symbol2 i=j ci=blue value=dot h=2 w=2;

	legend1 across = 1 position=(top right inside) mode = reserve fwidth =.2	shape = symbol(3,2) label=NONE 
   value = (f=centb h=3 "SOFA" "mean") offset=(0, -0.4 in) frame;


	axis1 	label=(f=centb h=2.5 "Day in SICU" ) split="*"	value=(f=centb h= 1.5)  order= (0 to 29 by 1) minor=none offset=(0 in, 0 in);
	axis2 	label=(f=centb h=2.5 a=90 "Total SOFA Score" ) 	value=(f=centb h=2) order= (0 to 24 by 2) offset=(.25 in, .25 in)
	                minor=(number=1); 

	title1	height=3 f=centb "Total SOFA Score";
	title2	height=2.5 f=centb "Treatment &group";


	* plot! ;
			proc gplot data= sofa gout = glnd_rep.graphs;
				plot sofa_tot*day2 estimate*day
	                   / overlay annotate= anno_mixed_sofa haxis = axis1 vaxis = axis2 legend=legend1;
...
filename output 'sofaclosed.eps';
goptions reset=all rotate = portrait device=pslepsfc gsfname=output gsfmode=replace;
	...
	goptions reset=all;
	ods pdf file="sofa_mixed_mean_closed.pdf" style=journal;
	ods ps file="sofa_mean.ps" style=journal startpage = no;
	ods rtf file="sofa_mean.rtf" style=journal startpage = no;
	title1	height=3 f=centb 'Total SOFA score longitudinal model means';

		* now make a table of the means and 95% CI for sofa score on days 0, 7, 14;
		data glnd_rep.sofa_mixed_closed;
				merge lsmeans_sofa1(keep = estimate upper lower day rename=(estimate=estimate_a upper=upper_a lower=lower_a)) 
						 sizes1(keep=day num_obs rename=(num_obs=num_a))
						 lsmeans_sofa2(keep = estimate upper lower day rename=(estimate=estimate_b upper=upper_b lower=lower_b)) 
						 sizes2(keep=day num_obs rename=(num_obs=num_b));	
			by day;
			
			where day in (1, 4, 7, 14,21,28);
 			sofa_a = strip(put(estimate_a, 5.1)) || " (" || strip(put(lower_a, 5.1)) || ", " || strip(put(upper_a, 5.1))|| ")";
			sofa_b = strip(put(estimate_b, 5.1)) || " (" || strip(put(lower_b, 5.1)) || ", " || strip(put(upper_b, 5.1))|| ")";

			
			label
				day = "Day"
				sofa_a = "Total Sofa Score for Treatment A*mean (95% CI)"
				sofa_b = "Total Sofa Score for Treatment B*mean (95% CI)"
				num_a="Treatment A*n="
				num_b="Treatment B*n="
			;
		   ...
		   format day;
		  	...
		  	keep day num_a sofa_a num_b sofa_b;
		run;

			proc print data = glnd_rep.sofa_mixed_closed noobs label split='*' style(data) = [just=center];
				var day num_a sofa_a num_b sofa_b;
			run;
            ...
            proc print data=tmp noobs label split='*' style(data) = [just=center];
                var effect probf;
            run;
            */
     ods rtf close;

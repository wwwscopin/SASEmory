...
data sofa;
	...
	center = floor(id/10000);
 		call symput( "center_name", compress(put(%eval(&i), center.)));
     if (center ~= &i) then delete;
...
proc print data= sofa; var day day2 id sofa_tot; run;
	...
	* loop through the n from proc means, for all days that we have observations;
		%macro get_sizes;
			%do j = 1 %to &last_day;
				data _null_;
					set sizes;
					where day = &j;
					call symput( "n_&j", compress(put(num_obs, 3.0)));
				run;
			%end;
		%mend get_sizes;
	%get_sizes run;
	...
	* change day 0 to blank ;
	...
	axis1 	label=(f=zapf h=2.5 "Day in SICU" ) split="*"	value=(f=zapf h= 1.5)  order= (0 to 28 by 1) minor=none offset=(0 in, 0.25 in);
	...
	title2	height=3 f=zapf 'Total SOFA Score';
...
%mend recruitment_by_center;
 ...
 ods ps file = "/glnd/sas/reporting/sofa_plot_by_center.ps";
	ods pdf file = "/glnd/sas/reporting/sofa_plot_by_center.pdf";

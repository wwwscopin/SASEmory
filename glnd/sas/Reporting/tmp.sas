...
title "wbh2";
proc print data=glnd.gluc_mixed_closed_gluc_mrna;run;
title "wbh3";
proc print data=glnd.gluc_mixed_closed_gluc_afta;run;
		...
		* now make a table of the means and 95% CI for morning, evening, and afternoon blood glucose on days 0, 7, 14;
		data gluc_mixed_&group;
			merge 	glnd.gluc_mixed_closedgluc_eve&group (keep = estimate upper lower day rename = (estimate = mean_eve upper = upper_eve lower = lower_eve))
					glnd.gluc_mixed_closedgluc_mrn&group (keep = estimate upper lower day rename = (estimate = mean_mrn upper = upper_mrn lower = lower_mrn))
					glnd.gluc_mixed_closedgluc_aft&group (keep = estimate upper lower day rename = (estimate = mean_aft upper = upper_aft lower = lower_aft))
				;
			by day;

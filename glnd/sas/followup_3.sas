...
proc print;
	...
	var id day gluc_mrn gluc_aft gluc_eve;
run;

data tmp;
    set glnd.plate23;
    diff1=abs(sofa_tot-6);
    diff2=abs(gluc_mrn-133);
run;

proc means data=tmp  n mean std median min max Q1 Q3;
    var sofa_tot diff1 gluc_mrn diff2;
run;

...
mon=month(y);
yr=year(y);
smon=month(dt_screen);
syr=year(dt_screen);

mon=mon-1;
**** if 0 then set to 12 previous year;
...
* old version pn_4_days and gast_whipple no longer
if sum(pn_4_days,
   ...
   xx=    sum(
        pregnant , clin_sepsis,  malig , seizure , unex_enceph , cirr_bilir,
        renal_dysfunc,  burn_trauma_inj ,  organ_trans,
        invest_drug,  ent_parent_feed, hiv_aids);
 if sum(
        pregnant , clin_sepsis,  malig , seizure , unex_enceph , cirr_bilir,
        renal_dysfunc,  burn_trauma_inj ,  organ_trans,
        invest_drug,  ent_parent_feed, hiv_aids)>=1 then pt_elig=0;   
...
* ods ps file='30dayinelig.ps';
 ...
 title "Reasons Not Eligible in Last Month";
 run;
 *ods ps close;
proc print data=screen;
 var id reas_no_consent study_proc; 
 data e;
 set screen;
 length reason $ 70;
...
data glnd.elignrlastmonth;
 ...
 proc print label noobs;
 * var affil id reason;
 title "Reasons Eligible But Not Enrolled in Last Month";
 run;
 *ods ps close;

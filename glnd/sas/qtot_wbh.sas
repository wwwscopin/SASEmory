...
data hypo_id;
	set glnd.status(keep=id dt_random treatment);
	where dt_random>=mdy(4,01,2008);
	drop 	dt_random;
run;


proc sort data=hypo_id nodupkey; by id; run;
proc means data=hypo_id; 
class treatment;
output out=num n=n;run;

data _null_;
	set num;
	if treatment=. then call symput('nAB', n);
	if treatment=1 then call symput('nA', n);
	if treatment=2 then call symput('nB', n);
run;
...
array ae(20) ae1-ae17 hyper hypo hypo1;
array bae(20) bae1-bae17 bhyper bhypo bhypo1;
array aae(20) aae1-aae17 ahyper ahypo ahypo1;
  do i=1 to 20;
     ae(i)=aae(i)+bae(i);
  end;
  drop i;
 ...
 **** now do cases;
 ...
 data caae;
  ...
  if treatment=1;
  array ae(20) ae1-ae17 hyper hypo hypo1;
  array caae(20) caae1-caae17 cahyper cahypo cahypo1;
  do i=1 to 20;
      caae(i)=ae(i);
  end;
   drop i ae1-ae17 hyper hypo hypo1;
  run;
  ...
  if treatment=2;
   array ae(20) ae1-ae17 hyper hypo hypo1; 
  array cbae(20) cbae1-cbae17 cbhyper cbhypo cbhypo1;
  do i=1 to 20;
      cbae(i)=ae(i);
  end;
   drop i ae1-ae17 hyper hypo hypo1;
  ...
  if treatment=.;
  array ae(20) ae1-ae17 hyper hypo hypo1;
  array cae(20) cae1-cae17 chyper chypo chypo1;
  do i=1 to 20;
      cae(i)=ae(i);
  end;
   drop i ae1-ae17 hyper hypo hypo1;
...
/* Add by WBH */
 if ae in(19,20) then do; na=&nA; nb=&nB; n=na+nb; end;
	length tae aae bae $20;
   drop i;
   format ae ae.;
   ...
   pcta=round(pata*100/na,.1);
    pctb=round(patb*100/nb,.1);
     ...
     tae=put(case,4.)||'('||put(pat,3.)||')'||put(pct,5.1)||'%';
 		if ae in(19,20) then do;
   	    aae=put(casea,4.)||'('||put(pata,3.)||'/'||compress(put(nA,3.))||')'||put(pcta,5.1)||'%';
	       bae=put(caseb,4.)||'('||put(patb,3.)||'/'||compress(put(nB,3.))||')'||put(pctb,5.1)||'%';
	    	 tae=put(case,4.)||'('||put(pat,3.)||'/'||compress(put(n,3.))||')'||put(pct,5.1)||'%';
		end;
...
data anysae;
  ...
  anysae=max(of sae1-sae8);
sanysae=sum(of sae1-sae8);

proc freq;
 tables anysae sanysae sae1-sae8;
run;
proc means data=anysae noprint;
where treatment=1;
var sae1-sae8 anysae;
output out=asae sum=asae1-asae8 aanysae;
run;
proc means data=anysae noprint;
where treatment=2;
var sae1-sae8 anysae;
output out=bsae sum=bsae1-bsae8 banysae;
run;
data psae;
merge asae bsae;
format asae1-asae8 bsae1-bsae8;
array sae(9) sae1-sae8 xanysae;
array bsae(9) bsae1-bsae8 banysae;
array asae(9) asae1-asae8 aanysae;
  do i=1 to 9;
     sae(i)=asae(i)+bsae(i);
  end;
  drop i;
...
array bsae(9) bsae1-bsae8 banysae;
array asae(9) asae1-asae8 aanysae;
array csae(9) csae1-csae8 ctot;
array casae(9) casae1-casae8 catot;
array cbsae(9) cbsae1-cbsae8 cbtot;
do i=1 to 9;
    pata=asae(i);
    casea=casae(i);
    patb=bsae(i);
 ...
 keep i pata--case na nb n xanysae;
  ...
  sae=i;
   ...
   proc print noobs label data=glnd.dsmcsaeall;
    var sae tsae msaedayst;
     ...
     ods rtf file="dsmcsaeclosed.rtf" style=journal;

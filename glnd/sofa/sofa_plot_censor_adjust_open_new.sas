...
%size(sofa_dead);
	...
	axis3 	label=(f=zapf h=2.5 a=90 "Total SOFA Score") value=(f=zapf h=2) order= (0 to 24 by 2) offset=(.25 in, .25 in) minor=(number=1); 
	...
	*ods pdf file = "/glnd/sas/reporting/sofa_plot_censor_adjust_open.pdf";
	...
	title1	height=3 f=zapf "Total SOFA Score for Survivors (n=&num_sur)";
	title2	height=2 f=zapf 'Longitudinal model (means and 95% CI)';

			proc gplot data= sofa_sur;
				plot sofa_tot*day2 estimate*day/overlay annotate= anno_mixed_sofa_sur haxis = axis1 vaxis = axis3 legend=legend1;

				* annotate with sample sizes - more should be added if we have more data on dat 23, 24...;
				note h=1.5 m=(7pct, 8.5 pct) "Day:" ;
				note h=1.5 m=(7pct, 7 pct) " (n)" ;
						
				format day day2 day_glnd_sofa_sur.; 
			run;	
			
	title1	height=3 f=zapf "Total SOFA Score for Non-Survivors (n=&num_dead)";
	title2	height=2 f=zapf 'Longitudinal model (means and 95% CI)';

			proc gplot data= sofa_dead;
				plot sofa_tot*day2 estimate*day/overlay annotate= anno_mixed_sofa_dead haxis = axis1 vaxis = axis2 legend=legend1;

				* annotate with sample sizes - more should be added if we have more data on dat 23, 24...;
				note h=1.5 m=(7pct, 8.5 pct) "Day:" ;
				note h=1.5 m=(7pct, 7 pct) " (n)" ;
						
				format day day2 day_glnd_sofa_dead.; 
			run;

...
data nec;
	...
	followup=StudyLeftDate-dob;
	if nec=1 then followup=ndate-dob;
	rename dfseq=dday;
run;
...
goptions reset=global rotate=landscape gunit=pct noborder cback=white
	...
	*title2 	height=2.5 "Slope of Non-NEC without tx=&sp0, Slope of Non-NEC with tx=&sp2, Slope of NEC=&sp1";
	...
	title "Mean Rate (mean slope) of Decline in Hemoglobin (g/dL) for Controls and NEC LBWIs (also adjusted for Gestational age)";
			...
			run;
		    ods escapechar='^';
			ods rtf text="^S={LEFTMARGIN=1.25in RIGHTMARGIN=1.25in font_size=11pt} * Transfusion rate per 30 days of follow-up. In NEC cases only transfusions given prior to NEC diagnosis are included.

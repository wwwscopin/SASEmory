...
proc means data=nec mean std;
    class nec;
    var gestage;
    output out=test1 mean(gestage)=ga std(gestage)=se_ga;
run;

data _null_;
    set test1;
    if nec=0 then call symput("ga0", compress(put(ga,4.0)||"("||put(se_ga,4.0)||")"));
    if nec=1 then call symput("ga1", compress(put(ga,4.0)||"("||put(se_ga,4.0)||")"));
    if nec=2 then call symput("ga2", compress(put(ga,4.0)||"("||put(se_ga,4.0)||")"));
run;
...
proc mixed data=nec covtest method=MIVQUE0;
	...
	estimate "No tx, slope" day 7 nec*day 7 0 0;
	estimate "Yes NEC, slope" day 7 nec*day 0 7 0;
	estimate "Yes tx, slope" day 7 nec*day 0 0 7;
...
data nec_slope;
    ...
    if _n_=1 then call symput("sp0", compress(put(estimate,5.2)||"("||put(stderr,5.2)||")"));
    if _n_=2 then call symput("sp1", compress(put(estimate,5.2)||"("||put(stderr,5.2)||")"));
    if _n_=3 then call symput("sp2", compress(put(estimate,5.2)||"("||put(stderr,5.2)||")"));
...
data nec_estimate;
	...
	if group=0 and day=0 then call symput("int0", compress(put(estimate,4.1)||"("||put(stderr,4.1)||")"));
	if group=1 and day=0 then call symput("int1", compress(put(estimate,4.1)||"("||put(stderr,4.1)||")"));
	if group=2 and day=0 then call symput("int2", compress(put(estimate,4.1)||"("||put(stderr,4.1)||")"));
...
data fit_tab;
    length n m ntx ga int hb pv followup $12;
    nec=0; n="&n0"; m="&m0"; ntx="&n0_tx"; followup="&day0_fp"; ntx_day=ntx/followup*30; ga="&ga0"; int="&int0"; hb="&sp0"; pv="-"; output;
    nec=1; n="&n1"; m="&m1"; ntx="&n1_tx"; followup="&day1_fp"; ntx_day=ntx/followup*30; ga="&ga1"; int="&int1"; hb="&sp1"; pv="&p3"||"**"; output;
    nec=2; n="&n2"; m="&m2"; ntx="&n2_tx"; followup="&day2_fp"; ntx_day=ntx/followup*30; ga="&ga2"; int="&int2"; hb="&sp2"; pv="-"; output;
...
goptions reset=global rotate=landscape gunit=pct noborder cback=white
	...
	axis1 	label=(h=3 " " ) split="*"	
	   value=(h=1.25) major=(c=black) origin=(,15)pct order= (-1 to 32 by 1) minor=none offset=(0 in, 0 in);
	   
	axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") value=(h=2.5) c=black order= (7 to 18 by 1) offset=(.25 in, .25 in) minor=(number=1); 


	legend across = 1 position=(top right inside) mode = reserve shape = symbol(3,2) label=NONE 
    value = ( h=2 c=black "Non-NEC without tx: Mean Slope(SE)= &sp0 g/dL/week" "NEC                          : Mean Slope(SE)= &sp1 g/dL/week" "Non-NEC with tx     : Mean Slope(SE)= &sp2 g/dL/week") offset=(-1in, -0.4 in) frame cframe=white cborder=black;
	                    
	*title1 	height=4 "Hemoglobin vs Days (non-NEC without tx=&n0(&m0 obs), NEC=&n1(&m1 obs), non-NEC with tx=&n2(&m2 obs))";
	title1 h=4 "Hemoglobin vs Age (days) by NEC /pRBC Transfusion Group adjusted for Gestational Age";
	title2 h=3 "&n0 LBWI Controls without pRBC transfusion, &n2 LBWI Controls with pRBC transfusion and &n1 LBWIs with NEC ";
	*title2 	height=2.5 "Slope of Non-NEC without tx=&sp0, Slope of Non-NEC with tx=&sp2, Slope of NEC=&sp1";
	...
	ods pdf file = "hb_nec_slope_tx_gestage_month.pdf" style=journal;
    ...
    ods rtf file="nec_tab_month.rtf" style=journal bodytitle;			
			...
			title "Mean Rate (mean slope) of Decline in Hemoglobin (g/dL) in the first 4 weeks of life for Controls and NEC LBWIs (also adjusted for Gestational age)";
			proc print data=fit_tab noobs label split="|";
			var nec/style=[just=center width=1.5in];
			var n m /*ntx followup*/ ntx_day ga int /style=[just=center width=1.25in];
			var hb /style=[just=center width=1.5in];
			var pv/style=[just=center width=0.75in];
			label nec="Group"
			      n="#LBWIs"
			      m="#Hb(obs)"
    		      ntx="Number of Tx"
			      ...
			      ga="Gestational Age|Mean(SD)(week)"
			      int="Hb Intercept(SE)|(g/dL)"
   			      Hb="Mean Hb Slope(SE)|(g/dL/week)"
   			      pv="p Value";
			run;
		    ods escapechar='^';
			ods rtf text="^S={LEFTMARGIN=1.25in RIGHTMARGIN=1in font_size=11pt} * Transfusion rate per 30 days of follow-up. In NEC cases only transfusions given prior to NEC diagnosis are included.
			^n ** P value comparing the slopes for Non-NEC with transfusion versus NEC.";	
			ods rtf close;

%include "age_blood_nec_tx.sas";

options orientation=portrait nodate nonumber nofmterr;
%let mu=%sysfunc(byte(181));

proc sort data=cmv.nec_tx out=tmp nodupkey; by id;run;
...
data infect;
	merge tmp hop(keep=id nec necdate in=A); by id;
	...
	if culturedate>=necdate then do; gbsi=0; sub_mrsa=0; sub_mssa=0; mrsa=0; mssa=0; gcard=0; ggi=0; gcns=0; glr=0; gss=0; gut=0; gother=0; gi=0; gnbsi=0;end;
run;

/*
proc print;
var id nec necdate culturedate SiteBlood SiteCardio SiteGI SiteCNS SiteLowerResp SiteSurgical SiteUT SiteOther;
run;
*/
...
ods rtf file="infect_nec_tx.rtf" style=journal bodytitle startpage=no;
proc print data=necinfect noobs label split="|"; 
title1 "Incident infection rates in the &nt LBWIs with pRBC before NEC and no longer hospitalized";
title2 "&total_hosp_days_nec0(&m0 LBWIs Transfused >48 hrs before NEC) and &total_hosp_days_nec1(&m1 LBWIs Transfused <=48 hrs before NEC) patient hospital days on study";
...
label   item="."
		col0="Transfused >48 hrs before NEC|#Patients (Incidence%)"
		rate0="Transfused >48 hrs before NEC|infec.1000/hosp. days 95%CI*"
		rate1="Transfused <=48 hrs before NEC|infec.1000/hosp. days 95%CI*"
		col1="Transfused <=48 hrs before NEC|#Patients (Incidence%)"
		pv="p value"
		;
run;
ODS ESCAPECHAR="^";
ODS rtf TEXT="^S={LEFTMARGIN=0.5in RIGHTMARGIN=0.5in font_size=10pt}
^n The infections are prior to NEC diagnosis date, and the hospital days are counted until NEC diagnosis date.

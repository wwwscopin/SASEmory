...
data tmp;
    merge hb cmv.comp_pat(in=comp);by id;
    if comp;
run;
...
data hwl hwl_base;
	merge hwl(in=hwl) tx(in=trans keep=id dt num idx) anemia(keep=id in=ane) anemia8(keep=id in=A); by id;
...
data tab1;
	...
	set anemia(where=(item<=3)) tab(where=(item<=10) in=A) anemia(where=(item=4) in=B) hb8(in=C) 

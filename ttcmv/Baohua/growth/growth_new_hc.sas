...
proc mixed data=hwl /*method=ml*/ ORDER=internal;
        ...
        model headcircum=tx day day*tx/s chisq;
    ...
    estimate "slope"  day 7  day*tx 7 0 0/e;
        estimate "slope"  day 7  day*tx 0 7 0/e;
            estimate "slope"  day 7  day*tx 0 0 7/e;
                 estimate "slope"  day*tx 7 0 -7/e;
...
run;
/*
data cmv.hc_slope(where=(effect='day'));
    set ind_slope;
run;
proc print;run;
*/
...
data _null_;
    set estimate_wt;
   	if _n_=28 then call symputx("s0", put(estimate,4.2)||"("||compress(put(stderr,4.2))||")");
	if _n_=29 then call symputx("s1", put(estimate,4.2)||"("||compress(put(stderr,4.2))||")");	   	
	if _n_=30 then call symputx("s2", put(estimate,4.2)||"("||compress(put(stderr,4.2))||")");
	if _n_=31 then call symputx("pv", put(probt,7.3));
run;
...
axis2 	label=(h=2.5 a=90 "Head Circumferenc(cm)") value=(h=2) order= (20 to 35 by 1) offset=(.25 in, .25 in) minor=(number=1);
 
title1 	height=3 "All LBWIs Head Circumference vs Age";
...
value = (h=2 "Without pRBC Transfusion, Slope(SE)=&s0 cm/week" "Before 1st pRBC Transfusion, Slope(SE)=&s1 cm/week" 
"After 1st pRBC Transfusion, Slope(SE)=&s2 cm/week") offset=(0.2in, -0.4 in) frame;
...
proc gplot data= wt gout=gcx.graphs;
	...
	note h=1.25 m=(4pct, 7 pct) "(#After tx)" ;

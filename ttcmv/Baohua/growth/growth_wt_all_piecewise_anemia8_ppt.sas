...
data _null_;
    set avg;
    if anemic8=0 then call symput("avg0", put(Weight_Mean,5.0));
        if anemic8=1 then call symput("avg1", put(Weight_Mean,5.0));
run;
...
data _null_;
	set tab;
	if anemic8=0 then call symput("no", compress(frequency));
	if anemic8=1 then call symput("yes",compress(frequency));
run;
...
data num_wt;
	set num_wt;
	if anemic8=. or daytx=. then delete;
run;
...
proc mixed /*method=ml*/ data=hwl_tx covtest;
	class id anemic8;
	model weight=d1 d2 anemic8 anemic8*d2/s;
	random int d1 d2/type=un subject=id;
	
	estimate "No, Day-40"  int 1 d1 -40 anemic8 1 0 ;  
	estimate "No, Day-28"  int 1 d1 -28 anemic8 1 0 ;  
	estimate "No, Day-21"  int 1 d1 -21 anemic8 1 0 ; 
	estimate "No, Day-14"  int 1 d1 -14 anemic8 1 0 ;
	estimate "No, Day-7"   int 1 d1 -7  anemic8 1 0 ; 
	estimate "No, Day-4"   int 1 d1 -4  anemic8 1 0 ;   	 
	estimate "No, Day-1"   int 1 d1 -1  anemic8 1 0 ;  
 	estimate "No, Day0"    int 1 d1 0   anemic8 1 0 ;

	estimate "No, Day1"   int 1 d2 1     anemic8 1 0 anemic8*d2    1 0;  
	estimate "No, Day4"   int 1 d2 4     anemic8 1 0 anemic8*d2    4 0;  
	estimate "No, Day7"   int 1 d2 7     anemic8 1 0 anemic8*d2    7 0; 
	estimate "No, Day14"  int 1 d2 14    anemic8 1 0 anemic8*d2   14 0;  
	estimate "No, Day21"  int 1 d2 21    anemic8 1 0 anemic8*d2   21 0;  
	estimate "No, Day28"  int 1 d2 28    anemic8 1 0 anemic8*d2   28 0;  
	estimate "No, Day40"  int 1 d2 40    anemic8 1 0 anemic8*d2   40 0;  	
 	 	 	
 	estimate "Yes, Day-40"  int 1 d1 -40 anemic8 0 1 ;  
	estimate "Yes, Day-28"  int 1 d1 -28 anemic8 0 1 ;  
	estimate "Yes, Day-21"  int 1 d1 -21 anemic8 0 1 ; 
	estimate "Yes, Day-14"  int 1 d1 -14 anemic8 0 1 ;
	estimate "Yes, Day-7"   int 1 d1 -7  anemic8 0 1 ; 
	estimate "Yes, Day-4"   int 1 d1 -4  anemic8 0 1 ;   	 
	estimate "Yes, Day-1"   int 1 d1 -1  anemic8 0 1 ;  
 	estimate "Yes, Day0"    int 1 d1 0   anemic8 0 1 ;
 	
	estimate "Yes, Day1"   int 1 d2 1     anemic8 0 1 anemic8*d2   0 1;  
	estimate "Yes, Day4"   int 1 d2 4     anemic8 0 1 anemic8*d2   0 4;  
	estimate "Yes, Day7"   int 1 d2 7     anemic8 0 1 anemic8*d2   0 7; 
	estimate "Yes, Day14"  int 1 d2 14    anemic8 0 1 anemic8*d2   0 14;  
	estimate "Yes, Day21"  int 1 d2 21    anemic8 0 1 anemic8*d2   0 21;  
	estimate "Yes, Day28"  int 1 d2 28    anemic8 0 1 anemic8*d2   0 28;  
	estimate "Yes, Day40"  int 1 d2 40    anemic8 0 1 anemic8*d2   0 40/cl;  
	
	estimate "post1" d2 1 anemic8*d2 1 0;
	estimate "post2" d2 1 anemic8*d2 0 1;
	
		estimate "pv" anemic8*d2 1 -1;
...
axis1 	label=(h=2.5 "Pre and Post 1st pRBC Transfusion (days)" ) split="*"	value=(h=1.25) 
c=blue order= (-41 to 41 by 1) minor=none offset=(0 in, 0 in) origin=(,15)pct;


axis2 	label=(h=2.5 a=90 f=swissb "Weight(g)") value=(h=2) order= (400 to 2400 by 100) offset=(.25 in, .25 in) minor=(number=1) c=white;
 
title1 	height=3 "All LBWIs Weight vs Pre and Post 1st pRBC Transfusion";
title2 	height=2.5 "n(With Anemia(Hb<=8))=&yes, n(Without Anemia(Hb>8))=&no";
...
value = (h=2 c=black "Without Anemia, Slope(SE)=&s1 g/day" "With Anemia, Slope(SE)=&s2 g/day") offset=(-2.5in, -0.4 in) frame cframe=white cborder=white;
...
proc gplot data= wt gout=wbh.graphs;
	...
	*note h=1.25 m=(0pct, 7.5 pct) "(#With Anemia)" ;
	...
	format estimate0 estimate1 4.0 ;
run;


ods pdf file = "growth_anemia8.pdf";

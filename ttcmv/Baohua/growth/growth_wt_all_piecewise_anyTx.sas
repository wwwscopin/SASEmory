...
value item 0="--"
           ...
           7="Caffeine used before 1st transfusion"
           ;
...
goptions reset=all;
ods pdf file = "growth_all_joint_anyTx.pdf";
...
data _null_;
    ...
    %if &var=anemic %then %do; if &var=0 then call symput("na0", compress(nobs)); if &var=1 then call symput("na1", compress(nobs));%end;
    %if &var=snapg  %then %do; if &var=0 then call symput("ns0", compress(nobs)); if &var=1 then call symput("ns1", compress(nobs));%end;
...
data est;
    ...
    if label='int1'  then effect="Intercept --Male (n=&nmale)"; 
    if label='int2'  then effect="Intercept --Female (n=&nfemale)"; 
    ...
    %if &var=race %then %do; 
    if label='int1'  then effect="Intercept --Black (n=&nb)"; 
    if label='int2'  then effect="Intercept --White (n=&nw)"; 
    ...
    %if &var=center %then %do; 
    if label='int1'  then effect="Intercept --Midtown (n=&nm)"; 
    if label='int2'  then effect="Intercept --Grady (n=&ng)"; 
    if label='int3'  then effect="Intercept --Northside (n=&nn)"; 
    ...
    if label='int1'  then effect="Intercept --Not Anemic (n=&na0)"; 
    if label='int2'  then effect="Intercept --Anemic (n=&na1)"; 
    ...
    if label='int1'  then effect="Intercept --SNAP score <=Median(&median) (n=&ns0)"; 
    if label='int2'  then effect="Intercept --SNAP score >Median(&median) (n=&ns1)"; 
    ...
    %if &var=feed %then %do; 
    if label='int1'  then effect="Intercept --No (n=&nf0)"; 
    if label='int2'  then effect="Intercept --Yes (n=&nf1)"; 
    ...
    if label='int1'  then effect="Intercept --No (n=&nc0)"; 
    if label='int2'  then effect="Intercept --Yes (n=&nc1)"; 
    if label='post1' then do; effect="Post- Transfusion Slope --No";probt=&pv;end;
    if label='post2' then do; effect="Post- Transfusion Slope --Yes"; probt=. ;end;
    %end;
...
ods rtf file="Interaction_anyTx.rtf" style=journal bodytitle;

...
data hb;
	set cmv.plate_015(in=A rename=(dfseq=day)) rbc(keep=id hbdate Hb); by id;
...
proc sort data=hwl_tx; by anemic8 id hbdate; run;


proc sort data=hwl_tx nodupkey out=at; by anemic8 tx id;run;
proc freq data=at; 
table anemic8*tx;
...
data _null_;
    set wbh;
    if anemic8=1 and tx=0 then call symput("a0", compress(frequency));
        if anemic8=1 and tx=1 then call symput("a1", compress(frequency));
            if anemic8=0 and tx=0 then call symput("na0", compress(frequency));
                if anemic8=0 and tx=1 then call symput("na1", compress(frequency));
run;

data hb_anemia;
    set hwl_tx;
    if 0<hb<=8;
    keep anemic8 tx id dt tx death  hbdate hb;
run;

proc sort nodupkey; by id hbdate hb; run;

ods rtf file="anemia8_listing.rtf" style=journal bodytitle;
proc print data=hb_anemia noobs label;
title "Anemia Status Listing";
by id;
id id;
var hbdate hb/style=[cellwidth=0.8in];
var anemic8 dt/style=[cellwidth=1in];
var tx /style=[cellwidth=1.6in];
label dt="Date of 1st pRBC Tx"
      tx="Pre or Post 1st pRBC Tx"
      Hb="Hemoglobin"
      anemic8="Anemia Status (Hb<=8 g/dL)"
      ;
format  tx ttx. anemic8 anemic.; 
run;

ODS ESCAPECHAR="^";
ODS rtf TEXT="^S={LEFTMARGIN=1.2in RIGHTMARGIN=0.9in font_size=10 pt}
^n 1. For anemic group, no LBWIs died after the 1st pRBC transfusion, 3 LBWIs had anemia again after the 1st pRBC transfusion.
^n 2. For not anemic group, there are 10 LBWIs died after the 1st pRBC transfusion, 10 LBWIs experienced anemia after the 1st pRBC transfusion.
";
ods rtf close;
...
%do %while (&x <2);
	...
	%if &x = 1  %then %do; %let var=anemic8;	%end;
		%if &x = 2  %then %do; %let var=anemic;	%end;
	
	%let variable =Hb;  %let description =  'Hemoglobin (g/dL)';  %let order=(6 to 20 by 1);	       
...
* get 'n' at each day;
	...
	* populate 'n' annotation variables ;
    		data _null_;
    			set sizes_&data;
     			if anemic8=0 and daytx=-40 then call symput( "n40",  compress(_freq_));
     			if anemic8=1 and daytx=-40 then call symput( "y40",  compress(_freq_));
     			if anemic8=0 and daytx=-28 then call symput( "n28",  compress(_freq_));
     			if anemic8=1 and daytx=-28 then call symput( "y28",  compress(_freq_));
     			if anemic8=0 and daytx=-21 then call symput( "n21",  compress(_freq_));
     			if anemic8=1 and daytx=-21 then call symput( "y21",  compress(_freq_));
     			if anemic8=0 and daytx=-14 then call symput( "n14",  compress(_freq_));
     			if anemic8=1 and daytx=-14 then call symput( "y14",  compress(_freq_));
     			if anemic8=0 and daytx=-7  then call symput( "n7",   compress(_freq_));
     			if anemic8=1 and daytx=-7  then call symput( "y7",   compress(_freq_));
     			if anemic8=0 and daytx=-4  then call symput( "n4",   compress(_freq_));
     			if anemic8=1 and daytx=-4  then call symput( "y4",   compress(_freq_));
     			if anemic8=0 and daytx=0   then call symput( "n0",   compress(_freq_));
     			if anemic8=1 and daytx=0   then call symput( "y0",   compress(_freq_));
     	   run; 
...
symbol1 i=j ci=blue value=circle h=0.5 w=1 repeat=100; 
	...
	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Anemic (Hb<=8 g/dL) (n=&a0)";  
		note h=2 m=(2pct, 8.5 pct) "Days:" ;
		where anemic8=1 and day<=0; 
		plot &variable*day=id/ overlay haxis = axis1 vaxis = axis2  nolegend vref=8 lvref=20 /*href=0 lhref=20*/; 
		format &variable 5.0;
	run;
	
	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Anemic (Hb<=8 g/dL) (n=&a1)";  
		note h=2 m=(2pct, 8.5 pct) "Days:" ;
		where anemic8=1 and day>0; 
		plot &variable*day=id/ overlay haxis = axis3 vaxis = axis2  nolegend vref=8 lvref=20 /*href=0 lhref=20*/; 
		format &variable 5.0;
	run;

	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Not Anemic (Hb>8 g/dL) (n=&na0)";  
		note h=2 m=(2pct, 8.5 pct) "Days:" ;
		where anemic8=0 and day<=0; 
		plot &variable*day=id/ overlay haxis = axis1 vaxis = axis2  nolegend vref=8 lvref=20 /*href=0 lhref=20*/; 
		format &variable 5.0;
	run;
	
	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Not Anemic (Hb>8 g/dL)  (n=&na1)";  
		note h=2 m=(2pct, 8.5 pct) "Days:" ;
		where anemic8=0 and day>=0; 
		plot &variable*day=id/ overlay haxis = axis3 vaxis = axis2  nolegend vref=8 lvref=20 /*href=0 lhref=20*/; 
		format &variable 5.0;
...
ods pdf file = "anemia8.pdf" startpage=no;

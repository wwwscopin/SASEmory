...
proc sort nodupkey out=ttt; by id;run;
proc print;
where nec=1;
var id nec;
run;
proc  freq; tables nec;run;
...
data anno0A;
	...
	function='label'; x=day; y=12; size=1.25; output;
	text=left(put(day,dt.));	output;
	function='label'; x=day; y=10; size=1; output;
	text=left(put(day,dta.));	output;
	function='label'; x=day; y=8.0; size=1; output;
	text=left(put(day,dtb.));	output;
run;
...
data anno1A;
	...
	function='label'; x=day; y=12; size=1.25; output;
	text=left(put(day,dt.));	output;
	function='label'; x=day; y=10; size=0.75; output;
	text=left(put(day,dta.));	output;
	function='label'; x=day; y=8.0; size=0.75; output;
	text=left(put(day,dtb.));	output;
run;
...
goptions reset=global  device=jpeg  rotate=landscape gunit=pct noborder cback=white
		colors = (black red) ftext="Times New Roman" FTITLE="Times New Roman" FBY="Times";
...
*/	
	...
	axis1 	label=(h=3 " " ) split="*"	
	   value=(h=1.2) major=(c=black) origin=(,15)pct order= (-61 to 1 by 1) minor=none offset=(0 in, 0 in);
	   
	axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") value=(h=2.5) c=black order= (8 to 16 by 1) offset=(.25 in, .25 in) minor=(number=1); 
	...
	ods pdf file = "hb_nec_slope0.pdf" style=journal;
			proc gplot data= nec_estimate;
				...
				note h=1.25 m=(3pct, 12 pct) "Day :" ;
               	note h=1.25 m=(3pct, 10.5 pct) "(#No NEC)" ;
               	note h=1.25 m=(3pct, 9.0 pct) "(#NEC)" ;
			run;	

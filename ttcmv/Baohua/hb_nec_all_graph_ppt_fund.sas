...
proc sort; by day;run;
		...
		DATA anno0; 
			...
			* draw top and bottoms of bars;
			TIPS:
			  X=day-0.2; FUNCTION='DRAW'; when = 'A'; line=1; size=2; OUTPUT;
			  X=day+0.2; FUNCTION='DRAW'; when = 'A'; line=1; size=2; OUTPUT;
...
data anno0A;
	...
	set nec_estimate;
	function='move'; x=day; y=16; output;
	function='draw'; x=day; y=14.5; output;
	function='label'; x=day; y=12; size=2.5; output;
...
run;
		...
		DATA anno1; 
			...
			* draw top and bottoms of bars;
			TIPS:
			  X=day1-0.2; FUNCTION='DRAW'; when = 'A'; line=1; size=2; OUTPUT;
			  X=day1+0.2; FUNCTION='DRAW'; when = 'A'; line=1; size=2; OUTPUT;
...
goptions reset=all rotate=landscape gunit=pct border CBACK=blue ctext=white ctitle=white colors=(orange green red)
	 ftitle=centb ftext=centb htitle=5 htext=4;

	symbol1 interpol=j mode=exclude value=circle co=blue cv=blue height=4 bwidth=3 width=2;
	symbol2 i=j ci=red value=dot co=red cv=red h=4 w=2;
...
*/		   
	axis1 	label=(h=3 " " ) split="*"	
	   value=(h=2.5) major=(c=blue) origin=(,16)pct order= (-61 to 1 by 1) minor=none offset=(0in, 0 in);
	   
	axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") value=(h=4) c=white order= (8 to 16 by 1) offset=(.25 in, .25 in) minor=(number=1); 


	legend across = 1 position=(top right inside) mode = reserve shape = symbol(3,2) label=NONE 
    value = ( h=3 c=black "Non-NEC" "NEC") offset=(-0.2in, -0.4 in) frame cframe=white cborder=black;
	...
	ods pdf file = "hb_nec_slope0.pdf" style=journal;
			proc gplot data= nec_estimate;
				...
				*note h=3 m=(20pct, 2 pct) "Age Before 1st NEC Diagnosis or Age of non-NEC LBWIs" ;
				note h=2.5 m=(12pct, 2 pct) "Days Before 1st NEC Diagnosis or Days Before the Last Measured Hemoglobin(non-NEC)" ;

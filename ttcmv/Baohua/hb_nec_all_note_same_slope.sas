...
proc mixed data=nec covtest;
	...
	estimate "Compare intercept" nec 1 -1;

	estimate "No NEC, intercept" int 1 nec 1 0/cl;
	estimate "No NEC, Day1"  int 1 nec 1 0 day 1 ;
	estimate "No NEC, Day4"  int 1 nec 1 0 day 4  ;
	estimate "No NEC, Day7"  int 1 nec 1 0 day 7  ;
	estimate "No NEC, Day14" int 1 nec 1 0 day 14 ;
	estimate "No NEC, Day21" int 1 nec 1 0 day 21 ;
	estimate "No NEC, Day28" int 1 nec 1 0 day 28 ;
	estimate "No NEC, Day40" int 1 nec 1 0 day 40 ;
	estimate "No NEC, Day60" int 1 nec 1 0 day 60 /e;

	estimate "Yes NEC, intercept" int 1 nec 0 1/cl;
	estimate "    NEC, Day1"  int 1 nec 0 1 day 1    ;
	estimate "    NEC, Day4"  int 1 nec 0 1 day 4  ;
	estimate "    NEC, Day7"  int 1 nec 0 1 day 7  ;
	estimate "    NEC, Day14" int 1 nec 0 1 day 14 ;
	estimate "    NEC, Day21" int 1 nec 0 1 day 21 ;
	estimate "    NEC, Day28" int 1 nec 0 1 day 28 ;
	estimate "    NEC, Day40" int 1 nec 0 1 day 40 ;
	estimate "    NEC, Day60" int 1 nec 0 1 day 60 /e;
	ods output Mixed.Estimates=nec_estimate;
run;
*ods trace off;
proc print data=nec_estimate(obs=3);run;

data nec_slope;
	set nec_estimate(firstobs=1 obs=1);
    ...
    *if _n_=2 then call symput("sp1", compress(put(estimate,7.3)||"&pm"||put(stderr,7.3)));
    if _n_=1 then  if probt<0.001 then call symput("p", '<0.001'); else call symput("p", put(Probt,4.2));
run;

data nec_estimate;
	set nec_estimate(firstobs=2);
...
run;
	...
	title2 	height=3 "Test of equal intercepts, p=&p";

	ods pdf file = "hb_nec_same_slope_note.pdf" style=journal;

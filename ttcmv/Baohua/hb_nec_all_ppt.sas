...
goptions reset=all rotate=landscape gunit=pct noborder CBACK=blue ctext=white ctitle=white colors=(orange green red)
	...
	axis1 	label2=(h=3 "" ) split="*"	
	   value=(h=1.25) major=(c=blue) origin=(,15)pct order= (-1 to 61 by 1) minor=none offset=(-0.5 in, 0 in) ;

	axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") value=(h=2.5) c=white order= (0 to 16 by 1) offset=(.25 in, .25 in) minor=(number=1); 
	...
	ods pdf file = "hb_nec_slope.pdf" style=journal;
			proc gplot data= nec_estimate;
				...
				*note h=1.25 m=(4pct, 9.0 pct) "Day :" ;
               	*note h=1.25 m=(4pct, 7.5 pct) "(#No NEC)" ;
               	*note h=1.25 m=(4pct, 6.0 pct) "(#NEC)" ;
			run;	

...
data nec;
	...
	cmv.plate_031(keep=id DateTransfusion in=in_tx)
...
data nec_tmp;
    set nec;
    where nec=1;
    where id in(2007621,2013511,3033611);
run;
proc print data=nec_tmp;
var id nec DateTransfusion ndate hbdate hb;
label ndate="NEC Date";
run;
...
goptions reset=global rotate=landscape gunit=pct noborder cback=white
	...
	symbol1 i=j ci=blue value=circle co=blue cv=blue h=3 w=2;
	symbol2 i=j ci=red value=dot co=red cv=red h=3 w=2;
	symbol3 i=j ci=black value=square cv=black  h=3 w=2;

	   
	axis1 	label=(h=3 " " ) split="*"	
	   value=(h=1.25) major=(c=black) origin=(,15)pct order= (-1 to 15 by 1) minor=none offset=(0 in, 0 in);
	   
	axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") value=(h=2.5) c=black order= (10 to 17 by 1) offset=(.25 in, .25 in) minor=(number=1); 
	...
	ods pdf file = "hb_nec_slope_tx3.pdf" style=journal;

...
data rbc;
	...
	if first.id and hb^=. then day=0; else day=.;
run;

data hb;
	set hb rbc(keep=id DateTransfusion hbdate Hb day); by id;
run;

proc sql;
	create table trans as
	select a.id, a.hbdate, a.hb, a.day, b.DateTransfusion
	from hb as a, rbc as b
	where hb.id=rbc.id
;
		...
		DATA anno_mixed_&data; 
			...
			* AFTER that, draw bars ('A' option ensure properly layering over the greay bars!);
			X=day; 	y=estimate; FUNCTION='MOVE'; when = 'A';  OUTPUT; * start at mean ;
					Y=lower; 	FUNCTION='DRAW'; when = 'A'; line=3; size=1; color=&mycolor;  OUTPUT; * draw down;
		
			LINK TIPS; * make bar;

			Y=upper;	FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor;  OUTPUT; * draw up; 
		
			LINK TIPS; * make bar;
		
			* draw top and bottoms of bars;
			TIPS:
			    if abs(day)<=4 then do;
    			  X=day-.1; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	       		  X=day+.1; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	            end;
	            if abs(day)=7 then do;
	              X=day-.2; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	       		  X=day+.2; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	            end;
	            
	           if 7<abs(day)<=28 then do;
	              X=day-.4; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	       		  X=day+.4; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	            end;
	            
	            if abs(day)>28 then do;
	              X=day-1; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	       		  X=day+1; FUNCTION='DRAW'; when = 'A'; line=1; size=3; color=&mycolor; OUTPUT;
	            end;
...
%anno(data=trans);
         ...
         goptions reset=all  device=jpeg   gunit=pct noborder cback=white 
         					...
         					symbol1 interpol=none mode=exclude value=none co=black cv=black height=0.6 bwidth=3 width=0.8;
...
         					symbol2 i=none ci=black value=circle h=1 w=1;
         					...
         					/* Set up Symbol for Data Points */
         					symbol3 i=j ci=red value=dot h=1 w=1;
         ...
         %macro make_plots(data=trans); 
          		...
          		%if &data = trans %then %do; 	%let variable =hb; %let description = "Hemoglobin(g/dL)- With pRBC Transfusion (n=&n_yes)";
         							%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/;%end;
          		%if &data = notrans %then %do; 	%let variable =hb; %let description = "Hemoglobin(g/dL)- Without pRBC Transfusion (n=&n_no)";
         							%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/;%end;
     ...
     -33=" " -32=" " -31=" " -30=" " -29=" " -28="-28*(&n_n28)" -27=" " -26=" " -25=" "
		...
		-12=" " -11=" " -10=" "   -9=" "    -8=" "   -7="-7*(&n_n7)" -6=" " - 5=" "  -4="-4*(&n_n4)"  -3=" "   -2=" "   -1=" "   
         		...
         		title3 h=3.5 justify=center Longitudinal models (means and 95% CI);
					...
					axis1 	label=(h=3 'Days Before and After 1st pRBC Transfusion' ) value=( h=2.0) order= (-60 to -40 by 20 -28 to -7 by 7 -4 to 4 by 4 7 to 28 by 7 40 to 80 by 20) split="*"  minor=none offset=(0 in, 0 in);
         		axis2 	label=(h=3 a=90 "Hemoglobin(g/dL)") order=(0 to 25 by 5) value=(h=2) &scale ;
         ...
         * clear graph catalog ;
         proc greplay igout= wbh.graphs  nofs; delete _ALL_; run;
			...
			*%make_plots(data=notrans); run;
        	...
        	ods ps file = "hb_trans_mixed.ps";
        	ods pdf file = "hb_trans_mixed.pdf" startpage=no;
...
/*
	...
	title1	height=3 'Hemoglobin longitudinal model-based means';

...
data cmv;
	set cmv.neeta_cmv_paper; by id;

	keep id lbwi_nat_pos;
	rename lbwi_nat_pos=np;
run;

proc sort data=cmv nodupkey; by id;run;
data _null_;
	set cmv;
...
/*
data hop;
	merge cmv.endofstudy(in=tmp) cmv.LBWI_Demo nec(in=A); by id;
	
	day=StudyLeftDate-lbwidob;
    if day=. then day=today()-dob;
    if A then nec=1; else nec=0;
...
data infect;
	merge tmp cmv(in=A) all_pat(in=B) cmv.plate_005(keep=id LBWIDOB); by id;
	...
	/*
    if np and culturedate>=npdate then do; gbsi=0; sub_mrsa=0; sub_mssa=0; mrsa=0; mssa=0; gcard=0; ggi=0; gcns=0; glr=0; gss=0; gut=0; gother=0; gi=0; gnbsi=0;end;
    */
run;
...
data cmv_nat;
    set infect;
    where  np and (SiteBlood or SiteGI or SiteLowerResp or SiteUT);
run;
proc sort nodupkey; by id SiteBlood SiteGI SiteLowerResp SiteUT; run;
proc sort; by id age;run;


proc freq data=infect; 
tables np*(gbsi sub_mrsa sub_mssa mrsa mssa gcard ggi gcns glr gss gut gother gi gnbsi);
run;
...
%pt(infect,cmv_infect,np,&varlist);

options orientation=landscape;
ods rtf file="infect_cmv.rtf" style=journal bodytitle startpage=no;
proc print data=cmv_infect noobs label split="|"; 
title1 "Incident infection rates in the &nt LBWIs with SERO Positive"; /*no longer hospitalized"*/;
*title2 "&total_hosp_days_nec0(No NEC) and &total_hosp_days_nec1(NEC) patient hospital days on study";
var item/style(data)=[just=left cellwidth=2.25in] style(header)=[just=left];
var col0 col1/style(data)=[just=center cellwidth=1.5in] style(header)=[just=center];
var pv/style(data)=[just=center cellwidth=0.75in] style(header)=[just=center];
label   item="."
		col0="CMV NAT not detected|#Patients (Incidence%)"
		col1="CMV NAT positive|#Patients (Incidence%)"
		pv="p value"
		;
run;
/*
...
^n The infections for NEC group are prior to CMV positive diagnosed date, and the hospital days are counted until CMV positive diagnosed date.
^n *95% confidence intervals are calculated using an exact method based on the Possion distribution.";
*/

proc print data=cmv_nat noobs label split="|"; 
title1 "ID listing for Infection";
var id age/style(data)=[just=center cellwidth=0.75in] style(header)=[just=center];
var SiteBlood SiteGI SiteLowerResp SiteUT/style(data)=[just=center cellwidth=0.75in] style(header)=[just=center];
run;
ods rtf close;

...
proc sort nodupkey; by id necdate; run;
data nec;
...
data hop;
	merge cmv.endofstudy cmv.LBWI_Demo nec(in=A) all_pat(in=tmp); by id;
    if tmp;
	day=StudyLeftDate-lbwidob;
	if day=. then day=today()-lbwidob;
    if A then do; day=necdate-lbwidob; nec=1;end;
    else nec=0;
    
    if day=. then delete;
    
   	keep id lbwidob studyleftdate day nec necdate;
run;
...
data infect;
	merge tmp nec(in=A) all_pat(in=B); by id;
	...
	if SiteBlood=0 and gnbsi=1 then gnbsi=1; else gnbsi=0;
	if B;	
	if A and culturedate>=necdate then do;
    	gbsi=0; sub_mrsa=0; sub_mssa=0; mrsa=0; mssa=0; gcard=0; ggi=0; gcns=0; glr=0; gss=0; gut=0; gother=0; gi=0; gnbsi=0;
    end;
run;
...
proc print data=tmp&i;run;
proc print data=p&i;run;
...
The infections for NEC group are prior to NEC diagnosed date, and the hospital days are counted until NEC diagnosed date.

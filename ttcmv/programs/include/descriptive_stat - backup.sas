...
%macro descriptive_stat (	data_in=, where=, data_out=, var= , type=, non_param= , first_var= , last_var=, by_cohort= , by_DOL = , 
				spaces= , dec_places= , custom_label=, print_rtf= , file=, title=  );
	...
	* Decide if we run the loop once or three times (just overall, or overall and by cohorts );
	...
	%do %while (&x <= &stop_num);
		...
		* for CONTINUOUS or BINARY types;
		%if ("&type" = "bin") | ("&type" = "contin") %then %do;
			...
			** clean up for display;
			data &var;
				...
				* if continuous ;
						...
						disp = compress(put(median, 12.&dec_places)) || " (" || compress(put(q1, 12.&dec_places)) || ", " || compress(put(q3, 12.&dec_places)) || "), " || compress(put(n, 12.0));
					end;
					else if ("&type" = "cont") then do;
						disp = compress(put(mean, 12.&dec_places)) || " (" || compress(put(stddev, 12.&dec_places)) || "), " || compress(put(n, 12.0));
					end;
			...
			* clean up for display;
		        data &var; set &var;
		            ...
		            if (&var ~= .) then do;
		                ...
		                row = "- " || putn(&var , varfmt(open("&data_in"), varnum(open("&data_in"), "&var")))  ; * use assigned format - MUST USE PUTN rather than PUT, since computed at runtime! ;
		...
		* if its the last variable, process the table for printing! ;
		%if (&last_var = 1) %then %do;
			%if (&x = 1) %then %let cohort = Overall;
			%else %if (&x = 2) %then %let cohort = CMV_pos;
			%else %if (&x = 3) %then %let cohort = CMV_neg;
			...
			data table_&cohort;
				...
				* add blank line at end of category;
				...
				label 		variable= '00'x
						row ='00'x
						disp_visit = '00'x
						%if &x = 1 %then %do; disp = "Overall*^S={font_weight = bold}total (%)" %end;					
		...
		** Post-process - MERGE all 3 tables if doing by cohort ** ;
		%if (&by_cohort = 0) %then %do;
			data &data_out;
				...
				label disp_overall = "^S={font_weight = bold}total (%)";
			run;
		%end;
		...
		%else %if (&by_cohort = 1) %then %do;
			...
			* sort each table then merge!;
			proc sort data = table_Overall; by variable row disp_visit; run;
	...
	*** PRINTING MODE! ***; 
	%if ("&print_rtf" = "1") %then %do;
		%if (&by_cohort = 0) %then %do;
			ods rtf file = &file  style=journal toc_data startpage = yes;
				ods noproctitle proclabel &title;
				title1 &title; 
				proc print data = &data_out label noobs split = "*" style(header) = [just=center];
					id  row;
					by  row notsorted;

					var disp_overall /style(data) = [just=center];;
				run;

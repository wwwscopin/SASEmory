...
select a.* ,b.lbwi_nat
 from cmv_id_final  as a left join
(select distinct id,moc_id, max(NATTestresult)as lbwi_nat   from lbwi_blood_nat where NATTestresult in (2,3,4) ) as b
on a.moc_id=b.moc_id
order by a.id, a.seq ;
...
if rapid_test In (2) then 
milk_culture_stat= "d" || compress(dol_milk_culture) ||  " (Pos)";
else if rapid_test In (1) then
...
from cmv_id_final as a left join
( select distinct id , UrineTestResult from cmv.plate_211_long where UrineTestResult In (2,3,4))  as b 
on a.id=b.id;
...
select a.id,DateOfBirth, b.DonorUnitId,source,dfseq,DateTransfusion,source
from lbwi_tx_7 as a left join
(
...
from  cmv.Lbwi_Blood_NAT_Result where NATTestResult in (2,3,4)
union
...
union
select count(distinct(id)) as count ,"Breast Milk NAT Positive" as variable , 4 as group
from bm_nat_long where BM_nat_result in (2,3,4)

union
select count(distinct(id)) as count ,"MOC NAT Positive" as variable , 5 as group
...
from cmv.plate_031 as a inner join cmv.valid_ids as b on a.id=b.id)

union
(

select count(distinct(a.id)) as count ,"LBWI Urine NAT Positive/Indeterminate" as variable , 8 as group
from cmv.plate_211_long as a inner join cmv.valid_ids as b on a.id=b.id where a.UrineTestResult  in (2,3,4))
...
select count(distinct(a.id)) as count ,"LBWI NAT Positive" as variable, 1 as group
from  cmv.Lbwi_Blood_NAT_Result as a inner join completedstudylist as b on a.id=b.id where NATTestResult in (2,3,4)
union
...
union
select count(distinct(a.id)) as count ,"Breast Milk NAT Positive" as variable , 4 as group
from bm_nat_long as a inner join completedstudylist as b on a.id=b.id where BM_nat_result in (2,3,4)

union
select count(distinct(a.id)) as count ,"MOC NAT Positive" as variable , 5 as group
...
select count(distinct(moc_id)) format=3.0 into :moc_all from cmv.valid_ids;
select count(distinct(id)) format=3.0 into :lbwi_all from cmv.valid_ids;
...
define variable /  group order=data   center  style(column) = [just=center cellwidth=2in font_size=9pt]   " " ;
define stat/group  order=internal    center  style(column) = [just=center cellwidth=1.5in font_size=9pt]   " Statistics_(LBWI Up to date) " ;
define comp_stat/group  order=internal    center   style(column) = [just=center cellwidth=1.5in font_size=9pt]  " Statistics _(LBWI who Completed)" ;
define dummy/NOPRINT ;
run;

title1  justify = center "&cmv_survey_summary_title : CMV - surveillance if MOC/LBWI NAT positive or IgG Positive";
...
title1  justify = center "&cmv_survey_summary_title : CMV - surveillance resuts if LBWI / MOC CMV NAT positive or IgG Positive";
title2  justify = center " ( MOC=&moc LBWI=&lbwi)";
title3 "";
...
define death_stat /  group order=data   style(column) = [just=center cellwidth=1in]   " Death? " ;


define lbwi_blood_NAT_stat /   order=data   style(column) = [just=center cellwidth=.8in font_size=8pt]   " LBWI_Blood_NAT " ;

...
data ivh_mark0;
	set ivh0; by id td;
	if td<=7 then if LeftIVHGrade in(2,3,4) or RightIVHGrade in (2,3,4) then ivh=1; else ivh=0;
	if LeftIVHGrade in(2,3,4) or RightIVHGrade in (2,3,4) then img=1; else img=0;
run;

proc sort data=ivh_mark0(where=(ivh=1)) out=ivh_mark(keep=id ivh) nodupkey; by id; run;
proc sort data=ivh_mark0(where=(img=1) drop=ivh) out=img_mark(keep=id img imagedate rename=(img=ivh imagedate=idate)) nodupkey; by id;run;

proc print data=img_mark;run;

data tx_id;
	set cmv.plate_031(keep=id  DateTransfusion rbc_TxStartTime)
			cmv.plate_033(keep=id DateTransfusion plt_TxStartTime)
...
data lbwi;
	...
	if txx then tx=1; else tx=0;
	if ivh and idate>DateTransfusion the tx=1;
	if ivh and idate<=DateTransfusion the tx=0;

	day1=IVHDiagDate-LBWIDOB;
	...
	keep 
			...
			RightIVHGrade 	Indomethacin AntiConvulsant ROM Steroids Indomethacin  AntiConvulsant gr bw rt pov day1 idate anemic tx;		 
run;
...
proc print data=lbwi(where=(ivh=1));
var id day day1 IVHDiagDate idate LBWIDOB; 
run;
...
data _null_;
	set median_day;
	call symput("ivh_day", compress(put(day1_median,4.0)));
...
data lbwi;
	...
	format gga gga. gsnap gsnap. ghb ghb. gplt gplt. gratio gratio. pco pco. po gpo. center center. gender gender. race race. ivh ivh. wc wc. BirthResusOxygen BirthResusCPAP ROM Steroids Indomethacin  AntiConvulsant tx ny. tbc tbc. gr gr. pov pov.;
run;
...
%let varlist=center gender race wc gga gsnap ghb gplt tbc BirthResusOxygen BirthResusCPAP ROM steroids pco po pov fio gratio gr Indomethacin tx;
...
*/
	surv_err&t1=put((survival&t1)*100,4.1)||"&pm"||put(stderr&t1*100,4.1);
	surv_err&t2=put((survival&t2)*100,4.1)||"&pm"||put(stderr&t2*100,4.1);
	rename n=nt;
...
ods rtf file="ivh1.rtf" style=journal startpage=no bodytitle ;
...
*The median day for IVH diagnosis is &ivh_day; the median day for IVH event(IVH with grade>=2) is &img_day .";

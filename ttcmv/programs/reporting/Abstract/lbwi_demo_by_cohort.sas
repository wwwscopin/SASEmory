...
* Merge tables and headers;
	...
	proc sql; select count(*) into :cohort1 from lbwi_demo where cohort=1;
	proc sql; select count(*) into :cohort2 from lbwi_demo where cohort=2;
	...
	ods rtf file = "&output./april2011abstracts/lbwi_demo_summary_by_cohort.rtf"  style=journal toc_data startpage = no bodytitle;

...
proc sort data = moc_demo; by moc_id; run;

data completed_moc; set cmv.completedstudylist; moc_id = input(substr(put(id,7.0),1,5),5.0); keep moc_id; run;
proc sort data = completed_moc; by moc_id; run;
data completed_moc; set completed_moc; by moc_id; if first.moc_id; run;
data moc_demo; merge completed_moc (in=b) moc_demo (in=a); by moc_id; if a & b; run;
...
* HEADERS ;
	...
	data header2; length row $ 120; row = "^S={font_weight=bold font_size=2}" || "Risk Factors"; run;
...
* Merge tables and headers;
	...
	* print table ;
	%descriptive_stat(print_rtf = 1, data_out= moc_demo_summary, file= "&output./april2011abstracts/moc_demo_summary.rtf", title = "MOC Demographic Summary");

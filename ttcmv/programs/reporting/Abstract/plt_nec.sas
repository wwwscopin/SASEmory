...
data plt0;
	set cmv.plate_015 
		 cmv.plate_033(keep=id PlateletCount /*DFSEQ*/ DatePlateletCount rename=(DatePlateletCount=pltdate PlateletCount=platelet) in=B);
...
proc sort nodupkey; by id pltdate plt;run;
...
data nec;
	merge plt0 nec0 cmv.comp_pat(in=comp keep=id gender dob); by id;
	if comp;
	...
	*rename dfseq=day;
	...
	if pltdate>ndate and nec then plt=.;
...
run;
proc sort data=nec nodupkey; by id pltdate; run;

*ods trace on/label listing;
proc mixed method=ml data=nec covtest;
	class id nec;
	model plt=nec day nec*day/s;
...
run;
	...
	axis2 	label=(f=zapf h=2.5 a=90 "Platelet Count(1000/" f=greek "m" f=zapf "L)") value=(f=zapf h=2) order= (0 to 600 by 50) offset=(.25 in, .25 in) minor=(number=1); 
   ...
   value = (f=zapf h=3 "No NEC" "NEC") offset=(0, -0.4 in) frame;
	...
	title1 	height=3 f=zapf "Platelet Count vs Days (No NEC=&no_nec(&no obs), NEC=&yes_nec(&yes obs))";
	title2 	height=3 f=zapf "Test of equal slopes, p=&p";

	ods pdf file = "plt_nec_slope.pdf";
...
proc mixed method=ml data=nec covtest;
	class id nec;
	model plt=nec day*nec/noint s;
	random int day/type=un subject=id;
run;


proc genmod data=nec;
	class id nec;
	model tc=nec day nec*day/link=logit dist=binomial type3;

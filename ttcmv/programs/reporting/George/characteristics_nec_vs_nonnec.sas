...
*** get ever rbc txn'd, age of blood, irradiation days ;

	proc sort data = cmv.age_of_blood out = rbc; by id; run;
	data rbc; merge rbc nec_list (in=a); by id; if a then has_nec = 1; else has_nec = 0; run; 


	* NON-NEC PATIENTS ;
	data rbc_nonnec; set rbc; if has_nec = 0; run;

	* keep age of OLDEST blood given ;
	proc sort data = rbc_nonnec out = aob; by id age_of_blood; run;
	data aob; set aob; by id; if has_nec = 0; if last.id; 
		label age_of_blood = "Age of pRBC transfused (oldest, by patient)*"; 
		keep id age_of_blood; 
	run;

	* keep longest number of days after irradiation ;
	proc sort data = rbc_nonnec out = irr; by id days_irradiated; run;
	data irr; set irr; by id; if has_nec = 0; if last.id; 
		label days_irradiated = "Days pRBC transfused after irradiation (max, by patient)*"; 
		keep id days_irradiated; 
	run;

	data rbc_nonnec; merge aob irr; by id; run;


	data evertxn; merge nonnec_list (in=a) rbc_nonnec (in=b); by id; if a;
		if b then everrbctxn = 1; else everrbctxn = 0; keep id everrbctxn age_of_blood days_irradiated; 
		label everrbctxn = "Transfusion - no. (%)*";
	run;


	* NEC PATIENTS ;
	data rbc_nec; set rbc; if has_nec = 1; run;

	* keep age of OLDEST blood given ;
	proc sort data = rbc_nec out = aob; by id age_of_blood; run;
	data aob; set aob; by id; if last.id; label age_of_blood = "Age of pRBC transfused (oldest, by patient)*"; keep id age_of_blood; run;

	* keep longest number of days after irradiation ;
	proc sort data = rbc_nec out = irr; by id days_irradiated; run;
	data irr; set irr; by id; if last.id; label days_irradiated = "Days pRBC transfused after irradiation (max, by patient)*"; keep id days_irradiated; run;

	* sort by transfusion date, check if first transfusion given before NEC ;
	proc sort data = rbc_nec out = txnprior; by id datetransfusion; run;
	data txnprior; set txnprior; by id; if first.id;
		if necdate > datetransfusion then txnprior = 1; else txnprior = 0;
		label txnprior = "Transfusion - no. (%)*";
		keep id txnprior;
	run;

	data rbc_nec; merge txnprior (in=b) aob irr nec_list (in=a); by id; if a;
		if b then everrbctxn = 1; else everrbctxn = 0; 
		if txnprior = 0 then age_of_blood = .; 
		if txnprior = 0 then days_irradiated = .; 
	run;
	
*** Save these variables for survival analysis ; 
	data cmv.km_rbc_variables; set evertxn (in=a) rbc_nec (in=b); 
		if a then rbctxn = everrbctxn;
		if b then rbctxn = txnprior; 
		if b then has_nec = 1; if a then has_nec = 0;
		keep id rbctxn has_nec age_of_blood days_irradiated;
	run;
...
*** Given pRBC < 48 hours prior to NEC ;
	...
	data txn_prior; merge rbc (keep = id datetransfusion) nec (in=b keep = id necdate) cmv.completedstudylist (in=a); by id; if a; if b; 
		...
		if txn_prior_days = 0 | txn_prior_days = -1 | txn_prior_days = -2;
	run;
...
data characteristics1; set characteristics; by id; if first.id; if has_nec; run;
	...
	%descriptive_stat(data_in= rbc_nec, data_out= characteristics_table1c, var= txnprior, type= bin, first_var=1);
	%descriptive_stat(data_in= rbc_nec, data_out= characteristics_table1c, var= age_of_blood, type= cont, non_param=1);
	%descriptive_stat(data_in= rbc_nec, data_out= characteristics_table1c, var= days_irradiated, type= cont, non_param=1);
...
data characteristics2; set characteristics; by id; if first.id; if has_nec=0; run;
	...
	%descriptive_stat(data_in= evertxn, data_out= characteristics_table2c, var= everrbctxn, type= bin, first_var=1);
	%descriptive_stat(data_in= evertxn, data_out= characteristics_table2c, var= age_of_blood, type= cont, non_param=1);
	%descriptive_stat(data_in= evertxn, data_out= characteristics_table2c, var= days_irradiated, type= cont, non_param=1);

...
data pending;
	merge tx_id(in=tx keep=id donorunitid) donor1(in=donor keep=donorunitid); by donorunitID; 
	if tx and not donor;
run;
...
create table donor as 
	...
	where donor1.donorunitid=tx_id.donorunitid;
	;
...
proc univariate data=donor gout=wbh.graphs;
	*histogram day_issue day_irrad day_trans day_receive/cfill=orange ;
	...
	label day_irrad="Date unit issued to NICU - Date blood donated"
			 day_issue="Date unit irradiated at the Red Cross - Date blood donated" 

...
data x;
 ...
 data final;
   ...
   by id;
   array banemia(13);
   if banemia(1)=. then banemia(1)=0;
   do i=2 to 13;
       if banemia(i)=. then banemia(i)=banemia(i-1);
   end;
proc sort; by censor;
proc print;
...
ods pdf file = 'nec7a.pdf';
title Anemia (yes/no) on a Weekly Interval;
title2 If no Hb values on a given week use results from previous week;
proc phreg ;

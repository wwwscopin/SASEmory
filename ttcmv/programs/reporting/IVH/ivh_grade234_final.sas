...
data lbwi;
	...
	if tx_plt and IVHDiagDate>dt_plt then plt=1; else if tx_plt and IVHDiagDate<=dt_plt then plt=0; else plt=.;
	...
	keep 
			...
			sepsis indo	RightIVHGrade 	Indomethacin AntiConvulsant ROM Steroids Indomethacin  AntiConvulsant gr bw rt pov snap with_rbc  
			tx rbc plt	IsChorloConfirm HistoChloro Apgar5Min apg anemic8 anemic9 idx_rbc dt_rbc dt_plt nrbc hb_num plt_num;		 
run;
...
*/
  ...
  value idx
	...
	5="Platelet Count at Birth(per 10*1000/&mu L increase)"
	6="Hemoglobin at Birth('< Median(&md_hb)' vs '>= Median')"
...
title "Univariable Analysis of Factors Associated With IVH (Grade II, III, IV)";
...
*The median day for IVH event(IVH with grade>=2) is &ivh_day day.";

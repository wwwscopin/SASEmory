...
data ivh;
	merge plt0 ivh2 tx(in=tmp keep=id) cmv.comp_pat(in=comp keep=id gender dob); by id;
	if comp;
	day=pltdate-dob;
...
proc mixed method=ml data=ivh covtest;
	class id ivh;
	model platelet=ivh day ivh*day/s;
...
run;
	...
	axis1 	label=(h=2.5 "Days of Age" ) split="*"	value=(h=1.25)  order= (-1 to 61 by 1) minor=none offset=(0 in, 0 in);
   ...
   value = (h=3 "No IVH" "IVH") offset=(0, -0.4 in) frame;
	                
	                	                
	*title1 	height=3 "Platelet Count vs Age (No IVH=&no_ivh(&no obs), IVH=&yes_ivh(&yes obs))";
	title1 	height=3 "Platelet Count vs Age (No IVH=&no_ivh, IVH=&yes_ivh (with Platelet before IVH diagnosis))";
	
	title2 	height=3 "Test of equal intercepts, p=&pi, Test of equal slopes, p=&p";

	ods pdf file = "plt_ivh_slope.pdf" style=journal;
			proc gplot data= ivh_estimate;
				...
				format estimate0 estimate1 4.0 day dt.;
					note h=1.25 m=(4pct, 8.5 pct) "Day :" ;
                	note h=1.25 m=(4pct, 7 pct) "(#No IVH)" ;
                	note h=1.25 m=(4pct, 5.5 pct) "(#IVH)" ;
			run;

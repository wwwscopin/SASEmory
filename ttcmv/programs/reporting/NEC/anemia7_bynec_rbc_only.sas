...
data hb_nec;
    ...
    if 0<hb<=7 then anemia=1; else anemia=0;
run;
...
title1 h=3 justify=center Cumulative incidence of anemia (Hb<=7 g/dl) for pRBC transfused LBWIs (n= &m NEC and n=&n Control LBWIs).;
...
axis2 	label=(h=2.5 a=90 "Probability of Anemia (Hb<=7 g/dL)") order=(0 to 0.7 by 0.05) value=(h=2) ;

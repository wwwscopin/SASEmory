...
data hb_nec;
    ...
    if 0<hb<=9 then anemia=1; else anemia=0;
run;
...
title1 h=3 justify=center Cumulative incidence of anemia (Hb<=9 g/dl) for pRBC transfused LBWIs (n= &m NEC and n=&n Control LBWIs).;
title2 h=2.5 justify=center Log-Rank Test: p=&pv;
         
axis1 	label=(h=2.5 'Age of Low Birth Weight Infants' ) split="*" value=(h=1.5) order= (-1 0 to 84 by 7) minor=none offset=(0in, 0);
axis2 	label=(h=2.5 a=90 "Probability of Anemia (Hb<=9 g/dL)") order=(0 to 1 by 0.1) value=(h=2) ;
     
             
proc gplot data=pl gout=wbh.graphs;
	plot  prob0*day prob1*day1/overlay haxis = axis1 vaxis = axis2  legend=legend1;
	format day dd. prob0 prob1 4.1;
run;	

options orientation=landscape;
filename output 'anemia9_nec_rbc_surv.eps';
goptions reset=all rotate=landscape BORDER device=pslepsfc gsfname=output gsfmode=replace ;

ods pdf file = "anemia9_nec_rbc_km_curve.pdf";

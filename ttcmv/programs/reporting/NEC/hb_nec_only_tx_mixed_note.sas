...
proc mixed data=nec(rename=(dday=day)) covtest;
	class id nec day;
...
data nec_estimate;
	merge avg(where=(nec=1) rename=(estimate=estimate1 lower=lower1 upper=upper1)) 
...
*/	
	...
	symbol1 interpol=j mode=exclude value=circle co=red cv=red height=3 bwidth=3 width=1;
	symbol2 i=j ci=black value=dot co=black cv=black h=3 w=1;
	*symbol3 i=j ci=green value=square co=green cv=green h=3 w=1;
	...
	ods pdf file = "hb_nec_only_tx_repeated.pdf" style=journal;

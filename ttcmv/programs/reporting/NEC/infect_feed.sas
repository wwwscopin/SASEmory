...
data hop;
	mege feed(in=tmp) cmv.comp_pat(in=comp keep=id dob) cmv.endofstudy(keep=id studyleftdate); by id;
    day=studyleftdate-dob;
    if tmp then bfeed=1; else bfeed=0;
    if comp;
run;
...
proc sort nodupkey; by bfeed; run;
...
run;
	...
	* calculate total patient days in hospital for the study ;
		...
		data _null_;
			set total_hosp_days;
			if bfeed=0 then call symput('total_hosp_days_bfeed0', compress(total_hosp_days));
			if bfeed=1 then call symput('total_hosp_days_bfeed1', compress(total_hosp_days));
		run;

data infect;
	merge tmp hop(keep=id bfeed in=A); by id;
...
data &out;
    set &out;
    rate_1000d0 = (n0/ &total_hosp_days_bfeed0) * 1000;
    rate_1000d1 = (n1/ &total_hosp_days_bfeed1) * 1000;
	denominator0 = input(&total_hosp_days_bfeed0, 12.);
		denominator1 = input(&total_hosp_days_bfeed1, 12.);
	conf_level = .95;
...
%pt(infect,feed_infect,bfeed,&varlist);

options orientation=landscape;
ods rtf file="infect_feed.rtf" style=journal bodytitle startpage=no;
proc print data=feed_infect noobs label split="|"; 
title1 "Incident infection rates in the &nt LBWIs ";
title2 "&total_hosp_days_bfeed0(&m0 LBWIs formula fed only) and &total_hosp_days_bfeed1(&m1 LBWIs fed breast milk) patient hospital days on study";
...
label   item="."
		col0="Formula fed only|#Patients (Incidence%)"
		rate0="Formula fed only|infec.1000/hosp. days 95%CI*"
		rate1="Fed breast milk|infec.1000/hosp. days 95%CI*"
		col1="Fed breast milk|#Patients (Incidence%)"
		pv="p value"
		;
run;
ODS ESCAPECHAR="^";
ODS rtf TEXT="^S={LEFTMARGIN=1in RIGHTMARGIN=0.5in font_size=10pt}
*95% confidence intervals are calculated using an exact method based on the Possion distribution.";
ods rtf close;

...
proc sort data = cmv.plate_031; by DonorUnitId;
proc sort data = cmv.plate_001_bu; by DonorUnitId; run;

data cmv.age_of_blood;
	merge 
		cmv.plate_031 	(	in = tx_record 
									keep = id datetransfusion dateirradiated donorunitid rbcvolumetransfused
									rename = (dateirradiated = dateirradiated_txn)
								)
...
data cmv.age_of_blood; merge cmv.age_of_blood (in=b) completedstudylist (in=a); by id; if a & b; run; 
...
ods rtf file = "&output./age_of_blood/ageofblood_list.rtf" style=journal;
...
run;
	...
	* print ;
	%descriptive_stat(	print_rtf = 1, 
									...
									file= "&output./age_of_blood/age_of_blood.rtf", 
...
proc sort data = cmv.age_of_blood; by id; run;
data evertxn; set cmv.age_of_blood; by id; if first.id; evertxn = 1; keep id evertxn; run;
* MERGE ;
data cmv.rbctxn_summary; merge completedstudylist (in=a) evertxn; 
	by id; 	if a; if evertxn = . then evertxn = 0; run;


* Number of patients who received rbc txns for metadata table ;
data temp; set cmv.rbctxn_summary; if evertxn = 1; run;
...
data numtxns; set cmv.age_of_blood; 
	...
	*if ~first.id & age_of_blood > 14 then evertxn14 = evertxn14+1; *count number of old blood txns;
	if ~first.id & age_of_blood > 14 then evertxn14 = 1;

	if last.id;
	keep id numrbctxns evertxn7 evertxn14;
run;
* MERGE ;
...
data aveage; set cmv.age_of_blood;
	...
	if last.id; keep id aveage aveirr avevol;
run;
...
data cmv.rbctxn_summary; merge cmv.rbctxn_summary (in=a) aveage; 
	...
	* print ;
	%descriptive_stat(	print_rtf = 1, 
									data_out= rbctxn, 
									file= "&output./age_of_blood/rbctxn_summary.rtf", 
...
run;
	...
	%descriptive_stat(	print_rtf = 1, 
									data_out= donors, 
									file= "&output./age_of_blood/rbctxn_summary2.rtf", 
...
data metadata; set metadata1 metadata2 metadata3; 
	...
	%descriptive_stat(	print_rtf = 1, 
									data_out= metadata, 
									file= "&output./age_of_blood/metadata.rtf", 
...
run;
	...
	ods rtf file = "&output./age_of_blood/km.rtf" style=journal;

...
run;
proc sql; select count(distinct(id)) into :total_n from cmv.valid_ids;
...
data cmv.completedstudylist; set cmv.endofstudy; if reason = 4 | reason = 5 then delete; center = floor(id/1000000); keep id center; run;





* print # of LBWI, MOC, completed in sas log ;


proc sql; select count(distinct(id)),count(distinct(moc_id)) into :lbwi_n, :moc_n from cmv.valid_ids;
proc sql; select count(distinct(id)) into :completed_n from cmv.completedstudylist;
%put &total_n;
%put &lbwi_n; 
%put &moc_n;
%put &completed_n;

...
data tab1;
    length gw1-gw4 $10;
	set ga(where=(item<=2)) tab(where=(item<=12)) ga(where=(item=3) in=A) tab(where=(item>12) in=B);
...
axis1 	label=(h=2.5 "Age of LBWIs (days)" ) split="*"	value=(h=1.0)  order= (-1 to 61 by 1) minor=none offset=(0 in, 0 in);
...
proc means data=hwl(where=(day>7));
var weight;
output out=wbh mean(weight)=wt;
run;

data _null_;
    set wbh;
    call symput("wt",put(wt,7.2));
run;

proc mixed data=hwl ORDER=internal;
     model weight=st1 st2/s chisq;
     random int st1 st2/type=un subject=id;
     ods output Mixed.SolutionF=cef_wt;
run;     
...
data _null_;
    set cef_hc;
    if _n_=2 then call symput("hc9",put(estimate,7.2));
run;   

proc mixed data=hwl ORDER=internal;
     model Htlength=day/s chisq;
     random int day/type=un subject=id;
     ods output Mixed.SolutionF=cef_hl;
run; 

data _null_;
    set cef_hl;
    if _n_=2 then call symput("hl9",put(estimate,7.2));
run;   

*#####################################################################################;
%macro slope(var=weight,tx=0);

%if &tx=1 %then %do; data tmp;  set hwl(where=(tx=&tx and day<=28)); run; %end;
...
proc mixed data=hwl ORDER=internal;
        ...
        %else %do;
            model &var=tx day day*tx/s chisq;
           	random int day/type=un subject=id;
        %end;
...
%if &var=HeadCircum %then %do;
    ...
    %else %do; proc mixed data=tmp method=mvque0; %end;
        class id gw t;
        model &var=gw day day*gw/s chisq;
        repeated t/type=un subject=id r;
        ods output Mixed.SolutionF=cef&tx;
    run;
%end;
...
data _null_;
        set cef&tx;
        if effect="Intercept" then  call symput("int", put(estimate,7.2));
        if effect="day" then  call symput("s2", put(estimate,7.2));
        if effect="gw" and gw=1 then  call symput("int1", put(estimate,7.2));
        if effect="gw" and gw=2 then  call symput("int2", put(estimate,7.2));
        if effect="gw" and gw=3 then  call symput("int3", put(estimate,7.2));
        if effect="gw" and gw=4 then  call symput("int4", put(estimate,7.2));
        if effect="day*gw" and gw=1 then  call symput("stb1", put(estimate,7.2));
        if effect="day*gw" and gw=2 then  call symput("stb2", put(estimate,7.2));
        if effect="day*gw" and gw=3 then  call symput("stb3", put(estimate,7.2));
        if effect="day*gw" and gw=4 then  call symput("stb4", put(estimate,7.2));
run;
...
%if &var=weight %then %do;
axis2 label=(h=2.5 a=90 "Weight(g)") value=(h=2) order= (400 to 3200 by 200) offset=(.25 in, .25 in) minor=(number=1);
...
%if &var=HtLength %then %do;
axis2 label=(h=2.5 a=90 "Length(cm)") value=( h=2) order= (30 to 50 by 2) offset=(.25 in, .25 in) minor=(number=1);
...
plot est*day=gw/haxis = axis1 vaxis = axis2 legend=legend;
format day dd.;
run;
%mend slope;
...
data growth;
    merge weight0(where=(day=1) rename=(s2=wt0))
          weight1(where=(day=1) rename=(s2=wt1))
          weight2(where=(day=1) rename=(s2=wt2))
          weight9(where=(day=1) rename=(s2=wt9))
          ...
          HeadCircum0(where=(day=1) rename=(s2=hc0))
          HeadCircum1(where=(day=1) rename=(s2=hc1))
          HeadCircum2(where=(day=1) rename=(s2=hc2))
          HeadCircum9(where=(day=1) rename=(s2=hc9))
          HtLength0(where=(day=1) rename=(s2=length0))
          HtLength1(where=(day=1) rename=(s2=length1))
          HtLength2(where=(day=1) rename=(s2=length2))         
          HtLength9(where=(day=1) rename=(s2=length9));   
          by gw;
          ...
          gwt9=wt9/avg9*1000;
           ...
           if gw=1 then do; wt0=. ; hc0=.; length0=.; gwt0=.;end;
...
axis2 label=(h=2.5 a=90 "Weight(g)") value=(h=2.0) order= (400 to 3200 by 200) offset=(.25 in, .25 in) minor=(number=1);

...
proc format;
	...
	value item 
		...
		16="Number of pRBC Tx: Median[Min-Max]"
...
data hwl hwl_base;
	merge hwl(in=hwl) tx(in=trans keep=id num dt idx) anemia(keep=id in=ane) anemia8(keep=id in=A); by id;
...
proc print data=hwl_base;
var id hb anemic;
run;
title "xxx";
proc means data=hwl_base(where=(anemic=1));
var hb;
run;
...
%macro tab(data, out, varlist);
	...
	data tmp;      
	       set tmp;
	       where &var=&idx and anemic^=.;
	       nf=compress(frequency||"("||put(colpercent,5.1)||"%)");
	       keep &var anemic frequency nf colpercent;
	run;
...
*ods trace off;
%let varlist=gender race group death bpd sepsis ivh nec pda rop feed;
...
proc means data=&data median min max;
	class anemic;
	var &var;
	ods output means.summary=tmp(keep=gw NObs &var._median &var._min &var._max);
run;

data tab0;
	set tmp;
	&var=compress(&var._median)||"["||compress(&var._min)||"-"||compress(&var._max)||"]";
	keep anemic nobs &var;
run;
%end;
...
%let varlist=gestage age snap hb num;
%num(hwl_base, anemia, &varlist);
%let varlist=hb;
%num(hb_anemia, hb9, &varlist);
...
data tab1;
	...
	set anemia(where=(item<=3)) tab(where=(item<=10) in=A) anemia(where=(item=4) in=B) hb9(in=C) 
	anemia(where=(item=5) in=D) tab(where=(item=11) in=E);
	if A then item=item+3;
	if B then item=item+10;
	if C then item=item+14;
		if D then item=item+11;
			if E then item=item+6;
	format item item.;
run;

ods rtf file="anemia9.rtf" style=journal bodytitle;
...
define item/"." style=[just=left cellwidth=2.5in];
define anemic0/"Not Anemic (Hb>9 g/dL)"     style(column)=[cellwidth=2in];
define anemic1/"Anemic (Hb<=9 g/dL)"  style(column)=[cellwidth=2in];
*break after item/ dol dul skip;
run;

ODS ESCAPECHAR="^";
ODS rtf TEXT="^S={LEFTMARGIN=0.9in RIGHTMARGIN=0.9in font_size=10 pt}
1. Anemic: Hemoglobin <=9 g/dL before 1st pRBC transfusion.

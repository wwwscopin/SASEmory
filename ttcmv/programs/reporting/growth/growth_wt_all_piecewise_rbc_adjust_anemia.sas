...
data hwl0;
    ...
    wbhx.death(keep=id deathdate in=dead)
	...
	keep id day Weight WeightDate HeadCircum HeadDate HtLength HeightDate MultipleBirth SNAPTotalScore
			LBWIDOB Gender  IsHispanic  Race RaceOther Hb HbDate Center snapg bw gestage gesta death;
...
proc means data=hwl_tx(where=(day=1));
var age;
run;
...
proc gplot data=line_wt1 gout=wbh.graphs;
	plot estimate*day/overlay annotate= anno1 haxis = axis1 vaxis = axis2 legend=legend href=0 lhref=20;
...
proc mixed /*method=ml*/ data=tmp covtest;
	class id &var race gesta snapg death;
	model weight=d1 d2 &var &var*d2 age race gesta snapg death/s;
	random int d1 d2/type=un subject=id;

    %if &var^=center %then %do; 
	estimate "int1" int 1 &var 1 0 age 9;
	estimate "int2" int 1 &var 0 1 age 9;
	estimate "pre" d1 1 ;
	estimate "post1" d2 1 &var*d2 1 0;
	estimate "post2" d2 1 &var*d2 0 1;

	*estimate "post2" d2 1 &var*d2 0 1/e;
	%end;
...
ods rtf file="Interaction_rbc_adjust.rtf" style=journal bodytitle;

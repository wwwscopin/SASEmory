...
data hwl0;
	...
	*if 501<bw<=750 then gw=1;
	...
	MultipleBirth gender LBWIDOB race gestage bw gw bpd sepsis ivh nec feed snap pda rop hb death;
	rename LBWIDOB=dob ;
...
data tab1;
	...
	set anemia(where=(item<=2)) tab(where=(item<=12)) anemia(where=(item=3) in=A) tab(where=(item>12) in=B);

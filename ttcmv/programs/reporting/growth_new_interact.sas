...
data hwl0;
	...
	*if 501<bw<=750 then gw=1;
	...
	if SNAP>&median then snapg=1;else snapg=0;
	...
	MultipleBirth gender LBWIDOB race gestage bw gw bpd sepsis ivh nec feed snap snapg pda rop;
...
data hwl hwl_base;
	merge hwl(in=hwl) tx(in=trans keep=id dt idx) anemia(in=A); by id;
...
proc mixed data=hwl /*method=ml*/ ORDER=internal;
        *class id tx t; *this is for repeated statement;
        class id tx group gender race anemic snapg nec; *this is for random statement;
        model weight=tx group gender race anemic snapg nec st1 st2 st1*tx st2*tx st2*group st2*gender st2*race st2*anemic st2*snapg st2*nec/s chisq;
...
data slope;
    ...
    est=estimate;
    stderr=stderr;
    if _n_=21 then call symput("s", put(est,7.4));
run;
...
data _null_;
	...
	if _n_=29 then do; 
        call symputx("s1", put(estimate,4.1)||"("||compress(put(stderr,3.1))||")");
	end;
	   	
	if _n_=30 then do; 
        call symputx("s2", put(estimate,4.1)||"("||compress(put(stderr,3.1))||")");
	end;
run;
...
axis2 	label=(h=2.5 a=90 "Weight(g)") value=(h=2) order= (600 to 3200 by 100) offset=(.25 in, .25 in) minor=(number=1);
 
title1 	height=3 "All LBWIs Weight vs Age";
title2  height=2.5 "--Analyzed with Covariates: Gender, Race, Anecmia, SNAP, NEC,  and Gestational Age";
title3 	height=2.5  "(With pRBC Transfusion=&t1, Without pRBC Transfusion=&t0)";
...
ods pdf file = "growth_all_inter.pdf";

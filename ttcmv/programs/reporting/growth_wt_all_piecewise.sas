...
value wk 1="Weigth gain during Week 1"
         2="Weigth gain during Week 2"
         3="Weigth gain during Week 3"
         4="Weigth gain during Week 4"
         5="Weigth gain during Day 28-40"
         6="Weigth gain during Day 40-60"
        ;
...
value item 0="--"
           ...
           3="Center"
           4="Anemia(Hemoglobin<=9 g/dL) before 1st Transfusion"
           5="SNAP at Birth"
           6="Any breast milk fed before 1st Transfusion"
...
data hwl hwl_tx hwl_no_tx;
	merge hwl(in=hwl) tx(in=trans keep=id dt) feed(keep=id in=breast) cafe(keep=id in=cafe) anemia(in=A); by id;
...
/************************************************************************************
data wk_gain;
   merge hwl(where=(day=1)  keep=id tx day weight rename=(weight=wt0))
         hwl(where=(day=4)  keep=id tx day weight rename=(weight=wt4))
         hwl(where=(day=7)  keep=id tx day weight rename=(weight=wt7))
         hwl(where=(day=14) keep=id tx day weight rename=(weight=wt14))
         hwl(where=(day=21) keep=id tx day weight rename=(weight=wt21))
         hwl(where=(day=28) keep=id tx day weight rename=(weight=wt28))
         hwl(where=(day=40) keep=id tx day weight rename=(weight=wt40))
         hwl(where=(day=60) keep=id tx day weight rename=(weight=wt60));
   by id;
      wg1=wt7-wt0;
         wg2=wt14-wt7;
            wg3=wt21-wt14;
...
data tab;
    merge tmp(where=(tx=0) keep=tx &var._Mean  &var._StdDev)
          tmp(where=(tx=1) keep=tx &var._Mean  &var._StdDev rename=(&var._Mean=avg &var._StdDev=err));
    wg_tx0=put(&var._Mean,4.1)||"&pm"||compress(put(&var._StdDev,4.1));
    wg_tx1=put(avg,4.1)||"&pm"||compress(put(err,4.1));
    keep wg_tx0 wg_tx1;
run;
...
%let varlist=wg1 wg2 wg3 wg4 wg5 wg6;
%wkgain(wk_gain, wg, &varlist);

ods rtf file="wk_gain.rtf" style=journal bodytitle;
proc print noobs label ;
title "Weight gain during period";
id wk/style(data)=[cellwidth=2.25in just=left];
var wg_tx0 wg_tx1 pv/style(data)=[cellwidth=1.5in just=center] style(header)=[just=center];
label wk="."
      wg_tx0="Without Transfusion (g)" 
      wg_tx1="With Transfusion (g)"
      pv="p value";
run;
ods rtf close;
**************************************************************************************************/
...
value idx 0="Without Transfusion (n=&no)" 1="With Transfusion (n=&yes)";
...
data info_notx;
	...
	if effect="st2" then do;
	   effect="Slope for LBWIs without Transfusion (g/day)";
...
proc mixed /*method=ml*/ data=hwl_tx covtest;
	...
	estimate "Before Transfusion, Month1"  int 1 d1 -30.5;  
	estimate "After Transfusion, Month1"  int 1 d2 30.5;  
	estimate "After Transfusion, Month2"  int 1 d2 61;  
...
data info_tx;
	...
	ms=put(estimate,4.0)||" &pm "||compress(put(stderr,4.0));
	if effect="Intercept"  then effect="Intercept (g, Weight at 1st Transfusion)";
	if effect="d1" then do; 
	   effect="Slope Before 1st Transfusion (g/day)";
	   ms=put(estimate,4.1)||" &pm "||compress(put(stderr,4.1));
	end;
	if effect="d2" then do;
	   effect="Slope After 1st Transfusion (g/day)";
	   ms=put(estimate,4.1)||" &pm "||compress(put(stderr,4.1));
	 end;
	if find(label, "Before") then effect="Mean Weight at 1 Month Before 1st Transfusion (g)";
	if find(label, "After") and find(label, "Month1") then effect="Mean Weight at 1 Month After 1st Transfusion (g)";
	if find(label, "After") and find(label, "Month2") then effect="Mean Weight at 2 Month After 1st Transfusion (g)";
...
value = (h=2 "Without Transfusion, Slope(SE)=&s0 g/day" )  offset=(0.2in, -0.4 in) frame;


axis2 	label=( h=2.5 a=90 "Weight(g)") value=(h=2) order= (0 to 2800 by 200) offset=(.25 in, .25 in) minor=(number=1);
 
title 	height=3 "Weight vs Days for All LBWIs without Transfusion (n=&no)";
...
symbol1 interpol=spline mode=exclude value=circle co=red cv=red height=2 bwidth=3 width=1;

axis1 	label=(h=2.5 'Days Before and After 1st Transfusion' ) split="*"	
value=(h=1.0 c=white) order= (-41 to 41 by 1) minor=none major=(c=white) offset=(0in, 0in) origin=(,15)pct;


legend across = 1 position=(top left inside) mode = reserve fwidth =.2	shape = symbol(3,2) label=NONE 
value = (h=2 "Pre-1st Transfusion, Slope(SE)=&sb g/day; Post-1st Transfusion, Slope(SE)=&sa g/day" )  offset=(0in, -0.4 in) frame;

title 	height=3 "Weight vs Days for All LBWIs with Transfusion (n=&yes)";
...
goptions reset=all;
ods pdf file = "growth_all_joint.pdf";
...
data &out;
    ...
    if label='pre'  then effect="Pre-Transfusion Slope (g/day)";
    if label='post' then effect="Post-Transfusion Slope (g/day)";
run;
...
data est;
    ...
    if label='pre' then effect="Pre-Transfusion Slope";
    %if &var=gender %then %do; 
    if label='int1'  then effect="Intercept --Male"; 
    if label='int2'  then effect="Intercept --Female"; 
    if label='post1' then do; effect="Post-Transfusion Slope --Male"; probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope --Female";probt=. ;end;
    %end;

    %if &var=race %then %do; 
    if label='int1'  then effect="Intercept --Black"; 
    if label='int2'  then effect="Intercept --White"; 
    if label='post1' then do; effect="Post-Transfusion Slope --Black"; probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope --White";probt=. ;end;
    %end;
    
    %if &var=center %then %do; 
    if label='int1'  then effect="Intercept --Midtown"; 
    if label='int2'  then effect="Intercept --Grady"; 
    if label='int3'  then effect="Intercept --Northside"; 
    if label='post1' then do; effect="Post-Transfusion Slope --Midtown"; probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope --Grady"; probt=. ;end;
    if label='post3' then do; effect="Post-Transfusion Slope --Northside";probt=. ;end;
    %end;
    

    %if &var=anemic %then %do; 
    if label='int1'  then effect="Intercept --Not Anemic"; 
    if label='int2'  then effect="Intercept --Anemic"; 
    if label='post1' then do; effect="Post-Transfusion Slope --Not Anemic&two";probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope --Anemic";probt=. ;end;
    %end;
    

    %if &var=snapg %then %do; 
    if label='int1'  then effect="Intercept --SNAP score <=Median(&median)"; 
    if label='int2'  then effect="Intercept --SNAP score >Median(&median)"; 
    if label='post1' then do; effect="Post-Transfusion Slope  --SNAP score<=Median(&median)";probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope  --SNAP score>Median(&median)"; probt=. ;end;
    %end;
    
    %if &var=feed %then %do; 
    if label='int1'  then effect="Intercept --No"; 
    if label='int2'  then effect="Intercept --Yes"; 
    if label='post1' then do; effect="Post-Transfusion Slope --No";probt=&pv;end;
    if label='post2' then do; effect="Post-Transfusion Slope --Yes"; probt=. ;end;
    %end;
...
data &out;
    ...
    estd=compress(put(estimate,7.2)||"&pm"||put(stderr,5.2));
...
%let varlist=gender race  center anemic snapg feed caffine;
*ods trace on/label listing;
...
data tabA tabB;
    ...
    if item<5 then output tabA; else output tabB;
    format item item.;
run;

data tabA;
    set tabA; by item; output;
    retain tmp;
    if last.item then do; Call missing( of _all_ ); output; end;
run;

data tabB;
    set tabB; by item; output;
    retain tmp;
    if last.item then do; Call missing( of _all_ ); output; end;
run;


ods rtf file="Interaction.rtf" style=journal bodytitle;
proc print data=tabA noobs label;
title "Univariable Analysis of Growth Before and After 1st Transfusion* (n=&yes)";
...
ODS ESCAPECHAR="^";
ODS rtf TEXT="^S={LEFTMARGIN=0.5in RIGHTMARGIN=0.5in font_face=Times font_size=12 pt}
* Median age at 1^{super st} transfusion: &medage days.
^n 1. Weight at time of 1^{super st} transfusion.
^n 2. Growth velocity after 1^{super st} transfusion is greater if anemic before 1^{super st} transfusion compared to LBWIs who are not anemic before 1^{super st} transfusion.";

proc print data=tabB noobs label;
title "Univariable Analysis of Growth Before and After 1st Transfusion* (n=&yes)";
id item0/style(data)=[just=left width=2in] style(header)=[just=left];
var effect/style(data)=[just=left width=2.5in] style(header)=[just=left];
var estd pv/style(data)=[just=center width=1.25in] style(header)=[just=center];

...
data hwl0;
	...
	hb           wbh.death(keep=id deathdate in=dead)
	...
	keep id day Weight WeightDate HeadCircum HeadDate HtLength HeightDate MultipleBirth SNAPTotalScore
			LBWIDOB Gender  IsHispanic  Race RaceOther Hb HbDate Center snapg bw gestage gesta deathdate dead;
...
proc print data=hwl_tx(where=(anemic8=1));
var id anemic8 tx hbdate dt hb day;
run;

proc sort data=hwl_tx nodupkey out=anemia; by id anemic8;run;
proc means data=anemia n;
class anemic8;
var id;
...
data _null_;
    set wbh;
    if anemic8=1 then call symput("m1", compress(n));
    if anemic8=0 then call symput("m0", compress(n));
run;  
...
%do %while (&x <2);
	...
	%if &x = 1  %then %do; 	%let variable =Hb;    	        %let description =  'Hemoglobin (g/dL)'; 
																				%let order=(6 to 20 by 1);	  %end;     
...
symbol1 i=j ci=blue value=circle h=0.5 w=1 repeat=100; 
    ...
    axis1 	label=(h=3 'Days before 1st pRBC Transfusion (days)' ) value=(h=1.25) split="*" order= (-41 to 1 by 1) minor=none offset=(0 in, 0 in);
    axis3 	label=(h=3 'Days After 1st pRBC Transfusion (days)' ) value=(h=1.25) split="*" order= (-1 to 61 by 1) minor=none offset=(0 in, 0 in);
    axis2 	label=(h=3 a=90 &description) value=(h=2) order=&order ;

                  
	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Anemic (n=&m1)";  
		...
		format daytx y_axis. &variable 5.0;
	run;
	...
	proc gplot data=tmp gout=wbh.graphs;
		title h=3.5 justify=center &description, "Not Anemic (n=&m0)";  
		note h=2 m=(2pct, 8.5 pct) "Days:" ;
		where anemic8=0 and day<=0; 
		plot &variable*day=id/ overlay haxis = axis1 vaxis = axis2  nolegend vref=8 lvref=20 /*href=0 lhref=20*/; 
		format daytx n_axis. &variable 5.0;
	run;

...
value item 0="--"
           ...
           7="SNAP at Birth"
           8="Any breast milk fed before 1st pRBC transfusion"
           9="Caffeine used before 1st pRBC transfusion"
           ;
value Anemic 0="Not Anemic" 1="Anemic";
value snapg  0="SNAP Score <=Median(&median)" 1="SNAP Score >Median";

    value group 1="SGA"  0="AGA" ;
...
data hwl0;
	...
	keep id day Weight WeightDate HeadCircum HeadDate HtLength HeightDate MultipleBirth SNAPTotalScore
			LBWIDOB Gender  IsHispanic  Race RaceOther Hb HbDate Center snapg bw gestage;
...
%macro test(data,out, varlist)/minoperator;
...
data _null_;
    ...
    %if &var=group %then %do; if &var=1 then call symput("ng1", compress(nobs)); if &var=0 then call symput("ng2", compress(nobs)); %end;
...
data est;
    ...
    if label='int1'  then do; effect="Intercept --AGA (n=&ng2)"; probt=&pv0; end;
    if label='int2'  then do; effect="Intercept --SGA (n=&ng1)"; probt=.; end;

    if label='post1' then do; effect="Post-pRBC Transfusion Slope --AGA"; probt=&pv;end;
    if label='post2' then do; effect="Post-pRBC Transfusion Slope --SGA"; probt=. ;end;
    %end;    
...
%let varlist=gender race  center anemic anemic8 group snapg feed caffine;
*ods trace on/label listing;

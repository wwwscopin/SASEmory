...
run;
	...
	axis1 	label=(h=2 "Age (days)" ) split="*"	value=(h=1.1)  order= (-1 to 61 by 1) minor=none offset=(0 in, 0 in);
	axis2 	label=(h=2 a=90 "Hemoglobin(g/dL)") value=(h=2) order= (6 to 16 by 1) offset=(.25 in, .25 in) minor=(number=1); 


	legend across = 1 position=(top right inside) mode = reserve fwidth =.2	shape = symbol(3,2) label=NONE 
   value = (h=2 "Non-IVH" "IVH") offset=(0, -0.4 in) frame;
	                
	*title1 	height=3 "Hemoglobin vs Age (Non-IVH=&no_ivh(&no obs), IVH=&yes_ivh(&yes obs))";
	title1 	height=2.5 "Hemoglobin vs Age (Non-IVH=&no_ivh, IVH=&yes_ivh (with Hemoglobin before IVH diagnosis))";

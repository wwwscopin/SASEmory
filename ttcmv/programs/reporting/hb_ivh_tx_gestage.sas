...
data ivh;
	...
	if hbdate>=imagedate and ivh=1 then hb=.;
    if hb=. then delete;
...
goptions reset=global rotate=landscape gunit=pct noborder cback=white
	...
	symbol1 i=j mode=exclude value=circle co=blue cv=blue h=2 bwidth=3 width=1;
	...
	legend across = 1 position=(top right inside) shape = symbol(3,2) label=NONE 
    value = ( h=2 c=black "Non-IVH without pRBC" "IVH" "Non-IVH with pRBC") offset=(-1in, -0.4 in) frame cframe=white cborder=black;
	                
	title1 	height=4 "Hemoglobin vs Days (non-IVH without pRBC=&n0(&m0 obs), IVH=&n1(&m1 obs), non-IVH with pRBC=&n2(&m2 obs))";
	title2 	height=2.5 "Slope of non-IVH without pRBC =&sp0, Slope of IVH=&sp1, Slope of non-IVH with pRBC=&sp2";
	title3  height=2.5 "Test of equal slopes between Non-IVH without pRBC and with pRBC, p=&p0; between Non-IVH without pRBC and IVH, p=&p1 between Non-IVH with pRBC and IVH, p=&p2";


	ods pdf file = "hb_ivh_tx_gestage.pdf" style=journal;
			proc gplot data= ivh_estimate;
				...
				note h=1 m=(2pct, 16 pct) "Day :" ;
               	note h=1 m=(0pct, 14 pct) "#Non-IVH without pRBC" ;
               	note h=1 m=(0pct, 12 pct) "#IVH" ;
               	note h=1 m=(0pct, 10 pct) "#Non-IVH with pRBC" ;
			run;	

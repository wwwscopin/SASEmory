...
proc sql;
	...
	select a.*
	from hb as a, notrans_id as b
	where a.id=b.id
;


data hb;
	set hb rbc(keep=id DateTransfusion hbdate Hb); by id;
run;
...
data hb;
	set hb; by id HbDate;
	if dob=. then 
	if first.id then do; base=HbDate; dob=base; retain base; end;
	else dob=base;
	day0=Hbdate-dob;
	if day=. then do;
...
             
        ...
        goptions rotate = portrait;
        	...
        	ods ps file = "hb_mixed.ps";
        	ods pdf file = "hb_mixed.pdf" startpage=no;

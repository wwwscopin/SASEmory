...
data one;
	...
	* AFTER that, draw bars ('A' option ensure properly layering over the greay bars!);
	...
	X=NECResolveDate; 	y=9; FUNCTION='LABEL'; when = 'A'; size=1; color='yellow'; text=put(NECResolveDate, date9.);  OUTPUT; 
	X=NECResolveDate; 	y=10; FUNCTION='MOVE'; when = 'A'; line=3; size=2; color='yellow';  OUTPUT; * start at mean ;
	y=8; 	FUNCTION='DRAW'; when = 'A'; line=2; size=1; color='yellow';  OUTPUT; * draw down;
	y=15; 	FUNCTION='DRAW'; when = 'A'; line=2; size=1; color='yellow';  OUTPUT; * draw down;
	...
	* draw a light gray rectangle ;
	if gender=1 then do;  
		...
		function = 'BAR'; x = end; y = 16.1; color = 'ltgray'; style = 'empty'; line= 0; output;
	end;
	else if gender=2 then do;   			
		function = 'move'; x = start; y = 11.4; output;
		function = 'BAR'; x = end; y = 14.4; color = 'ltgray'; style = 'empty'; line= 0; output;
	end;
...
proc gplot data=nec gout=wbh.graphs; 
	title font=arial bold height=16pt color=Brown "Gender=#byval(gender) Type=#byval(type)";
	where id=1000211;
	by id gender type;
...
ods pdf file="hb_noID.pdf";
	proc greplay igout =wbh.graphs tc=sashelp.templt template=v2s nofs nobyline;
		list igout;
		treplay 1:1 2:2 ;	
	run;
...
tdef t1 4 /llx=5    ulx=5   lrx=50   urx=50  lly=0    uly=25    lry=0      ury=25
        ...
        5 /llx=50   ulx=50  lrx=95   urx=95  lly=75   uly=100   lry=75     ury=100
                                                ;
template t1;
tplay 1:3 2:4 3:5 4:6 5:7 6:8 7:9 8:10;
run;

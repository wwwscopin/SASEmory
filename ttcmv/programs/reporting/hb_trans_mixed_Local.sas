...
proc sql;
	...
	where a.id=b.id and -7<=day<=7 and hb^=.;
;
...
%anno(data=trans);
         ...
         goptions reset=all  device=jpeg  rotate=landscape gunit=pct noborder cback=white colors = (black red);
...
         					symbol3 i=j ci=red value=dot h=1 w=4;
         ...
         %macro make_plots(data=trans); 
          		...
          		%if &data = trans %then %do; 	%let variable =hb; %let description = "Hemoglobin(g/dL)- With Transfusion (n=&n_yes)";
			...
			%let n_5= 0; %let n_6= 0; %let n_7= 0;
         		...
         		* populate 'n' annotation variables ;
         		...
         		proc format; 
         		 	value time_axis   
         		 	-8="Day*(n)"
         		 	-7="-7*(&n_n7)"  -6="-6*(&n_n6)"   -5="-5*(&n_n5)"  
         		 	-4="-4*(&n_n4)"  -3="-3*(&n_n3)"   -2="-2*(&n_n2)"   -1="-1*(&n_n1)" 
					0= "0*(&n_0)" 1="1*(&n_1)"   2="2*(&n_2)" 3="3*(&n_3)" 4="4*(&n_4)" 
					5= "5*(&n_5)"  6="6*(&n_6)" 7="7*(&n_7)" 8="8*" ;
				...
				axis1 	label=( h=3 'Days Before and After First RBC Transfusion' ) order=(-8 to 8 by 1) value=( h=2.0) split="*"  minor=none offset=(0 in, 0 in);
         		axis2 	label=( h=3 a=90 "Hemoglobin(g/dL)") order=(5 to 20 by 1) value=( h=2) &scale ;
...
             
        ...
        goptions rotate = portrait;
            ...
            filename output  "hb_trans_mixed_local.eps";

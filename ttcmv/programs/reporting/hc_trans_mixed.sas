...
data rbc;
	...
	keep id BodyWeight DateHbHct DateTransfusion hct HctTest Hct TimeHbHct rbcVolumeTransfused rbc_TxEndTime rbc_TxStartTime rbc_txt;
run;
...
data rbc;
	...
	if first.id and hct^=. then day=0; else day=.;
run;

data hct;
	set hct rbc(keep=id DateTransfusion hctdate hct day); by id;
run;
...
data trans;
	...
	if hct=. or hctdate=. then delete;
	drop base;
run;

proc sort nodupkey; by id day hct;run;
   	...
   	run;
		...
		DATA anno_mixed_&data; 
			...
			* draw a light gray rectangle ;
					function = 'move'; x = -42; y = 33.3; output;
					function = 'BAR'; x = 62; y = 46.5; color = 'ligr'; style = 'solid'; line= 0; output;
...
         					symbol3 i=j ci=reddL value=dot h=1 w=4;
         ...
         %macro make_plots(data=trans); 
          		...
          		%if &data = trans %then %do; 	%let variable =hct; %let description = f=zapf "Hematocrit(%)- With Transfusion (n=&n_yes)";
         							%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/;%end;
          		%if &data = notrans %then %do; 	%let variable =hct; %let description = f=zapf "Hematocrit(%)- Without Transfusion (n=&n_no)";
         							%let scale = /*order = (0 to 50 by 5) minor=(number=3)*/;%end;
...
             
        ...
        goptions rotate = portrait;
        	...
        	ods ps file = "hct_trans_mixed.ps";
        	ods pdf file = "hct_trans_mixed.pdf" startpage=no;
...
/*
  ...
  quit;
	...
	ods pdf file="hct_trans_mixed_data.pdf" style=journal;
*/
	...
	title1	height=3 f=zapf 'Hematocrit longitudinal model-based means';
		...
		* now make a table of the means and 95% CI for sofa score on days 0, 7, 14;
		data hct_trans_mixed;
		   ...
		   keep dday num_t hct_t;
		run;

			proc print data = hct_trans_mixed noobs label split="*" style(data) = [just=center];
				var dday num_t;
			 var hct_t/style(data) = [cellwidth=2in just=center] ;
			run;

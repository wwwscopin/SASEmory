...
data lbwi;
	...
	cmv.plate_006
	cmv.plate_008(keep=id ROM Steroids DeliveryMode)	
...
var id ivh rbc imagedate dt_rbc rbc_day plt dt_plt plt_day;
run;
*/
...
%table(lbwi, tx_tab, idx_rbc, idx_plt); quit;
...
ods rtf file="tx_tab.rtf" style=journal bodytitle;
proc print data=tx_tab noobs label split="*" style(header) = [just=center];
title "Table1: RBC Transfusion by Platelet Transfusion";
Var idx_rbc /style(data)=[cellwidth=1.5in just=left];
var c1-c3 c /style(data) = [cellwidth=0.8in just=center] ;
format idx_rbc ntx.;
label  idx_rbc="RBC Transfusion"
       c1="Tx=0*(n=&n1)"   
       c2="Tx=1~2*(n=&n2)"
       c3="Tx>=3*(n=&n3)"
       c="Total*(n=&n)";
...
%table(lbwi, rbc_tab, nrbc, ivh); quit;

ods rtf file="rbc_tab.rtf" style=journal bodytitle;
proc print data=rbc_tab noobs label split="*" style(header) = [just=center];
title "Table1: RBC Transfusion by IVH";
Var nrbc /style(data)=[cellwidth=1.5in just=left];
var c1-c2 c /style(data) = [cellwidth=0.8in just=center] ;
label  nrbc="RBC Transfusion"
       c1="IVH=No*(n=&n1)"   
       c2="IVH=Yes*(n=&n2)"
       c="Total*(n=&n)";
run;
...
%table(lbwi, plt_tab, nplt, ivh); quit;

ods rtf file="plt_tab.rtf" style=journal bodytitle;
proc print data=plt_tab noobs label split="*" style(header) = [just=center];
title "Table2: Platelet Transfusion by IVH";
Var nplt /style(data)=[cellwidth=1.5in just=left];
var c1-c2 c /style(data) = [cellwidth=0.8in just=center] ;
label  nplt="Platelet Transfusion"
       c1="IVH=No*(n=&n1)"   
       c2="IVH=Yes*(n=&n2)"
       c="Total*(n=&n)";
run;
...
proc sort; by item code;run;
	...
	data &out;
		...
		if item=36 then  do; code0=put(code, dmode.); end;
...
BirthResusCPAP ROM steroids pco po pov fio gratio gr indom indo AntiConvulsant sepsis apg IsChorloConfirm HistoChloro DeliveryMode;

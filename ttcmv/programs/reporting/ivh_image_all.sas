...
proc sql; 
	create table ivh as 
	select a.*
	from ivh0 as a , cmv.completedstudylist as b
	where a.id=b.id;
...
data count;
	...
	if leftIVHGrade=. and RightIVHGrade=. then do; 
		leftIVHGrade=100; rightIVHGrade=100; 
		call symput("m", compress(frequency));
	end;
...
data ivh_tab;
	...
	f1=nc1/nc*100; f2=nc2/nc*100; 	f3=nc3/nc*100; 	f4=nc4/nc*100; 	f99=nc99/nc*100; f=nc/&m*100; f_total=&n/&n_total*100;
	...
	nout=nc||"/"||compress(&m)||"("||compress(put(f,4.0))||"%)";
	if LeftIVHGrade=100 then do;
		nout1=nc1||"/"||compress(&m)||"("||compress(put(f1,4.0))||"%)";
		nout2=nc2||"/"||compress(&m)||"("||compress(put(f2,4.0))||"%)";
		nout3=nc3||"/"||compress(&m)||"("||compress(put(f3,4.0))||"%)";
		nout4=nc4||"/"||compress(&m)||"("||compress(put(f4,4.0))||"%)";
		nout99=nc99||"/"||compress(&m)||"("||compress(put(f99,4.0))||"%)";
...
ods rtf file="ivh_all.rtf" style=journal bodytitle;

...
proc sort data=lbwi_blood_nat_result; by center dfseq;run;
proc freq data=	lbwi_blood_nat_result;
by center dfseq; 
tables NATTestResult/out=xx ;;
run;

proc sql;
create table yy as
select count(*) as total, dfseq, center,UrineTestResult as classvar format=serotest., "UrineTestResult" as variable, 1 as group
from  lbwi_urine_nat_result	 where dfseq <> .
group by  center,dfseq,  variable, group
union
select count(*) as total, dfseq, center,NATTestResult as classvar format=Igtest., "NATTestResult" as variable, 2 as group
from  lbwi_blood_nat_result	 where dfseq <> .
group by  center, dfseq, variable, group


order by center, variable, dfseq
;
...
length stat $ 25;
length stat2 $ 25;
stat=compress( count || " (" || put(percent,5.1) || "%)" );
...
from xx as a,
yy as b
where  a.dfseq = b.dfseq and a.variable=b.variable and a.classvar=b.classvar and a.center=b.center;
...
length stat3 $ 25;
stat3=compress( count || " / " || total || " (" || put(percent,5.1) || "%)" );
...
define DFSEQ/ group order  center order=internal   width=15 " Day on * Study ";

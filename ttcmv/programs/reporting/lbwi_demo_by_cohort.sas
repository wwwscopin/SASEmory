...
*** get cohort ;
	...
	data cohort1; set cmv.moc_sero (rename = (ComboTestResult = cohort)); if dfseq = 1; 
		moc_id = input(substr(put(id,7.0),1,5),5.0); keep cohort moc_id; run;

	data cohort2; set cmv.plate_001; if enrollmentdate ~= .; 
		moc_id = input(substr(put(id,7.0),1,5),5.0); keep id moc_id; run;

	proc sort data = cohort1; by moc_id; run;
	proc sort data = cohort2; by moc_id; run;

	data cmv.cohort; merge cohort1 cohort2; by moc_id; keep id cohort; run;

	proc sort data = cmv.cohort; by id; run;
	data cmv.lbwi_demo; merge cmv.lbwi_demo (in=a) cmv.cohort; by id; if a; run;
...
data lbwi_demo; merge cmv.lbwi_demo cmv.valid_ids (in=a); by id; if a; run;
...
data lbwi_demo1; set lbwi_demo; if cohort = 1; run;
data lbwi_demo2; set lbwi_demo; if cohort = 2; run;

data singletons1; set singletons; if cohort = 1; run;
data singletons2; set singletons; if cohort = 2; run;
...
* Merge tables and headers;
	...
	proc sql; select count(*) into :cohort1 from cmv.lbwi_demo where cohort=1;
	proc sql; select count(*) into :cohort2 from cmv.lbwi_demo where cohort=2;

	data lbwi_demo_summary; set lbwi_demo_summary; 	
		keep row disp1 disp2; 
		label 	disp1 = "MOC Seronegative*(n = &cohort1)"
					disp2 = "MOC Seropositive*(n = &cohort2)";
	run;
	...
	ods rtf file = "&output./annual/lbwi_demo_summary_by_cohort.rtf"  style=journal toc_data startpage = no bodytitle;
		title1 "LBWI Demographic and Birth Characteristics Summary - By MOC CMV Serology Cohort"; 

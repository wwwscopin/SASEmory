...
******* DATE >= 1st of last month **************;
**** The only way Neeta and I could get this to work. ;

data _NULL_;
	call symput ("today_date", compress(put(today(), date9.)));
run;

 data complete; set complete; 
		today_date = today();
		format today_date date9.;
		prev_month = month(today_date) - 1;
		if month(today_date) = 1 then prev_month = 12;

		if month(DateTransfusion) = prev_month; 
 run;

 data incomplete; set incomplete; 
		today_date = today();
		format today_date date9.;
		prev_month = month(today_date) - 1;
		if month(today_date) = 1 then prev_month = 12;
...
************************************************;
	...
	ods rtf file = "&output./monthly/&mon_file_unit_status.unit_status_listing.rtf" style=journal toc_data startpage = yes bodytitle;
	ods noproctitle proclabel "&mon_pre_unit_status Transfusion records on patients with data since last monthly report";

		title1 "&mon_pre_unit_status Transfusion records on patients with data since last monthly report";

		proc print data = complete label split = "*" noobs style(header) = [just=center] contents = "";
			by id notsorted;
		...
		proc print data = incomplete label split = "*" noobs style(header) = [just=center] contents = "";
			by id notsorted;

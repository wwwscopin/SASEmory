...
select a.*, b.*
from donor as a left join
( select * from cmv.Plate_001_bu where dccunitid <>  20000032 ) as b
on a.donorunitid =b.donorunitid;
...
data temp; rename &var=category ; set temp /*(rename=(&var=category))*/;
...
%NATsample ( data=bu_wbc1, out=, var=wbc_result1,f=wbc., varlabel=Unit WBC Result,gp=5);

%NATsample ( data=bu_track, out=, var=ABOGroup,f=abo., varlabel=Blood Group,gp=6);
%NATsample ( data=bu_track, out=, var=RHGroup,f=rh., varlabel=RH Group,gp=7);
...
%getstat ( data=donor1, out=, var=UnitResult,f=NATTestResult., varlabel=Unit Test Result,gp=4,subheader=0);
%getstat( data=bu_wbc1, out=, var=wbc_result1,f=wbc., varlabel=Unit WBC Result,gp=5,subheader=0);
...
*define variable / group      style(column)=[just=center cellwidth=.5in]  'Variable ';

define treat / group order=data center      style(column)=[just=left cellwidth=.7in] 'Site' ;
define visitlist /  group order=data left      style(column)=[just=left cellwidth=0.9in]  'DOL ';


define category /across   center order=internal    style(column)=[just=left cellwidth=1in] '' ;
...
ods noproctitle proclabel "&nat_result_summary_title d: CMV NAT results for Donor Units for LBWI who completed study";

title  justify = center "&nat_result_summary_title d: CMV NAT results for Donor Units for LBWI who completed study";
...
column  /*variable visitlist */     treat  visitlist category ,(stat) dummy ;
...
ods noproctitle proclabel "&nat_result_summary_title f: Donor Unit Characteristics";

title  justify = center "&nat_result_summary_title f: Donor Unit Characteristics ";

footnote "";
proc report data=AllVarFreq nofs   style(header) = [just=Left] split="_" missing headline headskip contents = "" ;

where variable IN ( 'Blood Group' , 'RH Group' )and treat=0 ;

column  variable /* visitlist visitlist */     treat   category  stat dummy ;


define variable / group order=data center   width=15   style(column)=[just=left cellwidth=1in] 'Site' ;
define treat / group order=data center   width=15   style(column)=[just=left cellwidth=1in] 'Site' ;
*define visitlist /  group order=data left   width=15   style(column)=[just=left cellwidth=1in]  'Blood Donor Unit Type ';


define category /group   center order=internal    style(column)=[just=center cellwidth=1in] '' ;

define stat/  center   style(column)=[just=left cellwidth=0.5in]  'n/N (%) ' ;;
define dummy/ noprint;

...
symbol1 i=j mode=exclude value=none co=red cv=red height=0.6 bwidth=4 width=3 l=1;
symbol2 i=j mode=exclude value=none co=red cv=red height=0.6 bwidth=4 width=2 l=3;
symbol3 i=j mode=exclude value=none co=red cv=red height=0.6 bwidth=4 width=2 l=3;


title1 ;
title2 h=3.5 justify=center Cumulative Incidence of NEC and 95% Confidence Intervals;
         
axis1 	label=(h=3 'Age of Low Birth Weight Infants' ) split="*" value=(h=3.0) color=white order= (0 to 70 by 7) minor=none;
axis2 	label=(h=3 a=90 f=swissb "Probability of NEC") color=white order=(0 to 0.20 by 0.05) value=(h=3) ;
     
             
proc gplot data=pl gout=wbh.graphs;
	plot  prob*day upper*day lower*day/overlay haxis = axis1 vaxis = axis2  nolegend;

	note h=3 m=(1pct, 10.75 pct) "Day:" ;
	*note h=2 m=(1pct, 7.75 pct) "(No at Risk)" ;
	note h=3 m=(1pct, 8.25 pct) "(No)" ;
	format day dd. failure 4.2;
run;	

goptions reset=all  /*device=jpeg*/ gunit=pct noborder CBACK=blue ctext=white ctitle=white colors=(orange green red) ftext=Times hby = 3;

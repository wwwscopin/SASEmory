...
data column1_neeta; set ptcrf; run;
...
ptcrf2 as b
on a.id=b.id  where a.reason not in  (4,5);
...
data summary (keep =center  stat2  pipe) ; set summary;
stat2 = Trim(Left(total-count)) || "/" || Trim(Left(total)) || "\n( " || Trim(Left(put(((total-count)/total)*100,10.))) || " %)";

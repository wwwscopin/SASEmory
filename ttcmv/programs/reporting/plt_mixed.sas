...
proc sql;
	...
	select b.*
	from platelet_id as b;

proc sql;
	create table notrans as
	select a.*
	from plt as a, notrans_id as b
	where a.id=b.id
;


data plt;
	set plt platelet(keep=id DateTransfusion pltdate plt); by id;
run;
...
data plt;
	...
	if dob=. then 
	if first.id then do; base=pltDate; dob=base; retain base; end;
	else dob=base;
	day0=pltdate-dob;
	if day=. then do;
...
         					symbol3 i=j ci=red value=dot h=1 w=4;
	...
	%macro make_plots(data=trans); 
		...
		%let variable =plt; %let description = f=zapf "Platelet Count(1000/" f=greek 'm' f=zapf 'L)'; %let scale=; 
         		...
         		* populate 'n' annotation variables ;
         		...
         		run;
	      		...
	      		title2 h=3.5 justify=center &description (n=&n_total);
					*title2 h=3.5 justify=center  "E=mc^{super 2}";

...
data platelet;
	...
	rename PlateletNum=plt DatePlateletCount=pltDate;
run;


proc sql;
	create table platelet as 
	select a.*
	from platelet as a, cmv.comp_pat as b
	where a.id=b.id;
quit;
...
data platelet;
	set platelet; by id DateTransfusion; 
	if first.id and plt^=. then day=0; else day=.;
run;

data plt;
	set plt platelet(keep=id DateTransfusion pltdate plt day); by id;
run;

proc sql;
	create table trans as
	select a.id, a.pltdate, a.plt, a.day, b.DateTransfusion
	from plt as a, platelet as b
	where a.id=b.id
;
     ...
     -33=" " -32=" " -31=" " -30=" " -29=" " -28="-28*(&n_n28)" -27=" " -26=" " -25=" "
		...
		-12=" " -11=" " -10=" "   -9=" "    -8=" "   -7="-7*(&n_n7)" -6=" " - 5=" "  -4="-4*(&n_n4)"  -3=" "   -2=" "   -1=" "   
         		...
         		title3 h=3.5 justify=center Longitudinal models (means and 95% CI);
					...
					axis1 	label=(f=zapf h=3 'Days Before and After First Platelet Transfusion' ) value=(f=zapf h=2.0) order= (-60 to -40 by 20 -28 to -7 by 7 -4 to 4 by 4 7 to 28 by 7 40 to 80 by 20) split="*"  minor=none offset=(0 in, 0 in);
...
             
        ...
        goptions rotate = portrait;
        	...
        	ods ps file = "plt_trans_mixed.ps";
        	ods pdf file = "plt_trans_mixed.pdf" startpage=no;

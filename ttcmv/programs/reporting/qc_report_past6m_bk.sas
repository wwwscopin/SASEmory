...
**** collect descriptive statistics on QCs, by center ****;
	...
	proc means data = qcs n mean median min max fw = 5 maxdec = 0;
		...
		* n for days_to_resolution is a surrogate for the number of QCs resolved, since the dates are therefore non-missing ;
		...
		output out = qc_stats n(days_to_resolution) = days_n 
       median(days_to_resolution) = days_med 
	...
	%macro get_total_records (month = , year = , first_record= );
		...
		data _NULL_;
			...
			**** make 2-digit year ;
			...
			command = "/usr/local/apps/datafax/reports/DF_CTqcs 20 -t " || compress(last_year_t) || "/" || compress(last_month_t) || "/01-" || compress(year_t) || "/" || compress(month_t) || "/01 > /ttcmv/sas/data/" || compress(filename);	

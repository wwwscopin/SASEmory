...
create table rbctx as
select a.* ,b.DateDonated,b.dateirradiated_bu
from rbctx as a left join
 (  select distinct donorunitid, DateDonated,DateIrradiated as dateirradiated_bu
...
if DateIrradiated eq . and dateirradiated_bu ~=. then DateIrradiated=DateIrradiated_bu;

age_at_irradiation=(DateIrradiated -DateDonated );
if DateIrradiated ~= "" and DateIrradiated <= DateTransfusion  then do; 
	time_irradiation_tx=(DateTransfusion -DateIrradiated );end;



if time_irradiation_tx >=0 and age_at_irradiation < 0 then time_irradiation_tx=.;
run;
...
create table snap as
select a.* 
from snap as a inner join endofstudy as b
on a.id=b.id;
...
create table age_at_irradiation as
select age_at_irradiation as age_at_irradiation_max , donorunitid,center
from rbctx

; 

create table time_irradiation_tx as
select max(time_irradiation_tx) as time_irradiation_tx_max , id,center
from rbctx
group by id,center;
...
data xx; set &data; if &var=-99 then &var=.;run;


proc sort data=xx;  by center; run;

data xx; set xx; if &var < 0 and &var ~=. then &var=0;run;
proc univariate data=xx ;
by center;
var &var  ;
...
%if &subheader eq 1 %then %do;
		proc sql;
				...
				values ("&label",&grtime_irradiation_txoup,0);

		quit;

%end;

proc sql; drop table &var; drop table xx;quit;
%mend;
...
%univariate_stat (data=rbctx ,var =rbcvolumeTransfused  , label =	Volume (mL) per transfusion, group=4,stattype=1, format=6.1,subheader=0);


%univariate_stat (data=rbctx ,var =vol_wt  , label = 	Volume (mL/kg) per transfusion , group=5,stattype=1, format=6.1,subheader=0); 
...
%univariate_stat (data=age_at_irradiation ,var =age_at_irradiation_max  , label =  Age of blood at irradiation(days) , group=8,stattype=1, format=6.1,subheader=0); 

/*%univariate_stat (data=time_irradiation_tx ,var =time_irradiation_tx_max  , label =Time from irradiation to Tx(days) , group=9,stattype=1, format=6.1,subheader=0); */

%univariate_stat (data=rbctx ,var =time_irradiation_tx  , label =Time from irradiation to Tx(days) , group=9,stattype=1, format=6.1,subheader=0); 

%univariate_stat (data=rbctx ,var =center  , label =\n\S={font_weight=bold }Data on first RBC transfusion for LBWI, group=11,stattype=2, format=6.1,subheader=1);

%univariate_stat (data=time_tx ,var =time_to_tx_min  , label =  Days to first RBC Tx(days) , group=12,stattype=3, format=6.1,subheader=0); 

%univariate_stat (data=lowest_hb2 ,var =Hb  , label =  Hb level within 24hrs before first Tx(g/dl) , group=13,stattype=1, format=6.1,subheader=0); 
%univariate_stat (data=snap ,var =snaptotalscore  , label = SNAP score at Birth , group=14,stattype=3, format=6.1,subheader=0); 
%univariate_stat (data=birth ,var =birthweight , label = Weight at Birth (gms) , group=15,stattype=3, format=6.0,subheader=0); 
%univariate_stat (data=birth ,var =gestage , label = Gestational age at Birth (weeks), group=16,stattype=3, format=6.1,subheader=0); 

...
select compress(put(sum(lbwi_transfused),2.0)) into: tx_t from rbctx4 ;
...
if group=2 and center=5 then disp=&center0;
if group=4 and center=5 then disp= compress(&tx_t) || "(" || compress(put( (&tx_t/&center0)*100, 3.0)) || "%)"  ;
...
ods rtf   file = "&output./annual/&combo_tx_summary_file.rbc_tx_by_weight.rtf" style=journal   

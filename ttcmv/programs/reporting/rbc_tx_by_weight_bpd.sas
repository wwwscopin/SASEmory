...
select compress(put(count(*),2.0)) into: center_bpd from cmv.bpd;
select compress(put(sum(lbwi_transfused),2.0)) into:tx_bpd from rbctx4 where birthweight_cat is not null;

insert into cont_table2 (variable,group,center,subheader, visitlist)
values (' \n N ', 2,5,0,1);
insert into cont_table2 (variable,group,center,subheader, visitlist)
values ('Num of LBWI RBC Transfused\n \n N (%)', 4,5,0,1);



create table cont_table2 as
select * from cont_table2
order by group, center;
...
if group=2 and center=5 then disp=&center_bpd;
if group=4 and center=5 then disp= compress(&tx_bpd) || "(" || compress(put( (&tx_bpd/&center_bpd)*100, 3.0)) || "%)"  ;

if group =4  and center =1 then disp = compress(disp) || "(" || compress(put( (disp*1/&bw_n1)*100, 3.0)) || "%)"  ;
else if group =4 and center =2 then disp = compress(disp) || "(" || compress(put( (disp*1/&bw_n2)*100, 3.0))  || "%)"  ; 
else if group =4 and center =3 then disp = compress(disp) || "(" || compress(put( (disp*1/&bw_n3)*100, 3.0))  || "%)"  ; 
else if group =4 and center =4 then disp = compress(disp) || "(" || compress(put( (disp*1/&bw_n4)*100, 3.0))  || "%)"  ; 
...
ods rtf   file = "&output./annual/&combo_tx_summary_file.rbc_tx_by_weight_BPD.rtf" style=journal   

toc_data startpage = yes bodytitle ;

ods noproctitle proclabel "Summary of RBC Tx by birth weight group for LBWI with BPD.";

title1  justify = center "Summary of RBC Tx by birth weight group for LBWI with BPD.";
title2 "";

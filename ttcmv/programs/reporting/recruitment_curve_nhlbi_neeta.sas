...
run;
	...
	* generate expected recruitment ;
	data expected_recruit_all;
		...
		do month = 1 to 12;
			date = mdy(month, 1, 2013);
		output; 
		end;

   do month = 1 to 6;
			date = mdy(month, 1, 2014);
		output; 
		end;
	run;
		...
		* If you leave out the colors statement here, the symbols statements don't work. Isn't that awesome??? ;
		...
		symbol4 value = none h=3 w=3 c="red" i=join;
 		...
 		axis1 	label= (f=swissb h=3 'Study Month')
					value= (f=swiss h=1.5) 
					order= (18263 to  /*19358   18992*/ 19904 by 90)
					major= (h=3 w=2) 
					minor= none
		;
 		...
 		axis2 	label= (a=90 f=swissb h=3 'Patients Recruited') 
					value= (f=swissb h=2) 
					/*order= (0 to 400 by 25)neeta changed */ order= (0 to 700 by 50)
					major= (h=1.5 w=2) 
					minor= (number=3)
		;

 		legend1 across=1 down=2 position=(top right outside) mode=protect
 			shape=symbol(3,2) label=(f=swissb h=2 '')
...
ods pdf file = "&output./monthly/recruitment_curve_nhlbi_2014.pdf";

	title1 f=swissb h=3 justify=center "TT-CMV Patient Recruitment Summary - All Centers, July 1, 2011"; 

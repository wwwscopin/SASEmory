...
proc mixed data = chemistry_plot empirical covtest;
				...
				*where treatmentgroup in (1,2,3,0);
			where treatmentgroup in (0);
...
set  chemistry_plot;  where treatmentgroup in (0);
...
data median_mixed ;
         ...
         else  if treatmentgroup = 3 then do;  visit = visit - .2; visit_jitter = visit_jitter - .2;  end; 

			else  if treatmentgroup = 0 then do;  visit = visit - .15; visit_jitter = visit_jitter - .15;  end;


label  median_trt4="Overall"
						/*  median_trt1="EUHM"
						median_trt2="Grady" median_trt3="Northside" estimate_trt1="Estimate (EUHM)"
...
else if treatmentgroup = 3 then  color='Black';
...
x=visit; y=snap2score; function='label'; text='  '||trim(left(center));
run;


proc sql;
			create table xx as
			select center,visit, count(snap2score)  as number
			from chemistry_plot
			where center =0
...
goptions device=png target=png  xmax=10 in  xpixels=2500  
/*can not get this to be right xmax=10 in  xpixels=2500  ymax=20 in  ypixels=2000*/;
...
ods noproctitle proclabel "snap2_plots_whole_title Longitudinal SNAP II Score Plot ";
  ...
  axis2 label=(f=swissb h=1 a=90 "") value=(f=swissb h=2.5)  order=0 to 40 by 10
...
symbol1 value=circle h=1.5 color=black;
...
symbol7 value = triangle color=purple h = 3 i = join ;*/


symbol2 value = dot color=black h = 3 i = join ;
